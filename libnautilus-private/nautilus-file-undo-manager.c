...
static void
undo_info_apply_ready (GObject *source, GAsyncResult *res, void *user_data)
{
	NautilusFileUndoManager *self = user_data;
	NautilusFileUndoInfo *info = NAUTILUS_FILE_UNDO_INFO (source);
	_Bool success, user_cancel;
...
{
	_Bool undo = self->priv->state == NAUTILUS_FILE_UNDO_MANAGER_STATE_UNDO;
