...
static void
launch_application_from_command_internal (const char *full_command,
                                          GdkScreen  *screen,
                                          _Bool       use_terminal)
{
    GAppInfo *app;
			      ...
			      GtkWindow   *parent_window)
{
	...
	if (!g_file_is_native (desktop_file)) {
		...
		eel_show_error_dialog
			(_("<b>Sorry, but you cannot execute commands from a remote site.<\b>"),
			 _("This is disabled due to security considerations."),
			 "Nautilus", parent_window);
	...
	if (app_info == NULL) {
		eel_show_error_dialog
			(_("<b>There was an error launching the application.<\b>"),
			 NULL, "Nautilus",
			 parent_window);
		return;
	}
	...
	if (g_app_info_supports_files (G_APP_INFO (app_info)) &&
		...
		if (count == 0) {
			...
			eel_show_error_dialog
				(_("<b>This drop target only supports local files.<\b>"),
