...
static volatile _Bool thumbnail_thread_is_running = FALSE;
...
static NautilusThumbnailFactory *thumbnail_factory = NULL;
...
static NautilusThumbnailFactory *
get_thumbnail_factory (void)
{
	static NautilusThumbnailFactory *thumbnail_factory = NULL;

	if (thumbnail_factory == NULL) {
		thumbnail_factory = nautilus_thumbnail_factory_new (NAUTILUS_THUMBNAIL_SIZE_NORMAL);
	}
...
static int
thumbnail_thread_starter_cb (void *data)
{
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
thumbnail_thread_notify_file_changed (void *image_uri)
{
        NautilusFile *file;
...
nautilus_get_thumbnail_frame (void)
{
    ...
    {
        char *frame_filename;
        frame_filename = nautilus_get_pixmap_file ("thumbnail_frame.png");
        frame_pixbuf = gdk_pixbuf_new_from_file (frame_filename, NULL);
...
nautilus_can_thumbnail (NautilusFile *file)
{
	...
	res = nautilus_thumbnail_factory_can_thumbnail (factory,
							     uri,
							     mime_type,
							     mtime);
	g_free (mime_type);
...
#ifdef DEBUG_THUMBNAILS
		g_message ("%s (Main Thread) Updating non-current mtime: %s\n", __func__,
			   info->image_uri);
#endif
...
#ifdef DEBUG_THUMBNAILS
	g_message ("%s (Main Thread) Unlocking mutex\n", __func__);
#endif
	pthread_mutex_unlock (&thumbnails_mutex);
}
...
thumbnail_thread_start (void *data)
{
    ...
    for (;;) { /* smells bad */
        ...
        #ifdef DEBUG_THUMBNAILS
        g_message ("%s (Thumbnail Thread) Locking mutex\n", __func__);
        #endif
        pthread_mutex_lock (&thumbnails_mutex);
        ...
        if (g_queue_is_empty ((GQueue *)&thumbnails_to_make)) {
            #ifdef DEBUG_THUMBNAILS
            g_message ("%s (Thumbnail Thread) Exiting\n", __func__);
            #endif
        ...
        #ifdef DEBUG_THUMBNAILS
        g_message ("%s(Thumbnail Thread) Unlocking mutex\n", __func__);
        #endif
        pthread_mutex_unlock (&thumbnails_mutex);
        ...
        {
            #ifdef DEBUG_THUMBNAILS
            g_message ("%s (Thumbnail Thread) Skipping: %s\n", __func__,
                       info->image_uri);
            #endif
        ...
        #ifdef DEBUG_THUMBNAILS
        g_message ("%s (Thumbnail Thread) Creating thumbnail: %s\n", __func__,
                        info->image_uri);
        #endif

        pixbuf = nautilus_thumbnail_factory_generate_thumbnail (thumbnail_factory,
                                                                info->image_uri,
                                                                info->mime_type);

        if (pixbuf) {
            nautilus_thumbnail_factory_save_thumbnail (thumbnail_factory,
                                                       pixbuf,
                                                       info->image_uri,
                                                       current_orig_mtime);
            g_object_unref (pixbuf);
            ...
            nautilus_thumbnail_factory_create_failed_thumbnail (thumbnail_factory,
                                                                info->image_uri,
                                                                current_orig_mtime);
        }
        ...
        /* WEH 04/17/14: No, don't do this, it causes threads to be continuously created
         * for each image file until the user select a different folder, cpu usage goes
         * threw the roof, even the mouse does not have time to regenerate the cursor */
