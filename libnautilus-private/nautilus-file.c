...
#define ShowThumbs     show_image_thumbs
#define SizePrefix     size_prefix
#define ThumbnailLimit cached_thumbnail_limit
#define ThumbnailSize  cached_thumbnail_size
...
static unsigned int attribute_name_q,
	attribute_size_q,
	attribute_type_q,
...
static GObject*
nautilus_file_constructor (unsigned int type,
...
static _Bool
foreach_metadata_free (void *key, void *value, void *user_data)
{
	unsigned int id;
...
metadata_hash_free (GHashTable *hash)
{
	g_hash_table_foreach_remove (hash,
                                     (GHRFunc)foreach_metadata_free,
				     NULL);
	g_hash_table_destroy (hash);
}

static _Bool
metadata_hash_equal (GHashTable *hash1, GHashTable *hash2)
{
	GHashTableIter iter;
	void *key1;
        void *value1;
...
_Bool
nautilus_file_update_metadata_from_info (NautilusFile *file, GFileInfo *info)
{
	_Bool changed = FALSE;
  ...
  if (NAUTILUS_IS_DIRECTORY (directory)) {
    ...
    if (filename != NULL) {
      ...
      if (filename[0] != '\0') {
        ...
        BUG_MSG("invalid filename, empty string!");
      }
    }
    else {
    ...
    BUG_MSG("NAUTILUS_IS_DIRECTORY (directory) failed!");
  }
  return file;
}
			...
			ModifyListFunction modify_function)
{
	...
	found = g_hash_table_lookup_extended (symbolic_links, target_uri, &original_key, (void * *)&list_ptr);
	if (!found) {
...
static void
symbolic_link_weak_notify (void    *data,
...
static NautilusFile *
nautilus_file_get_internal (GFile *location, _Bool create)
{
  _Bool              self_owned;
  ...
  if (location != NULL) {
    ...
    if (file != NULL) {
      ...
      if (self_owned) {
        if (directory->details->as_file == NULL) {
          ...
          BUG_MSG("directory->details->as_file != NULL!");
        }
      }
      else {
    ...
    BUG_MSG("Location location is NULL");
    file = NULL;
  }
  return file;
}
...
finalize (GObject *object)
{
  ...
  if (file->details->get_info_error) {
    g_clear_error (&file->details->get_info_error);
  }
...
{
  NautilusFile *ptr = NULL;

  if (IS_A_NAUTILUS_FILE (file)) {

#ifdef NAUTILUS_FILE_DEBUG_REF
	DEBUG_REF_PRINTF("%s %10p ref'd", file->details->display_name, file);
#endif
    ptr = (NautilusFile*)g_object_ref (file);
  }
...
nautilus_file_unref (NautilusFile *file)
{
  if (IS_A_NAUTILUS_FILE (file)) {
    ...
    if ( G_OBJECT(file)->ref_count > 0) {

#ifdef NAUTILUS_FILE_DEBUG_REF
      DEBUG_REF_PRINTF("ref_count=<%d> %10p unref'd\n",  G_OBJECT(file)->ref_count, file);
#endif
...
nautilus_file_get_parent (NautilusFile *file)
{
  ...
  if (IS_A_NAUTILUS_FILE (file)) {
    ...
    if (!nautilus_file_is_self_owned (file)) {
      ...
      parent = nautilus_directory_get_corresponding_file (file->details->directory);
    }
  }
  else {
...
nautilus_file_can_execute (NautilusFile *file)
{
  ...
  if (IS_A_NAUTILUS_FILE (file)) {
    ...
    answer = file->details->can_execute;
  }
  else {
    ERROR_NOT_NAUTILUS_FILE;
  }
...
nautilus_file_can_unmount (NautilusFile *file)
{
  ...
  if (IS_A_NAUTILUS_FILE (file)) {
    ...
    if ( !getenv("LTSP_CLIENT")) {
      answer = file->details->can_unmount ||
      (file->details->mount != NULL && g_mount_can_unmount (file->details->mount));
    }
  }
  else {
    ERROR_NOT_NAUTILUS_FILE;
...
nautilus_file_can_eject (NautilusFile *file)
{
  ...
  if (IS_A_NAUTILUS_FILE (file)) {
    ...
    if (!getenv("LTSP_CLIENT")) {
      answer = file->details->can_eject ||
      (file->details->mount != NULL && g_mount_can_eject (file->details->mount));
    }
  }
  else {
    ERROR_NOT_NAUTILUS_FILE;
...
nautilus_file_can_start (NautilusFile *file)
{
  ...
  if (IS_A_NAUTILUS_FILE (file)) {
    ...
    if (file->details->can_start) {
      ...
      if (drive != NULL) {
        answer = g_drive_can_start (drive);
...
nautilus_file_can_start_degraded (NautilusFile *file)
{
  ...
  if (IS_A_NAUTILUS_FILE (file)) {
    ...
    if (file->details->can_start_degraded) {
      ...
      drive = g_mount_get_drive (file->details->mount);
      if (drive != NULL) {
        answer = g_drive_can_start_degraded (drive);
        g_object_unref (drive);
      }
    }
  }
...
nautilus_file_can_poll_for_media (NautilusFile *file)
{
  ...
  if (IS_A_NAUTILUS_FILE (file)) {
    ...
    if (file->details->can_poll_for_media) {
      ...
      drive = g_mount_get_drive (file->details->mount);
      if (drive != NULL) {
        answer = g_drive_can_poll_for_media (drive);
        g_object_unref (drive);
      }
    }
  }
...
nautilus_file_is_media_check_automatic (NautilusFile *file)
{
  ...
  if (IS_A_NAUTILUS_FILE (file)) {
    ...
    if (file->details->is_media_check_automatic) {
      ...
      drive = g_mount_get_drive (file->details->mount);
      if (drive != NULL) {
        answer = g_drive_is_media_check_automatic (drive);
        g_object_unref (drive);
      }
    }
  }
...
{
  GDriveStartStopType answer = G_DRIVE_START_STOP_TYPE_UNKNOWN; /* = 0 */
  GDrive *drive;

  if (IS_A_NAUTILUS_FILE (file)) {

    answer = file->details->start_stop_type;

    if (answer == G_DRIVE_START_STOP_TYPE_UNKNOWN) {

      if (file->details->mount != NULL) {
        drive = g_mount_get_drive (file->details->mount);
        if (drive != NULL) {
          answer = g_drive_get_start_stop_type (drive);
          g_object_unref (drive);
        }
      }
    }
                     ...
                     void                          *callback_data)
{
  ...
  if (NAUTILUS_FILE_GET_CLASS (file)->mount == NULL) {
    if (callback) {
      g_set_error_literal (&error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED, _("This file cannot be mounted"));
                       ...
                       void                         *callback_data)
{
  ...
  else if (file->details->mount != NULL && g_mount_can_unmount (file->details->mount))
  {
		    ...
		    void                         *callback_data)
{
  ...
  if (NAUTILUS_FILE_GET_CLASS (file)->stop != NULL) {
    ...
    if (drive != NULL && g_drive_can_stop (drive)) {
      ...
      if (callback) {
        error = NULL;
        g_set_error_literal (&error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED, _("This file cannot be stopped"));
        callback (file, NULL, error, callback_data);
        g_clear_error (&error);
      }
    }
...
can_rename_desktop_file (NautilusFile *file)
{
  ...
  answer = g_file_is_native (location);
  g_object_unref (location);
  return answer;
}
...
nautilus_file_get_location (NautilusFile *file)
{
  ...
  if (IS_A_NAUTILUS_FILE (file)) {
    ...
    if (nautilus_file_is_self_owned (file)) {
      dir = g_object_ref (dir);
    }
    else {
      dir = g_file_get_child (dir, eel_ref_str_peek (file->details->name));
    }
  }
  else {
    ERROR_NOT_NAUTILUS_FILE;
...
char *
nautilus_file_get_path (NautilusFile *file)
{
  char  *path = NULL;
  GFile *location;
...
{
	/* Claim that something changed even if the operation failed.  This
         * makes it easier for some clients who see the "reverting" as
         * "changing back".
	 */
	nautilus_file_operation_remove (op);
...
static void
rename_callback (GObject *source_object, GAsyncResult *res, void *callback_data)
{
	NautilusFileOperation *op;
	GFile  *new_file;
	GError *error;
		      ...
		      void * callback_data)
{
	...
	char *old_name;
	char *new_file_name;
	_Bool success, name_changed;
	_Bool is_renameable_desktop_file;
	GFile *location;
	GError *error;
		      ...
		      _Bool update_name)
{
	GList *node;
	_Bool changed;
	_Bool is_symlink, is_hidden, is_mountpoint;
	_Bool has_permissions;
	unsigned int permissions;
	_Bool can_read, can_write, can_execute, can_delete, can_trash, can_rename, can_mount, can_unmount, can_eject;
	_Bool can_start, can_start_degraded, can_stop, can_poll_for_media, is_media_check_automatic;
	GDriveStartStopType start_stop_type;
	_Bool thumbnailing_failed;
	int uid, gid;
	goffset size;
	int sort_order;
...
static Knowledge
get_item_count (NautilusFile *file, unsigned int *count)
{
	_Bool known, unreadable;
...
static Knowledge
get_size (NautilusFile *file, goffset *size)
{
...
compare_by_size (NautilusFile *file_1, NautilusFile *file_2)
{
	...
	_Bool is_directory_1, is_directory_2;
...
static int
compare_by_type (NautilusFile *file_1, NautilusFile *file_2, _Bool detailed)
{
  _Bool is_directory_1;
  _Bool is_directory_2;
  char *type_string_1;
  char *type_string_2;
  int result;
  ...
  if (detailed) {
    type_string_1 = nautilus_file_get_detailed_type_as_string (file_1);
    type_string_2 = nautilus_file_get_detailed_type_as_string (file_2);
  }
  else {
				...
				_Bool reversed)
{
	...
	if (result == 0) {
		...
		case NAUTILUS_FILE_SORT_BY_TYPE:
			...
			break;
                 case NAUTILUS_FILE_SORT_BY_DETAILED_TYPE:
...
static _Bool
filter_hidden_partition_callback (void * data,
				  void * callback_data)
{
	NautilusFile *file;
				  ...
				  _Bool  show_hidden)
{
	...
	filtered_files = eel_g_list_partition (filtered_files,
					     (EelPredicateFunction)filter_hidden_partition_callback,
                                    ...
                                    int           default_metadata)
{
    ...
    if (file == NULL) {
        ...
        result_as_string = nautilus_file_get_metadata (file, key, default_as_string);

        /* Normally we can't get a NULL, but we check for it here to
...
nautilus_file_should_show_thumbnail (NautilusFile *file)
{
	...
	if (nautilus_thumbnail_is_mimetype_limited_by_size (mime_type) &&
	    file->details->thumbnail_path == NULL &&
	    nautilus_file_get_size (file) > ThumbnailLimit) {
		return FALSE;
	}

	if (ShowThumbs == NAUTILUS_SPEED_TRADEOFF_ALWAYS) {
	...
	else if (ShowThumbs == NAUTILUS_SPEED_TRADEOFF_NEVER) {
		return FALSE;
	}
	else {
		if (use_preview == G_FILESYSTEM_PREVIEW_TYPE_NEVER) {
			 ...
			 NautilusFileIconFlags flags)
{
	...
	GEmblem *emblem;
	int i;
	_Bool is_folder = FALSE, is_preview = FALSE, is_inode_directory = FALSE;
    ...
    if (file != NULL) {
        ...
        if (gicon) {
            ...
            if (flags & NAUTILUS_FILE_ICON_FLAGS_FORCE_THUMBNAIL_SIZE) {
                ...
                modified_size = size * ThumbnailSize / NAUTILUS_ICON_SIZE_STANDARD;
                DEBUG ("Modifying icon size to %d, as our cached thumbnail size is %d",
                        modified_size, ThumbnailSize);
            }
				  ...
				  void *measure_context)
{
	...
	if (DateFormat == NAUTILUS_DATE_FORMAT_LOCALE) {
		return eel_strdup_strftime ("%c", file_time);
	}
	else if (DateFormat == NAUTILUS_DATE_FORMAT_ISO) {
...
static void
show_text_in_icons_changed_callback (void * callback_data)
{
	show_text_in_icons = eel_settings_get_enum (nautilus_preferences, NAUTILUS_PREFERENCES_SHOW_TEXT_IN_ICONS);
}

static void
show_directory_item_count_changed_callback (void * callback_data)
{
	show_directory_item_count = eel_settings_get_enum (nautilus_preferences, NAUTILUS_PREFERENCES_SHOW_DIRECTORY_ITEM_COUNTS);
}
...
{
	static _Bool show_directory_item_count_callback_added = FALSE;
...
{
	static _Bool show_text_in_icons_callback_added = FALSE;
...
nautilus_file_get_octal_permissions_as_string (NautilusFile *file)
{
	...
	g_assert (IS_A_NAUTILUS_FILE (file));
 ...
 **/
char *
nautilus_file_get_owner_as_string (NautilusFile *file, _Bool include_real_name)
{
	char *user_name;
...
static char *
format_item_count_for_display (unsigned int item_count,
			       _Bool includes_directories,
			       _Bool includes_files)
{
...
/**
 * \brief Get File preference dependent File Size String
 *
...
nautilus_file_get_size_as_string (NautilusFile *file)
{
   ...
   _Bool      count_unreadable;
    ...
    if (file != NULL) {
        ...
        size_string = g_format_size_full (file->details->size, SizePrefix);
    }
    return size_string;
}
...
nautilus_file_get_size_as_string_with_real_size (NautilusFile *file)
{
    ...
    if (file != NULL) {
        ...
        prefix = (SizePrefix >> 1) << 1;
        size_string = g_format_size_full (file->details->size, G_FORMAT_SIZE_LONG_FORMAT + prefix);
    }
    return size_string;
}
...
nautilus_file_get_deep_count_as_string_internal (NautilusFile *file,
                                                 _Bool report_size,
                                                 _Bool report_directory_count,
                                                 _Bool report_file_count)
{
        NautilusRequestStatus status;
        unsigned int directory_count;
        unsigned int file_count;
        unsigned int unreadable_count;
        unsigned int hidden_count;
         ...
         */
        if (report_size) {
                return g_format_size_full (total_size, SizePrefix);
        }
 ...
 **/
char *
nautilus_file_get_string_attribute_q (NautilusFile *file, unsigned int attribute_q)
{
 ...
 **/
char *
nautilus_file_get_string_attribute_with_default_q (NautilusFile *file, unsigned int attribute_q)
{
        char *result;
        unsigned int item_count;
        _Bool count_unreadable;
        ...
        if (attribute_q == attribute_deep_size_q) {
                status = nautilus_file_get_deep_counts (file, NULL, NULL, NULL, NULL, NULL, FALSE);
...
_Bool
nautilus_file_is_date_sort_attribute_q (unsigned int attribute_q)
{
...
static char *
get_description (NautilusFile *file, _Bool detailed)
{
    const char *mime_type;
...
nautilus_file_get_volume_free_space (NautilusFile *file)
{
    ...
    if (file->details->free_space != (guint64)-1) {
        size_string = g_format_size_full (file->details->free_space, SizePrefix);
    }
                              ...
                              NautilusFileAttributes file_attributes)
{
  ...
  /* To be parallel with call_when_ready, return TRUE for NULL file. */

  if (IS_A_NAUTILUS_FILE (file)) {

    if (NAUTILUS_FILE_CLASS (G_OBJECT_GET_CLASS (file))->call_when_ready) {
      answer = NAUTILUS_FILE_CLASS (G_OBJECT_GET_CLASS (file))->check_if_ready (file, file_attributes);
    }
    else {
      fprintf(stderr,"%s error\n", __func__);
      answer = FALSE;
    }
    /* fall-out botton returning TRUE */;
  }
  else {
    answer = FALSE;
  }
                               ...
                               void *callback_data)
{
  ...
  if (IS_A_NAUTILUS_FILE (file)) {
    ...
    if (NAUTILUS_FILE_CLASS (G_OBJECT_GET_CLASS (file))->call_when_ready) {
      ...
      NAUTILUS_FILE_CLASS (G_OBJECT_GET_CLASS (file))->call_when_ready (file, file_attributes, callback, callback_data);
    }
    else {
      fprintf(stderr,"%s error\n", __func__);
    }
...
static GString *
add_line (GString *string, const char *add, _Bool prefix_newline)
{
    if (prefix_newline)
...
char *
nautilus_file_construct_tooltip (NautilusFile *file, NautilusFileIconFlags flags)
{
    char *scheme = nautilus_file_get_uri_scheme (file);
    char *nice = NULL;
    char *tmp = NULL;
    char *date;
    char *ret;

    if (g_strcmp0 (scheme, "x-nautilus-desktop") == 0) {
        g_free (scheme);
        return NULL;
    }
    ...
    if (flags & NAUTILUS_FILE_TOOLTIP_FLAGS_FILE_TYPE) {
        tmp = nautilus_file_get_detailed_type_as_string (file);
    ...
    if (nautilus_file_is_directory (file)) {
        int item_count;
        nautilus_file_get_directory_item_count (file, &item_count, NULL);
        char *launchpad_sucks = THOU_TO_STR (item_count);
        char *count = g_strdup_printf (ngettext ("%s item", "%s items", item_count), launchpad_sucks);
        ...
        char *size_string;
        size_string = g_format_size_full (nautilus_file_get_size (file), SizePrefix);
    ...
    if (flags & NAUTILUS_FILE_TOOLTIP_FLAGS_ACCESS_DATE) {
        date = nautilus_file_get_date_as_string (file, NAUTILUS_DATE_TYPE_ACCESSED);
    ...
    if (flags & NAUTILUS_FILE_TOOLTIP_FLAGS_MOD_DATE) {
        date = nautilus_file_get_date_as_string (file, NAUTILUS_DATE_TYPE_MODIFIED);
    ...
    if (flags & NAUTILUS_FILE_TOOLTIP_FLAGS_PATH) {
        NautilusFile *parent = nautilus_file_get_parent (file);
        tmp = nautilus_file_get_path (parent);
    ...
    if (flags & NAUTILUS_FILE_TOOLTIP_FLAGS_PERMISSIONS) {
        tmp = nautilus_file_get_permissions_as_string (file);
        nice = g_strdup_printf (_("Permissions: %s"), tmp);
        string = add_line (string, nice, TRUE);
        g_free (tmp);
        g_free (nice);
    }

    if (flags & NAUTILUS_FILE_TOOLTIP_FLAGS_OCTAL) {
        tmp = nautilus_file_get_octal_permissions_as_string (file);
        nice = g_strdup_printf (_("Octal Permissions: %s"), tmp);
        string = add_line (string, nice, TRUE);
        g_free (tmp);
        g_free (nice);
    }
...
char *
nautilus_file_construct_treetip (NautilusFile *file, NautilusFileIconFlags flags)
{
    char *scheme = nautilus_file_get_uri_scheme (file);
    char *nice = NULL;
    char *tmp = NULL;
...
int
nautilus_file_get_monitor_number (NautilusFile *file)
{
    return nautilus_file_get_integer_metadata (file, NAUTILUS_METADATA_KEY_MONITOR, 0);
}
...
void
nautilus_file_increment_thumbnail_try_count (NautilusFile *file)
{
    g_return_if_fail (NAUTILUS_IS_FILE (file));
...
get_attributes_for_default_sort_type (NautilusFile *file,
                                      ...
                                      _Bool *is_trash)
{
  _Bool is_download_dir, is_desktop_dir, is_trash_dir, ret_val;
...
nautilus_file_get_default_sort_type (NautilusFile *file, _Bool *reversed)
{
  NautilusFileSortType ret_val;
  _Bool is_recent, is_download, is_trash, result;

  ret_val = NAUTILUS_FILE_SORT_NONE;

  is_recent = is_download = is_trash = FALSE;
  result = get_attributes_for_default_sort_type (file, &is_recent, &is_download, &is_trash);

  if (result) {
    if (is_recent) {
      ret_val = NAUTILUS_FILE_SORT_BY_ATIME;
    } else if (is_download) {
      ret_val = NAUTILUS_FILE_SORT_BY_MTIME;
    } else if (is_trash) {
      ret_val = NAUTILUS_FILE_SORT_BY_TRASHED_TIME;
    }
...
const char *
nautilus_file_get_default_sort_attribute (NautilusFile *file, _Bool *reversed)
{
  const char *ret_val;
  _Bool is_recent, is_download, is_trash, res;
  ...
  if (res) {
    if (is_recent || is_download) {
      ret_val = g_quark_to_string (attribute_date_modified_q);
    } else if (is_trash) {
      ret_val = g_quark_to_string (attribute_trashed_on_q);
    }
...
char *
nautilus_extract_top_left_text (const char *text, _Bool large, int length)
{
    GString* buffer;
    gunichar c;
...
/* ---------- Callback handler for Preference Settings change ---------- */

static void
nautilus_file_size_prefix_preference_changed (void *nothing)
{
    SizePrefix = nautilus_settings_get_size_prefixes();
   /* This only servers to update display while pref dialog is open, other
    * wise, the size format would not update until the dialog closed */
    emit_change_signals_for_all_files_in_all_directories ();

}

static void
date_format_pref_preference_changed_callback (void *nothing)
{
    DateFormat  = eel_settings_get_enum (nautilus_preferences,
                                          NAUTILUS_PREFERENCES_DATE_FORMAT);
}

static void
thumbnail_limit_prefs_changed_callback (void *nothing)
{
...
static void
thumbnail_size_prefs_changed_callback (void *nothing)
{
    ThumbnailSize =
        eel_settings_get_int (nautilus_icon_view_preferences,
...
static void
show_thumbnails_prefs_changed_callback (void *nothing)
{
	ShowThumbs = eel_settings_get_enum (nautilus_preferences,
...
static void
mime_type_data_changed_callback (GObject *signaller, void *user_data)
{
	/* Tell the world that icons might have changed. We could invent a narrower-scope
	 * signal to mean only "thumbnails might have changed" if this ends up being slow
	 * for some reason.
...
static void
icon_theme_changed_callback (GtkIconTheme *icon_theme, void *user_data)
{
...
/* End Settings changed callbacks */

static void
real_set_metadata (NautilusFile *file, const char *key, const char *value)
{
	/* Dummy default impl */
}

static void
real_set_metadata_as_list (NautilusFile *file, const char *key, char **value)
{
	/* Dummy default impl */
}
...
nautilus_file_class_init (NautilusFileClass *class)
{
	...
	g_type_class_add_private (class, sizeof (NautilusFileDetails));
        ...
        g_signal_connect_swapped (nautilus_preferences,
                                  "changed::" NAUTILUS_PREFERENCES_SIZE_PREFIXES,
                                  G_CALLBACK(nautilus_file_size_prefix_preference_changed),
                                  NULL);

        g_signal_connect_swapped (nautilus_preferences,
                                  "changed::" NAUTILUS_PREFERENCES_DATE_FORMAT,
                                  G_CALLBACK(date_format_pref_preference_changed_callback),
                                  NULL);

        SizePrefix = nautilus_settings_get_size_prefixes();

        /* Retrieve thumbnail limit settings */
        g_settings_get (nautilus_preferences,
                        NAUTILUS_PREFERENCES_IMAGE_FILE_THUMBNAIL_LIMIT,
                        "t", &ThumbnailLimit);

        /* Set notify if thumbnail limit settings changes */
        g_signal_connect_swapped (nautilus_preferences,
                                  "changed::" NAUTILUS_PREFERENCES_IMAGE_FILE_THUMBNAIL_LIMIT,
                                  G_CALLBACK (thumbnail_limit_prefs_changed_callback),
                                  NULL);

        ThumbnailSize = eel_settings_get_int (nautilus_icon_view_preferences,
                                              NAUTILUS_PREFERENCES_ICON_VIEW_THUMBNAIL_SIZE);

        g_signal_connect_swapped (nautilus_preferences,
                                  "changed::" NAUTILUS_PREFERENCES_ICON_VIEW_THUMBNAIL_SIZE,
                                  G_CALLBACK (thumbnail_size_prefs_changed_callback),
                                  NULL);
        ...
        g_signal_connect_swapped (nautilus_preferences,
                                  ...
                                  G_CALLBACK (show_thumbnails_prefs_changed_callback),
                                  NULL);
...
{
        file->head_marker  = nautilus_file_get_type();
