...
update_buttons (NautilusColumnChooser *chooser)
{
	...
	if (gtk_tree_selection_get_selected (selection, NULL, &iter)) {
		...
		gtk_widget_set_sensitive (chooser->details->move_up_button, !top);
		gtk_widget_set_sensitive (chooser->details->move_down_button, !bottom);
	}
	else {
...
static void
visible_toggled_callback (GtkCellRendererToggle *cell, char *path_string, void *user_data)
{
	NautilusColumnChooser *chooser;
	GtkTreePath           *path;
	GtkTreeIter            iter;
	int                    visible; /* can not be boolean */

	chooser = NAUTILUS_COLUMN_CHOOSER (user_data);

	path = gtk_tree_path_new_from_string (path_string);

	gtk_tree_model_get_iter (GTK_TREE_MODEL (chooser->details->store), &iter, path);

	gtk_tree_model_get (GTK_TREE_MODEL (chooser->details->store), &iter, COLUMN_VISIBLE, &visible, -1);

	gtk_list_store_set (chooser->details->store, &iter, COLUMN_VISIBLE, !visible, -1);
...
static void
selection_changed_callback (GtkTreeSelection *selection, void *user_data)
{
...
static void
move_up_clicked_callback (GtkWidget *button, void *user_data)
{
	NautilusColumnChooser *chooser;
	GtkTreeIter iter;
	GtkTreeSelection *selection;
...
static void
move_down_clicked_callback (GtkWidget *button, void *user_data)
{
	NautilusColumnChooser *chooser;
	GtkTreeIter iter;
	GtkTreeSelection *selection;
...
static void
use_default_clicked_callback (GtkWidget *button, void *user_data)
{
...
static GtkWidget *
button_new_with_mnemonic (const char *stockid, const char *str)
{
	GtkWidget *image;
	GtkWidget *button;

	button = gtk_button_new_with_mnemonic (str);
	image = gtk_image_new_from_stock (stockid, STD_ICON_SIZE_BUTTON);
...
nautilus_column_chooser_init (NautilusColumnChooser *chooser)
{
	...
	g_object_set (G_OBJECT (chooser), "homogeneous", FALSE,
		      "spacing", 8,
...
static char **
get_column_names (NautilusColumnChooser *chooser, _Bool only_visible)
{
	GPtrArray *ret;
	GtkTreeIter iter;
