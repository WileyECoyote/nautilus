...
static char * nautilus_icon_container_find_drop_target (NautilusIconContainer *container,
							...
							int x, int y, _Bool *icon_hit,
							_Bool rewrite_desktop);
...
nautilus_icon_container_each_selected_icon (NautilusIconContainer *container,
	_Bool (*each_function) (NautilusIcon *, gpointer), gpointer data)
{
	GList *p;
	NautilusIcon *icon;
                   ...
                   double world_x, double world_y)
{
	...
	for (p = container->details->dnd_info->drag_info.selection_list; p != NULL; p = p->next) {
        ...
        if (file == NULL)
            file = NAUTILUS_FILE (icon->data);

        //nautilus_file_set_is_desktop_orphan (file, FALSE);

        monitor = nautilus_ui_get_monitor_for_widget (GTK_WIDGET (container));
        nautilus_file_set_integer_metadata (file, NAUTILUS_METADATA_KEY_MONITOR, 0, monitor);
