...
static void
update_mimelist (NautilusMimeApplicationChooser *chooser, const char *fn, _Bool def)
{
    char *list_fn = g_build_filename (g_get_user_data_dir (), "applications", "mimeapps.list", NULL);
    GKeyFile *mimeapps;
    GFile *list_file;
    ...
    if (def) {
        ...
        char **l = g_key_file_get_string_list (mimeapps,
                                                ADDED_ASS,
                                                chooser->details->content_type,
                                                &count,
                                                NULL);

        char *new_string_list;
    ...
    unsigned int size;
    char *buffer = g_key_file_to_data (mimeapps, &size, NULL);
...
static void
create_custom_desktop_file (NautilusMimeApplicationChooser *chooser, _Bool def)
{
    ...
    unsigned int size;
    char *buffer = g_key_file_to_data (keyfile, &size, NULL);

    signed int rn = g_random_int_range (0, G_MAXINT32 - 1);
    char *fn = g_strdup_printf ("nautilus_%s_%d.desktop",
                                 g_app_info_get_display_name (chooser->details->custom_info),
                                 rn);

    char *path = g_build_filename (g_get_user_data_dir (), "applications", fn, NULL);
	...
	if (info) {
		...
		if (!g_app_info_remove_supports_type (info,
			...
			eel_show_error_dialog (_("Could not forget association"),
					       error->message, _("Application Chooser"),
...
static _Bool
app_info_can_add (GAppInfo *info, const char *content_type)
{
...
static void
custom_entry_changed_cb (GtkEditable *entry, void    *user_data)
{
    NautilusMimeApplicationChooser *chooser = user_data;
    ...
    if (g_strcmp0 (entry_text, "") != 0) {
        GAppInfo *default_app;
        char *cl = g_strdup_printf ("%s", entry_text);
...
{
	char *label, *extension = NULL, *description = NULL;
...
GAppInfo *
nautilus_mime_application_chooser_get_info (NautilusMimeApplicationChooser *chooser)
{
    if (chooser->details->custom_info == NULL)
...
const char *
nautilus_mime_application_chooser_get_uri (NautilusMimeApplicationChooser *chooser)
{
