...
struct ThumbnailState {
	...
	GCancellable *cancellable;
	NautilusFile *file;
	_Bool trying_original;
	_Bool tried_original;
};
...
typedef struct {
	...
	void * callback_data;
	Request request;
	_Bool active; /* Set to FALSE when the callback is triggered and
...
typedef struct {
	NautilusFile *file; /* Which file, NULL means all. */
	_Bool monitor_hidden_files; /* defines whether "all" includes hidden files */
	const void * client;
	Request request;
} Monitor;
...
typedef _Bool (* RequestCheck) (Request);
typedef _Bool (* FileCheck) (NautilusFile *);
...
static void     deep_count_load                               (DeepCountState         *state,
							       GFile                  *location);
static _Bool request_is_satisfied                          (NautilusDirectory      *directory,
							       NautilusFile           *file,
							       Request                 request);
...
static _Bool
istr_equal (const void * v, const void * v2)
{
 return (g_ascii_strcasecmp (v, v2) == 0);
}

static unsigned int
istr_hash (const void * key)
{
	const char *p;
	guint h;
...
istr_set_new (void)
{
 return g_hash_table_new_full (istr_hash, (GEqualFunc)istr_equal, g_free, NULL);
}
...
static void
add_istr_to_list (void * key, void * value, void * callback_data)
{
	GList **list;
 ...
 */
static _Bool
async_job_start (NautilusDirectory *directory, const char *job)
{
  _Bool ret_val = FALSE;
...
/* End a job. */
static void
async_job_end (NautilusDirectory *directory, const char *job)
{
#ifdef DEBUG_ASYNC_JOBS
  char *key;
  void *table_key, value;
#endif
...
static void
get_one_value_callback (void *key, void *value, void *callback_data)
{
  void **returned_value;
...
{
  static _Bool already_waking_up = FALSE;
  void *value;
...
static int
monitor_key_compare (const void *a, const void *data)
{
...
static GList *
find_monitor (NautilusDirectory *directory, NautilusFile *file, const void *client)
{
  Monitor monitor;
...
static void
remove_monitor (NautilusDirectory *directory, NautilusFile *file, const void *client)
{
...
static void
set_file_unconfirmed (NautilusFile *file, _Bool unconfirmed)
{
  NautilusDirectory *directory;
...
should_skip_file (NautilusDirectory *directory, GFileInfo *info)
{
    ...
    if (is_hidden || is_backup) {
        ...
        show_hidden_files = nautilus_settings_get_show_hidden();

        if (!show_hidden_files && ((directory != NULL && directory->details->hidden_file_hash != NULL &&
            g_hash_table_lookup (directory->details->hidden_file_hash, g_file_info_get_name (info)) != NULL)))
        {
            ret_val = TRUE;
        }
    }
...
static int
dequeue_pending_idle_callback (void * callback_data)
{
  NautilusDirectory *directory;
...
static _Bool
remove_callback (void *key, void *value, void *user_data)
{
 return TRUE;
}
...
file_list_cancel (NautilusDirectory *directory)
{
	...
	EEL_SOURCE_REMOVE_IF_THEN_ZERO (directory->details->dequeue_pending_idle_id);
	...
	if (directory->details->hidden_file_hash) {
		g_hash_table_foreach_remove (directory->details->hidden_file_hash,(GHRFunc) remove_callback, NULL);
	}
}

static void
directory_load_done (NautilusDirectory *directory, GError *error)
{
  GList *node;
...
static int
ready_callback_key_compare (const void * a, const void * b)
{
...
static int
ready_callback_key_compare_only_active (const void * a, const void * b)
{
...
static void
ready_callback_call (NautilusDirectory *directory, const ReadyCallback *callback)
{
  GList *file_list;
                                             ...
                                             void                     *callback_data)
{
  ...
  if (directory == NULL || NAUTILUS_IS_DIRECTORY (directory)) {

    if (file == NULL || NAUTILUS_IS_FILE (file)) {
                                             ...
                                             void * callback_data)
{
  ...
  if ((directory != NULL) && (NAUTILUS_IS_DIRECTORY (directory))) {
    ...
    if (file == NULL || NAUTILUS_IS_FILE (file)) {
      ...
      if (file != NULL || directory_callback != NULL) {
...
static void
new_files_callback (GObject *source_object, GAsyncResult *res, void *user_data)
{
  NautilusDirectory *directory;
  GFileInfo         *info;
  NewFilesState     *state;
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
call_ready_callbacks_at_idle (void * callback_data)
{
  NautilusDirectory *directory;
  GList *node, *next;
  ReadyCallback *callback;
...
static void
more_files_callback (GObject *source_object, GAsyncResult *res, void *user_data)
{
  DirectoryLoadState *state;
  NautilusDirectory  *directory;
  GError             *error;
...
start_monitoring_file_list (NautilusDirectory *directory)
{
  ...
  if (!directory->details->file_list_monitored) {
    ...
    if (!directory->details->directory_load_in_progress) {
...
count_children_done (NautilusDirectory *directory,
                     ...
                     int count)
{
  if (NAUTILUS_IS_FILE (count_file)) {
...
static inline _Bool
seen_inode (DeepCountState *state, GFileInfo *info)
{
...
static inline void
mark_inode_as_seen (DeepCountState *state, GFileInfo *info)
{
  guint64 inode;
...
static void
deep_count_one (DeepCountState *state, GFileInfo *info)
{
  NautilusFile *file;
  GFile *subdir;
  _Bool is_seen_inode;
...
static void
mime_list_done (MimeListState *state, _Bool success)
{
  NautilusFile *file;
  NautilusDirectory *directory;
      ...
      return;
                ...
                *doing_io = TRUE;
                ...
                g_file_load_partial_contents_async (location,
                                                    state->cancellable,
                                                    (GFileReadMoreCallback) /* aka a boolean */
...
static GdkPixbuf*
get_pixbuf_for_content (goffset file_len, char *file_contents)
{
  _Bool res;
  GdkPixbuf         *pixbuf;
  GdkPixbuf         *pixbuf2;
...
static void
thumbnail_start (NautilusDirectory *directory, NautilusFile *file, _Bool *doing_io)
{
  GFile *location;
  ThumbnailState *state;
...
extension_info_cancel (NautilusDirectory *directory)
{
  if (directory->details->extension_info_in_progress != NULL) {
    if (directory->details->extension_info_idle) {
      EEL_SOURCE_REMOVE_THEN_ZERO (directory->details->extension_info_idle);
    }
    else {
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
info_provider_idle_callback (void * user_data)
{
