...
G_DEFINE_TYPE (NautilusTreeViewDragDest, nautilus_tree_view_drag_dest, G_TYPE_OBJECT);
...
{
  EEL_SOURCE_REMOVE_IF_THEN_ZERO (dest->details->scroll_id);
}

static int
expand_timeout (void *data)
{
        NautilusTreeViewDragDest *dest = data;
        GtkTreeView *tree_view;
...
{
  EEL_SOURCE_REMOVE_IF_THEN_ZERO (dest->details->expand_id);
}
...
static void
set_widget_highlight (NautilusTreeViewDragDest *dest, _Bool highlight)
{
...
static _Bool
receive_xds (NautilusTreeViewDragDest *dest, GtkWidget *widget,
             unsigned int time, GdkDragContext *context, int x, int y)
{
                   GFile *location;
    const unsigned char  *selection_data;
...
static void
tree_view_weak_notify (void *user_data, GObject *object)
{
	NautilusTreeViewDragDest *dest;
