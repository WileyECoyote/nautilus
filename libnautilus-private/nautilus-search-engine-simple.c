...
search_thread_data_new (NautilusSearchEngineSimple *engine, NautilusQuery *query)
{
...
static int
search_thread_done_idle (void *user_data)
{
	SearchThreadData *data;
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
search_thread_add_hits_idle (void *user_data)
{
	SearchHits *hits;
...
static void   *
search_thread_func (void   * user_data)
{
	SearchThreadData *data;
	GFile *dir;
	GFileInfo *info;
