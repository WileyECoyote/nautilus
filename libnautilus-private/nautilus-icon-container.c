...
static NautilusIcon *get_nth_selected_icon                          (NautilusIconContainer *container,
                                                                     int                    index);
static _Bool         has_multiple_selection                         (NautilusIconContainer *container);
static _Bool         all_selected                                   (NautilusIconContainer *container);
static _Bool         has_selection                                  (NautilusIconContainer *container);
...
static _Bool         is_renaming                                    (NautilusIconContainer *container);
static _Bool         is_renaming_pending                            (NautilusIconContainer *container);
...
static double	     get_mirror_x_position                         (NautilusIconContainer *container,
								  NautilusIcon *icon,
								  double x);
static void         text_ellipsis_limit_changed_container_callback  (void                 *callback_data);
...
static void *accessible_parent_class;
...
static void
tooltip_prefs_changed_callback (NautilusIconContainer *container)
{
    container->details->show_desktop_tooltips = nautilus_settings_get_show_desktop_tooltips();

    container->details->show_icon_view_tooltips = nautilus_settings_get_show_icon_tooltips();

    container->details->tooltip_flags = nautilus_global_preferences_get_tooltip_flags ();
		      ...
		      NautilusIcon *icon)
{
	...
	eel_canvas_item_set (EEL_CANVAS_ITEM (icon->item),
			     "highlighted_for_selection", (_Bool   ) icon->is_selected,
			     NULL);
...
static int
keyboard_icon_reveal_timeout_callback (void *data)
{
    NautilusIconContainer *container;
    NautilusIcon          *icon;
    ...
    if (icon != NULL) {
         ...
         */
        if (icon == container->details->keyboard_focus || icon->is_selected) {
...
unschedule_keyboard_icon_reveal (NautilusIconContainer *container)
{
    ...
    details->keyboard_icon_to_reveal = NULL;

    EEL_SOURCE_REMOVE_IF_THEN_ZERO (details->keyboard_icon_reveal_timer_id);
}
                               ...
                               NautilusIcon          *icon)
{
    ...
    if (icon != NULL) {
        ...
        BUG_MSG("Got Null pointer to Icon!");
    }
}
...
static int
compare_icons (const void * a, const void * b, void    * icon_container)
{
...
static int
compare_icons_by_position (const void * a, const void * b)
{
...
placement_grid_new (NautilusIconContainer *container, _Bool    tight)
{
	PlacementGrid *grid;
	int width, height;
	int num_columns;
...
lay_down_icons (NautilusIconContainer *container, GList *icons, double start_y)
{
	...
	default:
		BUG_IMSG("Unknown case", container->details->layout_mode);
	}
}
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
redo_layout_callback (void *callback_data)
{
	NautilusIconContainer *container;
...
{
        EEL_SOURCE_REMOVE_IF_THEN_ZERO(container->details->idle_id);
}
...
select_range (NautilusIconContainer *container,
	      NautilusIcon *icon1,
	      NautilusIcon *icon2,
	      _Bool    unselect_outside_range)
{
	_Bool    selection_changed;
	GList *p;
	NautilusIcon *icon;
...
{
	GList *p;
	_Bool    selection_changed, is_in, canvas_rect_calculated;
	NautilusIcon *icon;
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
rubberband_timeout_callback (void *data)
{
	NautilusIconContainer *container;
	GtkWidget *widget;
...
static void
stop_rubberbanding (NautilusIconContainer *container, unsigned int time)
{
	NautilusIconRubberbandInfo *band_info;
	GList                      *icons;
	_Bool enable_animation;

	band_info = &container->details->rubberband_info;

	EEL_SOURCE_REMOVE_IF_THEN_ZERO(band_info->timer_id);
...
typedef _Bool    (* IsBetterIconFunction) (NautilusIconContainer *container,
		       ...
		       void *data)
{
	...
	case GTK_DIR_RIGHT:
		...
		break;
	default:
		BUG_IMSG("Unknown case", container->details->arrow_key_direction);
	}
...
destroy (GtkWidget *object)
{
	...
	EEL_SOURCE_REMOVE_IF_THEN_ZERO (container->details->rubberband_info.timer_id);

        EEL_SOURCE_REMOVE_IF_THEN_ZERO (container->details->idle_id);

        EEL_SOURCE_REMOVE_IF_THEN_ZERO (container->details->stretch_idle_id);

        EEL_SOURCE_REMOVE_IF_THEN_ZERO (container->details->align_idle_id);

        EEL_SOURCE_REMOVE_IF_THEN_ZERO (container->details->selection_changed_id);

        EEL_SOURCE_REMOVE_IF_THEN_ZERO(container->details->size_allocation_count_id);
...
finalize (GObject *object)
{
	NautilusIconContainerDetails *details;
        ...
        g_signal_handlers_disconnect_by_func (nautilus_tooltips_preferences,
	...
	EEL_SOURCE_REMOVE_IF_THEN_ZERO(details->a11y_item_action_idle_handler);
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
clear_size_allocation_count (void *data)
{
	NautilusIconContainer *container;
		    ...
		    GdkEventButton *event)
{
	...
	if (clicked_on_icon) {
		NautilusIcon *icon;//current icon which was clicked on
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
update_stretch_at_idle (void *data)
{
    NautilusIconContainer *container = NAUTILUS_ICON_CONTAINER(data);
                     ...
                     double world_x, double world_y)
{
    ...
    if (container->details->stretch_idle_id == 0) {
        container->details->stretch_idle_id = g_idle_add (update_stretch_at_idle, container);
    }
}
...
static void
send_focus_change (GtkWidget *widget, _Bool    in)
{
	GdkEvent *fevent;
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
nautilus_icon_container_search_entry_flush_timeout (void *data)
{
...
{
  EEL_SOURCE_REMOVE_IF_THEN_ZERO (container->details->typeselect_flush_timeout);
}
  ...
  if (text != NULL) {
    ...
    if (container->details->selected_iter != 0) {
      ...
      if (len > 0) {
        ...
        found = nautilus_icon_container_search_iter (container, text,
                                     ...
                                     NautilusIconContainer *container)
{
  ...
  const char  *text;
	...
	found = nautilus_icon_container_search_iter (container, text, 1);
...
key_press_event (GtkWidget *widget, GdkEventKey *event)
{
    ...
    event->keyval != GDK_KEY_Delete)
  {
    ...
    g_signal_handler_disconnect (container->details->search_entry, popup_menu_id);
...
static void
draw_canvas_background (EelCanvas *canvas, cairo_t *cr)
{
...
static void
grab_notify_cb  (GtkWidget *widget, int was_grabbed)
{
	NautilusIconContainer *container;
...
static void
text_ellipsis_limit_changed_container_callback (void *callback_data)
{
	NautilusIconContainer *container;
...
static GObject*
nautilus_icon_container_constructor (unsigned int           type,
...
static _Bool
handle_focus_in_event (GtkWidget *widget, GdkEventFocus *event, void    * user_data)
{
...
static _Bool
handle_focus_out_event (GtkWidget *widget, GdkEventFocus *event, void    * user_data)
{
...
static void
text_ellipsis_limit_changed_callback (void    *callback_data)
{
	char **pref;
	unsigned int i;
	int one_limit;

	pref = eel_settings_get_strv (nautilus_icon_view_preferences,
...
static void
desktop_text_ellipsis_limit_changed_callback (void    *callback_data)
{
	int pref;

	pref = eel_settings_get_int (nautilus_desktop_preferences, NAUTILUS_PREFERENCES_DESKTOP_TEXT_ELLIPSIS_LIMIT);
...
{
    NautilusIconContainerDetails *details;
    static _Bool    setup_prefs = FALSE;
...
fprintf(stderr,"%s %d\n", __func__, setup_prefs);
    if (!setup_prefs) {
        ...
        g_signal_connect_swapped (nautilus_tooltips_preferences,
                                  "changed::" NAUTILUS_TOOLTIP_PREFERENCES_ICON_VIEW,
                                  G_CALLBACK (tooltip_prefs_changed_callback),
                                  container);
...
static void
call_icon_callback (void    * data, void    *callback_data)
{
	NautilusIcon *icon;
...
static int
selection_changed_at_idle_callback (void    * data)
{
	NautilusIconContainer *container;
...
nautilus_icon_container_get_icon_images (NautilusIconContainer *container,
					 ...
					 int                    size,
					 char                 **embedded_text,
					 _Bool                  for_drag_accept,
					 _Bool                  need_large_embeddded_text,
					 _Bool                 *embedded_text_needs_loading,
                                     ...
                                     NautilusIcon          *icon)
{
	NautilusIconContainerDetails *details;
	unsigned int icon_size;
	unsigned int min_image_size, max_image_size;
	NautilusIconInfo *icon_info;
	GdkPoint *attach_points;
	int n_attach_points;
	_Bool    has_embedded_text_rect;
	GdkPixbuf *pixbuf;
	char *editable_text, *additional_text;
	char *embedded_text;
	GdkRectangle embedded_text_rect;
	_Bool    large_embedded_text;
	_Bool    embedded_text_needs_loading;
	_Bool    has_open_window;
    ...
    _Bool    is_desktop = nautilus_icon_container_get_is_desktop (container);

    _Bool    show_tooltip = (container->details->show_desktop_tooltips && is_desktop) ||
    ...
    if (show_tooltip) {
        ...
        tooltip_text = nautilus_file_construct_tooltip (file, container->details->tooltip_flags);

        nautilus_icon_canvas_item_set_tooltip_text (icon->item, tooltip_text);
        g_free (tooltip_text);
    }
    else {
        nautilus_icon_canvas_item_set_tooltip_text (icon->item, "");
    }
	 ...
	 */
	if (icon == get_icon_being_renamed (container) &&
	    g_strcmp0 (editable_text, nautilus_icon_canvas_item_get_editable_text (icon->item)) != 0)
        {
		end_renaming_mode (container, FALSE);
	}
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
align_icons_callback (void *callback_data)
{
	NautilusIconContainer *container;
						      ...
						      _Bool    select_all)
{
  ...
  details = container->details;
  if (details->renaming) {
    eel_editable_label_select_region (EEL_EDITABLE_LABEL (details->rename_widget),0,-1);
  }
  else {
    ...
    if (icon != NULL) { /* drop out the botton and exit if no icon */
      ...
      if (!icon_is_positioned (icon)) {
        ...
        if (editable_text != NULL) {
          ...
          eel_editable_label_set_font_description (EEL_EDITABLE_LABEL (details->rename_widget), desc);
          ...
          gtk_layout_move (GTK_LAYOUT (container), details->rename_widget, x, y);

          gtk_widget_set_size_request (details->rename_widget, width, -1);
          ...
          if (select_all) {
            ...
            if (NAUTILUS_IS_DIRECTORY(icon->data)) {
              start_offset = 0;
              end_offset   = -1;
            }
            else{
...
static void
end_renaming_mode (NautilusIconContainer *container, _Bool    commit)
{
	NautilusIcon *icon;
	                                           ...
	                                           GList                 *clipboard_icon_data)
{
	GList *l;
	NautilusIcon *icon;
	_Bool    highlighted_for_clipboard;
