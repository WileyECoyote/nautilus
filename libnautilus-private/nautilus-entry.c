...
nautilus_entry_finalize (GObject *object)
{
	...
	EEL_SOURCE_REMOVE_IF_THEN_ZERO (entry->details->select_idle_id);
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
select_all_at_idle (void *callback_data)
{
	NautilusEntry *entry;
