...
struct _NautilusIconInfo
{
	...
	_Bool        sole_owner;
	signed long  last_use_time;
	GdkPixbuf   *pixbuf;

	_Bool        got_embedded_rect;
		      ...
		      _Bool      is_last_ref)
{
	...
	if (is_last_ref) {
		...
		g_object_remove_toggle_ref (object,
                                            (GToggleNotify)pixbuf_toggle_notify,
					    info);
		icon->last_use_time = g_get_monotonic_time ();
		schedule_reap_cache ();
	}
...
static unsigned int reap_cache_timeout = 0;

#define MICROSEC_PER_SEC ((signed long)1000000L)

static unsigned int time_now;

static _Bool reap_old_icon (void *key, void *value, void *user_info)
{
	NautilusIconInfo *icon = value;
	_Bool *reapable_icons_left = user_info;
...
static int reap_cache (void *data)
{
	_Bool reapable_icons_left;
...
static void schedule_reap_cache (void)
{
  if (reap_cache_timeout == 0) {
    reap_cache_timeout = g_timeout_add_seconds_full (0, 5, reap_cache, NULL, NULL);
  }
}
...
static unsigned int loadable_icon_key_hash (LoadableIconKey *key)
{
...
static LoadableIconKey *loadable_icon_key_new (GIcon *icon, int size)
{
	LoadableIconKey *key;
...
static void loadable_icon_key_free (LoadableIconKey *key)
{
	g_object_unref (key->icon);
	g_slice_free (LoadableIconKey, key);
}

static unsigned int themed_icon_key_hash (ThemedIconKey *key)
{
...
static ThemedIconKey *themed_icon_key_new (const char *filename, int size)
{
	ThemedIconKey *key;
...
static void themed_icon_key_free (ThemedIconKey *key)
{
	g_free (key->filename);
	g_slice_free (ThemedIconKey, key);
}

NautilusIconInfo *nautilus_icon_info_lookup (GIcon *icon, int size)
{
...
NautilusIconInfo *
nautilus_icon_info_lookup_from_name (const char *name, int size)
{
  GIcon *icon;
...
NautilusIconInfo *
nautilus_icon_info_lookup_from_path (const char *path, int size)
{
  GFile *icon_file;
  GIcon *icon;
  NautilusIconInfo *info;
...
nautilus_icon_info_get_pixbuf_nodefault (NautilusIconInfo  *icon)
{
  ...
  if (icon->pixbuf == NULL) {
    ...
    pixbuf = g_object_ref (icon->pixbuf);
                                                 ...
                                                 unsigned long      forced_size)
{
  ...
  if (pixbuf) {
    ...
    scaled_pixbuf = pixbuf; /* Set return to what we got so far */

    if (forced_size > 0 && size != forced_size && size) {

      scale = (double)forced_size / size;
...
int
nautilus_get_list_icon_size_for_zoom_level (NautilusZoomLevel zoom_level)
{
    switch (zoom_level) {
    case NAUTILUS_ZOOM_LEVEL_SMALLEST:
        return NAUTILUS_ICON_SIZE_SMALLEST;
...
int nautilus_get_icon_size_for_stock_size (StdIconSize size)
{
  int w, h;
...
int
nautilus_icon_get_emblem_size_for_icon_size (int size)
{
	if (size >= 96)
		return 48;
	if (size >= 64)
