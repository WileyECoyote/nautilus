...
static PangoLayout *get_label_layout                 (PangoLayout                  **layout,
						      ...
						      const char                    *text);
static _Bool hit_test_stretch_handle              (NautilusIconCanvasItem        *item,
						      EelIRect                       canvas_rect,
...
static void
click_policy_changed_callback (NautilusIconCanvasItem *item)
{
    item->click_policy = nautilus_settings_get_click_policy();
...
nautilus_icon_canvas_item_finalize (GObject *object)
{
    ...
    g_signal_handlers_disconnect_by_func (nautilus_preferences,
                                          click_policy_changed_callback, item);

    if (item->details->cursor_window != NULL) {
        gdk_window_set_cursor (item->details->cursor_window, NULL);
        g_object_unref (item->details->cursor_window);
    }

    if (item->details->pixbuf != NULL) {
        g_object_unref (item->details->pixbuf);
    }

    if (item->details->text_util != NULL) {
        g_object_unref (item->details->text_util);
    }

    g_free (item->details->editable_text);
    g_free (item->details->additional_text);
    g_free (item->details->attach_points);

    if (item->details->rendered_pixbuf != NULL) {
        g_object_unref (item->details->rendered_pixbuf);
    }
...
measure_label_text (NautilusIconCanvasItem *item)
{
	NautilusIconCanvasItemDetails *details;
	NautilusIconContainer *container;
	int editable_height, editable_height_for_layout, editable_height_for_entire_text, editable_width, editable_dx;
	int additional_height, additional_width, additional_dx;
	PangoLayout *editable_layout;
	PangoLayout *additional_layout;
	_Bool have_editable, have_additional;
		 ...
		 EelIRect icon_rect)
{
	...
	_Bool have_editable, have_additional;
	_Bool needs_highlight, prelight_label, is_rtl_label_beside;
	EelIRect text_rect;
	int x;
	int max_text_width;
	gdouble frame_w, frame_h, frame_x, frame_y;
	_Bool draw_frame = TRUE;
...
get_knob_pixbuf (void)
{
	...
	knob_pixbuf = gtk_icon_theme_load_icon (gtk_icon_theme_get_default (),
						...
						8, 0, NULL);
	if (!knob_pixbuf) {
		GInputStream *stream = g_resources_open_stream ("/org/nautilus/icons/knob.png", 0, NULL);
		if (stream != NULL) {
...
static void
draw_embedded_text (NautilusIconCanvasItem *item, cairo_t *cr, int x, int y)
{
        PangoLayout *layout;
...
nautilus_icon_canvas_item_event (EelCanvasItem *item, GdkEvent *event)
{
	...
	case GDK_ENTER_NOTIFY:
		...
		if (!icon_item->details->is_prelit) {
			...
			/* show a hand cursor */
			if (icon_item->click_policy == NAUTILUS_CLICK_POLICY_SINGLE) {
...
void
nautilus_icon_canvas_item_set_renaming (NautilusIconCanvasItem *item, _Bool state)
{
	g_return_if_fail (NAUTILUS_IS_ICON_CANVAS_ITEM (item));
...
/* accessible AtkAction interface, is really _Bool */
static int
nautilus_icon_canvas_item_accessible_idle_do_action (gpointer data)
{
	...
	while (!g_queue_is_empty (container->details->a11y_item_action_queue)) {
		...
		case ACTION_MENU:
			...
			break;
		default :
			BUG_IMSG("Unknown action number", action_number);
			break;
		}
	}
	return FALSE;
...
nautilus_icon_canvas_item_accessible_do_action (AtkAction *accessible, int i)
{
	...
	case ACTION_MENU:
		...
		if (container->details->a11y_item_action_idle_handler == 0) {
			container->details->a11y_item_action_idle_handler =
			g_idle_add (nautilus_icon_canvas_item_accessible_idle_do_action, container);
		}
		break;
        default :
...
nautilus_icon_canvas_item_accessible_get_offset_at_point (AtkText	 *text,
                                                          ...
                                                          AtkCoordType coords)
{
        int real_x, real_y, real_width, real_height;
        NautilusIconCanvasItem *item;
        int editable_height;
        int offset = 0;
        int index;
...
void
nautilus_icon_canvas_item_set_tooltip_text            (NautilusIconCanvasItem *item, const char *text)
{
