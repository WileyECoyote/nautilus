...
typedef void (* SelectAllCallback)    (void *target);
...
struct _TargetCallbackData {
	...
	_Bool shares_selection_changes;
...
static void
cut_callback (void *target)
{
  if (target == NULL) {
    fprintf(stderr, "%s target == NULL\n", __func__);
  }
  else {
...
static void
editable_select_all_callback (void *target)
{
  GtkEditable *editable = GTK_EDITABLE (target);
...
static void
text_view_select_all_callback (void *target)
{
  if (GTK_IS_TEXT_VIEW (target)) {
    g_signal_emit_by_name (target, "select-all", TRUE);
  }
  else {
    fprintf(stderr, "%s target not a TEXT VIEW object=<%p>\n", __func__, target);
  }
}

static void
action_cut_callback (GtkAction *action, void *callback_data)
{
   cut_callback (callback_data);
}

static void
action_copy_callback (GtkAction *action, void *callback_data)
{
   copy_callback (callback_data);
}

static void
action_paste_callback (GtkAction *action, void *callback_data)
{
	paste_callback (callback_data);
}

static void
action_select_all_callback (GtkAction *action, void *callback_data)
{
  ...
  if (callback_data != NULL) {
    ...
    if (target_data != NULL) {
      ...
      fprintf(stderr, "%s target_data == NULL\n", __func__);
  }
  }
  else {
    fprintf(stderr, "%s callback_data == NULL\n", __func__);
  }
}
...
static void
set_clipboard_items_are_merged_in (GObject *widget_as_object, _Bool merged_in)
{
...
text_buffer_update_sensitivity (GtkTextBuffer *buffer, TargetCallbackData *target_data)
{
  if (GTK_IS_TEXT_BUFFER (buffer)) {
    ...
    fprintf(stderr, "%s buffer not a TEXT BUFFER object=<%p>\n", __func__, buffer);
  }
}
...
static void
text_view_connect_callbacks (GObject *object, TargetCallbackData *target_data)
{
...
static void
text_view_disconnect_callbacks (GObject *object, TargetCallbackData *target_data)
{
  GtkTextBuffer *buffer;
...
static void
merge_in_clipboard_menu_items (GObject *widget_as_object, TargetCallbackData *target_data)
{
  _Bool add_selection_callback;
...
static void
merge_out_clipboard_menu_items (GObject *widget_as_object, TargetCallbackData *target_data)

{
  _Bool selection_callback_was_added;
...
static _Bool
focus_changed_callback (GtkWidget *widget, GdkEventAny *event, void *callback_data)
{
...
static void
selection_changed_callback (GtkWidget *widget, void *callback_data)
{
  TargetCallbackData *target_data;
  ...
  if (callback_data != NULL) {
    ...
    if (editable != NULL) {
      ...
      fprintf(stderr, "%s editable == NULL\n", __func__);
    }
  }
  else {
    fprintf(stderr, "%s callback_data == NULL\n", __func__);
...
static void
target_destroy_callback (GtkWidget *object, void *callback_data)
{
  if (callback_data == NULL) {
    fprintf(stderr, "%s callback_data == NULL\n", __func__);
  }
...
static void
nautilus_clipboard_real_set_up (void *target,
				GtkUIManager *ui_manager,
				_Bool shares_selection_changes,
				SelectAllCallback select_all_callback,
				ConnectCallbacksFunc connect_callbacks,
				ConnectCallbacksFunc disconnect_callbacks)
{
	...
	target_data =
	initialize_clipboard_component_with_callback_data (target,
                                                           ui_manager,
...
nautilus_clipboard_set_up_editable (GtkEditable *target,
                                   ...
                                   _Bool shares_selection_changes)
{
  if (GTK_IS_EDITABLE (target) &&  GTK_IS_UI_MANAGER (ui_manager)) {
...
static GList *
convert_lines_to_str_list (char **lines, _Bool *cut)
{
	int i;
	GList *result;
                                                     ...
                                                     GdkAtom copied_files_atom)
{
	...
	if (gtk_selection_data_get_data_type (selection_data) != copied_files_atom
		 ...
		 * think it is OK.
		 */
		data = (unsigned char*) gtk_selection_data_get_data (selection_data);
					    ...
					    GdkAtom      copied_files_atom)
{
  ...
  if (data != NULL) {
    ...
    clipboard_item_uris =
    nautilus_clipboard_get_uri_list_from_selection_data (data,
                                                         NULL,
                                                         copied_files_atom);

    for (iter = (GList *) item_uris; iter; iter = iter->next) {
      if (g_list_find_custom ((GList *) item_uris, iter->data,
