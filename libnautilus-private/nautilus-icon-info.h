...
#include <gtk/gtk.h>
*/
/* Nominal icon sizes for each Nautilus zoom level. This scheme assumes
 * icons are designed to fit in a square space, though each image need not
 * be square. Since individual icons can be stretched, icons are not con-
 * strained to this nominal size.
 */
...
unsigned int       nautilus_icon_info_get_type                       (void) G_GNUC_CONST;
...
NautilusIconInfo  *nautilus_icon_info_lookup_from_path               (const char        *path,
                                                                      int                size);
_Bool              nautilus_icon_info_is_fallback                    (NautilusIconInfo  *icon);
...
GdkPixbuf *        nautilus_icon_info_get_pixbuf_at_size             (NautilusIconInfo  *icon,
                                                                      unsigned long      forced_size);
_Bool              nautilus_icon_info_get_embedded_rect              (NautilusIconInfo  *icon,
                                                                      GdkRectangle      *rectangle);
_Bool              nautilus_icon_info_get_attach_points              (NautilusIconInfo  *icon,
                                                                      GdkPoint         **points,
                                                                      int               *n_points);
...
unsigned int       nautilus_get_icon_size_for_zoom_level             (NautilusZoomLevel  zoom_level);

int                nautilus_get_icon_size_for_stock_size             (StdIconSize        size);
int                nautilus_icon_get_emblem_size_for_icon_size       (int                size);

_Bool              nautilus_icon_theme_can_render                    (GThemedIcon *icon);
