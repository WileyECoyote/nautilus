...
static void
directory_ready_callback (NautilusDirectory *directory, GList *files, void *data)
{
    ...
    if (NAUTILUS_IS_DIRECTORY (directory)) {
        ...
        merged_callback = data;

        g_assert (g_list_find (merged_callback->non_ready_directories, directory) != NULL);

        /* Update based on this call. */
        merged_callback->merged_file_list = g_list_concat (merged_callback->merged_file_list,
        ...
        fprintf(stderr, "%s invalid directory pointer=<%p>\n", __func__, directory);
    }
}
...
static _Bool
merged_contains_file (NautilusDirectory *directory, NautilusFile *file)
{
	NautilusMergedDirectory *merged;
	GList *node;
...
static void
monitor_add_directory (void *key,
		       void *value,
		       void *callback_data)
{
	MergedMonitor *monitor;
...
static void
merged_callback_remove_directory_cover (void *key,
					void *value,
					void *callback_data)
{
...
static void
monitor_remove_directory (void *key,
			  void *value,
			  void *callback_data)
{
...
static void
merged_monitor_destroy_cover (void *key,
			      void *value,
			      void *callback_data)
{
...
static void
merged_callback_destroy_cover (void *key,
			       void *value,
			       void *callback_data)
{
...
nautilus_merged_directory_init (NautilusMergedDirectory *merged)
{
    ...
    merged->details->callbacks = g_hash_table_new (merged_callback_hash,
