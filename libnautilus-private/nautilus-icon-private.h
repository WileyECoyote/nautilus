...
struct NautilusIconContainerDetails {
	...
	unsigned int stretch_initial_size;
	...
	_Bool    drag_allow_moves;

	_Bool    icon_selected_on_button_down;
	NautilusIcon *double_click_icon[2]; /* Both clicks in a double click need to be on the same icon */
	unsigned int double_click_button[2];
	...
	unsigned int stretch_idle_id;

	/* Align idle id */
	unsigned int align_idle_id;
	...
	_Bool    has_been_allocated;

	int size_allocation_count;
	unsigned int size_allocation_count_id;
	...
	_Bool    drop_shadows_requested;

	/* a11y items used by canvas items */
	unsigned int a11y_item_action_idle_handler;
	...
	/* interactive search */
	_Bool    imcontext_changed;
	int selected_iter;
	GtkWidget *search_window;
	GtkWidget *search_entry;
	unsigned int search_entry_changed_id;
	unsigned int typeselect_flush_timeout;
};
...
void          nautilus_icon_container_update_icon                 (NautilusIconContainer *container,
								   NautilusIcon          *icon);
_Bool         nautilus_icon_container_has_stored_icon_positions   (NautilusIconContainer *container);
_Bool         nautilus_icon_container_scroll                      (NautilusIconContainer *container,
								   int                    delta_x,
								   int                    delta_y);
