...
static _Bool              real_is_editable                    (NautilusDirectory      *directory);
...
nautilus_directory_finalize (GObject *object)
{
  ...
  if (directory->details->count_in_progress != NULL) {
    BUG_MSG("count_in_progress != NULL");
  }
  if (directory->details->top_left_read_state != NULL) {
    BUG_MSG("top_left_read_state != NULL");
  }
  ...
  EEL_SOURCE_REMOVE_IF_THEN_ZERO (directory->details->call_ready_idle_id);
  ...
  if (directory->details->file_list != NULL) {
    ...
    BUG_MSG("directory->details->file_list != NULL");
  }
...
//  }
  ...
  if (directory->details->directory_load_in_progress != NULL) {
    BUG_MSG("directory_load_in_progress != NULL");
  }
  ...
  if (directory->details->dequeue_pending_idle_id != 0) {
    BUG_MSG("dequeue_pending_idle_id != 0");
  }
...
static void
invalidate_one_count (void *key, void *value, void *nothing)
{
  NautilusDirectory *directory;

  if (NAUTILUS_IS_DIRECTORY (value)) {
    ...
    BUG_MSG("NAUTILUS_IS_DIRECTORY (value) failed!");
  }
}

static void
filtering_changed_callback (void *callback_data)
{
...
static void
collect_all_directories (void *key, void *value, void *callback_data)
{
	NautilusDirectory *directory;
	GList **dirs;
...
emit_change_signals_for_all_files_in_all_directories (void)
{
  ...
  for (iter = dirs; iter != NULL; iter = iter->next) {
    directory = NAUTILUS_DIRECTORY (iter->data);
...
static void
async_state_changed_one (void *key, void *value, void *user_data)
{
  NautilusDirectory *directory;
...
static void
async_data_preference_changed_callback (void *callback_data)
{
 ...
 */
NautilusDirectory *
nautilus_directory_get_internal (GFile *location, _Bool create)
{
	NautilusDirectory *directory;
	...
	directory = g_hash_table_lookup (directories, location);
...
nautilus_directory_is_local (NautilusDirectory *directory)
{
  ...
  in_trash  = nautilus_directory_is_in_trash (directory) ;
  in_recent = nautilus_directory_is_in_recent (directory);
  is_native = g_file_is_native (directory->details->location);
  ...
  return in_trash || in_recent || is_native || has_path;
}
...
nautilus_directory_is_in_trash (NautilusDirectory *directory)
{
  ...
  if (NAUTILUS_IS_DIRECTORY (directory)) {
    ...
    if (directory->details->location != NULL) {
      answer = g_file_has_uri_scheme (directory->details->location, "trash");
    }
  }
  else {
...
nautilus_directory_is_in_recent (NautilusDirectory *directory)
{
  ...
  if (NAUTILUS_IS_DIRECTORY (directory)) {
    ...
    if (directory->details->location != NULL) {
      ...
      answer = g_file_has_uri_scheme (directory->details->location, "recent");
    }
  }
  else {
    BUG_MSG("NAUTILUS_IS_DIRECTORY (directory) failed!");
...
nautilus_directory_are_all_files_seen (NautilusDirectory *directory)
{
    ...
    if (NAUTILUS_IS_DIRECTORY (directory)) {
        ...
        answer = NAUTILUS_DIRECTORY_CLASS (G_OBJECT_GET_CLASS (directory))->are_all_files_seen (directory);
...
add_to_hash_table (NautilusDirectory *directory, NautilusFile *file, GList *node)
{
  ...
  if (node != NULL) {
    if (g_hash_table_lookup (directory->details->file_hash, name) == NULL) {
      g_hash_table_insert (directory->details->file_hash, (char *) name, node);
    }
    else {
      BUG_MSG("file already in table!");
    }
  }
  else {
    BUG_MSG("node can not be NULL!");
  }
}
  ...
  if (NAUTILUS_IS_DIRECTORY (directory)) {
    ...
    if (NAUTILUS_IS_FILE (file)) {
      ...
      if (file->details->name != NULL) {
        ...
        BUG_MSG("file->details->name == NULL!");
      }
    }
  }
}
...
static void
hash_table_list_prepend (GHashTable *table, const void  *key, void *data)
{
	GList *list;

	list = g_hash_table_lookup (table, key);
	list = g_list_prepend (list, data);
	g_hash_table_insert (table, (void*) key, list);
}

static void
call_files_added_free_list (void *key, void *value, void *nothing)
{
    if (NAUTILUS_IS_DIRECTORY (key)) {
        ...
        BUG_MSG("NAUTILUS_IS_DIRECTORY (key) failed!");
    }
}
...
static void
call_files_changed_free_list (void *key, void *value, void *nothing)
{
    if (value != NULL) {
...
static void
call_files_changed_unref_free_list (void *key, void *value, void *nothing)
{
    if (value != NULL) {
...
static void
call_get_file_info_free_list (void *key, void *value, void *nothing)
{
  NautilusDirectory *directory;
  GList *files;
...
static void
invalidate_count_and_unref (void *key, void *value, void *nothing)
{
  if (NAUTILUS_IS_DIRECTORY (key)) {
...
static void
collect_directories_by_container (void *key, void *value, void *callback_data)
{
	NautilusDirectory *directory;
...
nautilus_directory_schedule_position_set (GList *position_setting_list)
{
	...
	for (p = position_setting_list; p != NULL; p = p->next) {
        ...
        nautilus_file_set_metadata (file, NAUTILUS_METADATA_KEY_MONITOR, NULL, str);
...
static _Bool
is_tentative (void *data, void *nothing)
{
	NautilusFile *file;
...
static _Bool got_files_flag;

static void
got_files_callback (NautilusDirectory *directory, GList *files, void *callback_data)
{
  if (NAUTILUS_IS_DIRECTORY (directory)) {

    if (g_list_length (files) > 10) {
      if (callback_data == &data_dummy) {

        got_files_flag = TRUE;
      }
      else {
        BUG_MSG("callback_data != &data_dummy");
      }
    }
    else {
      BUG_IMSG("files list length", g_list_length (files));
    }
  }
  else {
    BUG_MSG("NAUTILUS_IS_DIRECTORY (directory) failed!");
