...
struct _NautilusSelectionCanvasItemDetails {
	...
	unsigned int fade_out_handler_id;
};
...
/* FIXME WEH Generally, defined intergers should be in a common header or a user
 * configurable option */
...
static int
fade_and_request_redraw (void   *user_data)
{
					 ...
					 unsigned int transition_time)
{
	...
	self->priv->fade_out_handler_id =
		g_timeout_add ((unsigned int) (transition_time / FADE_OUT_STEPS),
...
nautilus_selection_canvas_item_dispose (GObject *obj)
{
	...
	EEL_SOURCE_REMOVE_IF_THEN_ZERO (self->priv->fade_out_handler_id);
