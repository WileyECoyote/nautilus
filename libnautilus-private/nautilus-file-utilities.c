...
nautilus_compute_title_for_location (GFile *location)
{
    ...
    if (nautilus_is_home_directory (location)) {
        title = g_strdup (_("Home"));
    }
    else {
        tmp_str = NULL;
        if (location) {
            file = nautilus_file_get (location);
            tmp_str = nautilus_file_get_description (file);
            if (tmp_str == NULL) {
                tmp_str = nautilus_file_get_display_name (file);
            }
            nautilus_file_unref (file);
        }

        if (tmp_str == NULL) {
            tmp_str = g_file_get_basename (location);
        }

        if (nautilus_settings_get_show_full_path())
        {
            file = nautilus_file_get (location);
            char *path = g_filename_from_uri (nautilus_file_get_uri (file), NULL, NULL);
            if (path != NULL) {
                title = g_strdup_printf("%s - %s", tmp_str, path);
            }
            else {
                title = g_strdup(tmp_str);
            }
            nautilus_file_unref (file);
            g_free (path);
            g_free (tmp_str);
...
nautilus_get_user_directory (void)
{
  ...
  /* The Nautilus folder must have permissions DEFAULT_NAUTILUS_DIRECTORY_MODE.
   * If is a file, the program will attempt to delete the file silently. */
  if (g_file_test(user_directory, G_FILE_TEST_IS_DIR) == FALSE ||
    g_access(user_directory, DEFAULT_NAUTILUS_DIRECTORY_MODE) == -1)
  {
    /* Can we obtain a symbolic linkage to location */
    if (g_file_test(user_directory, G_FILE_TEST_IS_SYMLINK) == TRUE)
    {
      /* Try to find out if the link is a folder, and have the
       * appropriate permissions */
      char* link = g_file_read_link(user_directory, NULL);

      if (link)
      {
        /* If the link is not a directory, or if it fails to do chmod,
         * Deletes the link and create the folder */
        if (g_file_test(link, G_FILE_TEST_IS_DIR) != TRUE ||
          chmod(link, DEFAULT_NAUTILUS_DIRECTORY_MODE) != 0)
        {
          /* We can delete the link and create the folder */
    ...
    {
      chmod(user_directory, DEFAULT_NAUTILUS_DIRECTORY_MODE);
    }
    else if (g_file_test(user_directory, G_FILE_TEST_EXISTS) == TRUE)
    {
      ...
      /* If no file exists, the folder is created */
      g_mkdir_with_parents(user_directory, DEFAULT_NAUTILUS_DIRECTORY_MODE);
    }

    /* Missing permissions */
    if (chmod(user_directory, DEFAULT_NAUTILUS_DIRECTORY_MODE) != 0)
    {
      GtkWidget* dialog = gtk_message_dialog_new(
        ...
        GTK_MESSAGE_ERROR,
        GTK_BUTTONS_CLOSE,
        "The path for nautilus settings must read and write permissions: %s",
        user_directory);
...
char *nautilus_get_accel_map_file (void)
{
  ...
  if (override) {
    ...
    return g_build_filename(g_get_home_dir(), ".config", "accels", "nautilus", NULL);
  }
}
  ...
  if (g_file_get_contents (config_file, &data, NULL, NULL))
      ...
      for (i = 0; lines[i] != NULL; i++)
	{
	  ...
	  *value++ = 0;
	  ...
	  g_strchug (g_strchomp (ptr));
	  if (!g_str_has_prefix (ptr, "XDG_"))
	    continue;
	  if (!g_str_has_suffix (ptr, "_DIR"))
	    continue;
	  type_start = ptr + 4;
	  type_end = ptr + strlen (ptr) - 4;
	  ...
	  directory = unescaped = g_malloc (strlen (value) + 1);
	  while (*value && *value != '"')
	    {
	      if ((*value == '\\') && (*(value + 1) != 0))
		value++;
...
static int
emit_user_dirs_changed_idle (void * data)
{
...
{
	EEL_SOURCE_REMOVE_IF_THEN_ZERO (user_dirs_changed_tag);
}
...
{
  if (nautilus_settings_get_desktop_is_home_dir()) {
    return g_strdup (g_get_home_dir());
  }
  else {
...
nautilus_should_use_templates_directory (void)
{
  ...
  anwser = strcmp (dir, g_get_home_dir ()) != 0;
  g_free (dir);
  return anwser;
}
...
/* WEH Static for module hiding on line 641'ish, how nice */
...
static _Bool  desktop_dir_changed_callback_installed = FALSE;
...
static void
desktop_dir_changed_callback (void * callback_data)
{
	desktop_dir_changed ();
}
...
_Bool
nautilus_is_desktop_directory_file (GFile *dir, const char *file)
{
...
char *nautilus_get_pixmap_file (const char *file)
{
  char *pathname;

  pathname = g_build_filename (DATADIR, NAUTILUS_PIXMAP_DIRECTORY, file, NULL);
  ...
  if (g_file_test (pathname, G_FILE_TEST_EXISTS)) {
    ...
    g_free (pathname);
    pathname =  g_build_filename (DATADIR, NAUTILUS_PIXMAP_DIRECTORY, NULL);
    g_debug ("Failed to locate \"%s\" in \"%s\". Check permissions or re-install nautilus", file, pathname);
  }
  g_free (pathname);
  return NULL;
}
 ...
 */
void
nautilus_uninhibit_power_manager (int cookie)
{
	GDBusConnection *connection;
	GVariant *result;
	GError *error = NULL;
...
_Bool
nautilus_dir_has_children_now (GFile *dir, _Bool *has_subdirs)
{
