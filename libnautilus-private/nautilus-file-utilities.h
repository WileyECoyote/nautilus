...
/* Currently these defines are only used in nautilus-file-utilities.c */
...
#define NAUTILUS_PIXMAP_DIRECTORY       "/nautilus/pixmaps"

/* These defines are referenced by other modules */
#define NAUTILUS_SAVED_SEARCH_EXTENSION ".savedSearch"
#define NAUTILUS_SAVED_SEARCH_MIMETYPE  "application/x-gnome-saved-search"

/* Most of these functions return something that needs to be freed with g_free,
 * is not NULL, and is guaranteed to exist (the function returning char*, do not
 * g_free the GFile, this must be unref and let glib free).
 */
...
_Bool    nautilus_is_desktop_directory_file          (GFile *dir,
                                                      const char *filename);
_Bool    nautilus_is_root_directory                  (GFile *dir);
_Bool    nautilus_is_desktop_directory               (GFile *dir);
_Bool    nautilus_is_home_directory                  (GFile *dir);
_Bool    nautilus_is_home_directory_file             (GFile *dir,
                                                      const char *filename);
_Bool    nautilus_is_in_system_dir                   (GFile *location);
char *   nautilus_get_gmc_desktop_directory          (void);

_Bool    nautilus_should_use_templates_directory     (void);
char *   nautilus_get_templates_directory            (void);
char *   nautilus_get_templates_directory_uri        (void);
_Bool    nautilus_create_templates_directory         (void);
...
_Bool    nautilus_is_file_roller_installed                      (void);
...
/* Return an allocated file name that is guranteed to be unique, but tries to
 * make the name readable to users. This isn't race-free, so don't use for
...
GFile *  nautilus_find_file_insensitive                        (GFile *parent, const gchar *name);
...
void     nautilus_get_x_content_types_for_mount_async           (GMount *mount,
                                                                ...
                                                                gpointer user_data);
_Bool    nautilus_dir_has_children_now                          (GFile *dir,
                                                                _Bool *has_subdirs);
