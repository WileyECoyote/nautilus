...
typedef struct {
	...
	_Bool skip_all_error;
	_Bool skip_all_conflict;
	_Bool merge_all;
	_Bool replace_all;
	_Bool delete_all;
} CommonJob;
...
#define IS_IO_ERROR(__err, KIND) ((__err) && ((__err)->domain == G_IO_ERROR && (__err)->code == G_IO_ERROR_ ## KIND))
...
static void mark_desktop_file_trusted (CommonJob *common,
                                       GCancellable *cancellable,
                                       GFile *file,
                                       _Bool interactive);

static void delete_dir (CommonJob *job, GFile *dir,
                        _Bool *skipped_file,
...
static int empty_trash_job (GIOSchedulerJob *io_job, GCancellable *cancellable, void *user_data);
...
static void
generate_initial_job_details (NautilusProgressInfo *info,
                              OpKind                kind,
                              GList                *files,
                              GFile                *destination)
{
    ...
    nautilus_progress_info_take_details (info, s);
}

#define op_job_new(__type, parent_window) ((__type *)(init_common (sizeof(__type), parent_window)))

static gpointer
init_common (size_t job_size, GtkWindow *parent_window)
{
	CommonJob *common;
	GdkScreen *screen;
...
/* Is really _Bool but glib errently defines _Bool as int */
static int
...
should_confirm_trash (void)
{
	...
	prefs = g_settings_new ("apps.nautilus.preferences");
	confirm_trash = eel_settings_get_boolean (prefs, NAUTILUS_PREFERENCES_CONFIRM_TRASH);
			...
			TransferInfo *transfer_info)
{
	...
	if (transfer_info->last_report_time != 0 &&
	    ABS ((signed long)(transfer_info->last_report_time - now)) < 100 * NSEC_PER_MICROSEC) {
		return;
	}
            ...
            _Bool toplevel)
{
  ...
  if (!job_aborted (job) && !local_skipped_file) {
...
/* Is really _Bool but glib errently defines _Bool as type int */
static int
delete_job_done (void *user_data)
{
	DeleteJob *job;
...
/* Is really _Bool but glib errently defines _Bool as type int */
static int
delete_job (GIOSchedulerJob *io_job, GCancellable *cancellable, void *user_data)
{
	...
	_Bool confirmed;
	CommonJob *common;
	_Bool must_confirm_delete_in_trash;
	_Bool must_confirm_delete;
	int files_skipped;
                          ...
                          void                   *done_callback_data)
{
    ...
    g_io_scheduler_push_job (delete_job, job, NULL, 0, NULL);
}
...
static void
unmount_mount_callback (GObject *source_object, GAsyncResult *res, void *user_data)
{
...
static void
empty_trash_for_unmount_done (_Bool success,
                              void *user_data)
{
	UnmountData *data = user_data;
	do_unmount (data);
                                             ...
                                             void                    *callback_data)
{
    ...
    if (check_trash && has_trash_files (mount)) {
        ...
        if (response == GTK_RESPONSE_ACCEPT) {
            ...
            NautilusJobQueue *job_queue = nautilus_job_queue_get ();
            nautilus_job_queue_add_new_job (job_queue, empty_trash_job, job, job->common.cancellable, job->common.progress, FALSE);
            return;
        }
        else if (response == GTK_RESPONSE_CANCEL) {
            if (callback) {
		...
		void     *user_data)
{
	...
	if (!g_volume_mount_finish (G_VOLUME (source_object), res, &error)) {
		if (error->code != G_IO_ERROR_FAILED_HANDLED &&
			...
			g_free (name);
			success = FALSE;
			eel_show_error_dialog (primary, error->message, NULL, NULL);
			g_free (primary);
		}
		g_error_free (error);
	}
...
static void
report_count_progress (CommonJob *job, SourceInfo *source_info)
{
	char *s;
...
static void
count_file (GFileInfo *info, CommonJob *job, SourceInfo *source_info)
{
...
static void
scan_dir (GFile *dir, SourceInfo *source_info, CommonJob *job, GQueue *dirs)
{
    GFileInfo        *info;
    GFile            *subdir;
    GFileEnumerator  *enumerator;
            ...
            if (IS_IO_ERROR (error, CANCELLED)) {
                ...
                if (response == 0 || response == GTK_RESPONSE_DELETE_EVENT) {
                    ...
                    *source_info = saved_info;
                    ...
                    g_warning ("%s Invalid response <%d>", response);
                }
            }
...
static void
scan_file (GFile *file, SourceInfo *source_info, CommonJob *job)
{
    GFileInfo *info;
    GError    *error;
    GQueue    *dirs;
    ...
    while (!done) {
        ...
        if (is_native) {
            filename = eel_get_filename(file);
            if (stat (filename, &stat_buf) < 0) {
                ...
                source_info->num_files += 1;
                source_info->num_bytes += stat_buf.st_size;
            }
            g_free(filename);
            done = TRUE;
        }
...
static void
scan_sources (GList *files, SourceInfo *source_info, CommonJob *job, OpKind kind)
{
    GList *list;
    GFile *file;
    ...
    for (list = files; list != NULL && !job_aborted (job); list = list->next) {
        file = list->data;
        scan_file (file, source_info, job);
    }
...
retry:
  ...
  has_attrib = g_file_info_has_attribute (fsinfo, G_FILE_ATTRIBUTE_FILESYSTEM_FREE);

  if (required_size > 0 && has_attrib) {
		 ...
		 char **dest_fs_type)
{
	...
	char *primary, *secondary, *details;
	int response;
	_Bool handled_invalid_filename;
			   ...
			   GFile *file)
{
	...
	if (!g_file_delete (file, job->cancellable, &error)) {
		...
		if (job->skip_all_error || IS_IO_ERROR (error, CANCELLED)) {
			goto skip2;
		}
		...
		_Bool readonly_source_fs)
{
 ...
 retry:
	...
	/* Needs to recurse */
	else if (error &&  (IS_IO_ERROR (error, WOULD_RECURSE) ||
...
/* Is really _Bool but glib errently defines _Bool as type int */
static int
copy_job (GIOSchedulerJob *io_job, GCancellable *cancellable, void *user_data)
{
    CopyMoveJob *job;
    CommonJob *common;
    SourceInfo source_info;
...
/* Is really _Bool but glib errently defines _Bool as int */
static int
link_job (GIOSchedulerJob *io_job, GCancellable *cancellable, void *user_data)
{
    CopyMoveJob *job;
    CommonJob *common;
    GFile *src;
...
/* Is really _Bool but glib errently defines _Bool as type int */
static int
set_permissions_job_done (void *user_data)
{
				    ...
				    gpointer done_callback_data)
{
	...
	GFile *dest, *src_dir;
	GtkWindow *parent_window;
	_Bool target_is_mapping;
	_Bool have_nonmapping_source;
...
/* Is really _Bool but glib errently defines _Bool as int */
static int
create_job_done (void *user_data)
{
	CreateJob *job;
...
/* Is really _Bool but glib errently defines _Bool as int */
static int
create_job (GIOSchedulerJob *io_job, GCancellable *cancellable, void *user_data)
{
  CreateJob *job;
  CommonJob *common;
  int count;
...
/* Is really _Bool but glib errently defines _Bool as type int */
static int
empty_trash_job_done (void *user_data)
{
	EmptyTrashJob *job;
