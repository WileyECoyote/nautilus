...
nautilus_window_slot_dispose (GObject *object)
{
	...
	if (slot->location) {
		/* TODO? why do we ref here, instead of unrefing?
...
nautilus_window_slot_class_init (NautilusWindowSlotClass *klass)
{
  ...
  signals[ACTIVE] = g_signal_new ("active",
  ...
  signals[INACTIVE] = g_signal_new ("inactive",
                                    G_TYPE_FROM_CLASS (klass),
                                    G_SIGNAL_RUN_LAST,
  ...
  signals[CHANGED_PANE] = g_signal_new ("changed-pane",
                                        G_TYPE_FROM_CLASS (klass),
                                        G_SIGNAL_RUN_LAST,
                                        G_STRUCT_OFFSET (NautilusWindowSlotClass, changed_pane),
                                        NULL, NULL,
                                        g_cclosure_marshal_VOID__VOID,
                                        G_TYPE_NONE, 0);
}
...
nautilus_window_slot_update_title (NautilusWindowSlot *slot)
{
    ...
    if (strlen (slot->title) > 0 && slot->current_location_bookmark != NULL)
    {
        do_sync = TRUE;
    }
			    ...
			    const char *status)
{
	...
	show_statusbar = nautilus_settings_get_start_with_status_bar();
...
static void
set_status_data_free (void *data)
{
...
static int
set_status_timeout_cb (void   *data)
{
	SetStatusData *status_data = data;
...
static void
set_floating_bar_status (NautilusWindowSlot *slot, const char *status)
{
	...
	slot->set_status_timeout_id =
		g_timeout_add_full (G_PRIORITY_DEFAULT,
				    (unsigned int) (double_click_time / 2),
...
nautilus_window_slot_get_current_uri (NautilusWindowSlot *slot)
{
  ...
  if (slot->pending_location != NULL) {
    uri = g_file_get_uri (slot->pending_location);
  }
