...
nautilus_query_editor_dispose (GObject *object)
{
	...
	EEL_SOURCE_REMOVE_IF_THEN_ZERO(editor->details->typing_timeout_id);
...
static int
nautilus_query_editor_draw (GtkWidget *widget, cairo_t *cr)
{
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
typing_timeout_cb (void *user_data)
{
	NautilusQueryEditor *editor;
...
entry_changed_cb (GtkWidget *entry, NautilusQueryEditor *editor)
{
	...
	EEL_SOURCE_REMOVE_IF (editor->details->typing_timeout_id);
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
type_separator_func (GtkTreeModel *model, GtkTreeIter *iter, void *data)
{
	char *text;
	_Bool result;
	...
	result = text != NULL && strcmp (text, "---") == 0;
...
static void
finish_first_line (NautilusQueryEditor *editor, GtkWidget *hbox, _Bool use_go)
{
...
static void
nautilus_query_editor_changed_force (NautilusQueryEditor *editor, _Bool force_reload)
{
	NautilusQuery *query;
...
GtkWidget *
nautilus_query_editor_new (_Bool start_hidden)
{
	GtkWidget *editor;
...
GtkWidget*
nautilus_query_editor_new_with_bar (_Bool start_hidden,
