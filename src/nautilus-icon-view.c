...
struct NautilusIconViewDetails
{
    ...
    unsigned int    icon_merge_id;

    unsigned int    react_to_icon_change_idle_id;

    _Bool           compact;

    _Bool           filter_by_screen;
    int             num_screens;
    ...
    _Bool    supports_auto_layout;
    _Bool    supports_scaling;
    _Bool    supports_keep_aligned;
    _Bool    supports_labels_beside_icons;
};
...
static void                 nautilus_icon_view_set_directory_sort_by        (NautilusIconView     *icon_view,
                                                                             NautilusFile         *file,
                                                                             const char           *sort_by);
static void                 icon_view_reset_to_defaults                     (NautilusView         *view);
...
static void                 nautilus_icon_view_set_directory_tighter_layout (NautilusIconView     *icon_view,
                                                                             NautilusFile         *file,
                                                                             _Bool                 tighter_layout);
static _Bool                nautilus_icon_view_supports_scaling	      (NautilusIconView    *icon_view);
static void                 icon_view_reveal_selection             (NautilusView         *view);
static const SortCriterion *get_sort_criterion_by_sort_type                 (NautilusFileSortType  sort_type);
static void                 set_sort_criterion_by_sort_type                 (NautilusIconView     *icon_view,
                                                                             NautilusFileSortType  sort_type);
static _Bool                set_sort_reversed                               (NautilusIconView     *icon_view,
                                                                             _Bool                 new_value,
                                                                            _Bool                  set_metadata);
...
static NautilusFileSortType get_default_sort_order                          (NautilusFile         *file,
                                                                            _Bool                 *reversed);
static void                 icon_view_clear                                 (NautilusView         *view);
...
nautilus_icon_view_destroy (GtkWidget *object)
{
	...
	icon_view_clear (NAUTILUS_VIEW (object));

        EEL_SOURCE_REMOVE_IF_THEN_ZERO (icon_view->details->react_to_icon_change_idle_id);
...
static void
action_stretch_callback (GtkAction *action, void *callback_data)
{
...
static void
action_unstretch_callback (GtkAction *action, void *callback_data)
{
	g_assert (NAUTILUS_IS_ICON_VIEW (callback_data));
...
static void
action_clean_up_callback (GtkAction *action, void *callback_data)
{
	nautilus_icon_view_clean_up (NAUTILUS_ICON_VIEW (callback_data));
}

static void
set_tighter_layout (NautilusIconView *icon_view, _Bool    new_value)
{
...
static void
action_tighter_layout_callback (GtkAction *action, void *user_data)
{
...
static void
list_covers (NautilusIconData *data, void *callback_data)
{
	GSList **file_list;
...
static void
unref_cover (NautilusIconData *data, void *callback_data)
{
	nautilus_file_unref (NAUTILUS_FILE (data));
}

static void
icon_view_clear (NautilusView *view)
{
	NautilusIconContainer *icon_container;
	GSList *file_list;
...
static void
icon_view_remove_file (NautilusView *view, NautilusFile *file, NautilusDirectory *directory)
{
	NautilusIconView *icon_view;
	...
	if (directory != nautilus_view_get_model (view)) {
		...
		g_warning ("icon_view_remove_file() - directory not icon view model, shouldn't happen.\n"
...
static void
icon_view_add_file (NautilusView *view, NautilusFile *file, NautilusDirectory *directory)
{
	NautilusIconView *icon_view;
	NautilusIconContainer *icon_container;
...
static void
icon_view_file_changed (NautilusView *view, NautilusFile *file, NautilusDirectory *directory)
{
    NautilusIconView *icon_view;
    ...
    if (directory == nautilus_view_get_model (view)) {
        ...
        if (!should_show_file_on_screen (view, file)) {
            icon_view_remove_file (view, file, directory);
        }
        else {
        ...
        BUG_MSG("directory != model object!");
    }
}
...
get_default_sort_order (NautilusFile *file, _Bool *reversed)
{
    ...
    _Bool default_sort_in_reverse_order;

    default_sort_order = nautilus_settings_get_sort_order();

    default_sort_in_reverse_order = nautilus_settings_get_sort_reversed();
                                                ...
                                                NautilusFile *file)
{
    ...
    return nautilus_file_get_boolean_metadata (file,
...
{
    return eel_settings_get_boolean (nautilus_icon_view_preferences,
                        ...
                        NautilusFile *file)
{
    ...
    if (nautilus_settings_get_ignore_view_metadata ()) {
        int t = nautilus_window_get_ignore_meta_tighter_layout (nautilus_view_get_nautilus_window (NAUTILUS_VIEW (icon_view)));
                        ...
                        _Bool    tighter_layout)
{
    ...
    if (nautilus_settings_get_ignore_view_metadata ()) {
        int t = tighter_layout ? TIGHTER_YES : TIGHTER_NO;
        nautilus_window_set_ignore_meta_tighter_layout (nautilus_view_get_nautilus_window (NAUTILUS_VIEW (icon_view)), t);
    } else {
        nautilus_file_set_boolean_metadata (file, NAUTILUS_METADATA_KEY_ICON_VIEW_TIGHTER_LAYOUT,
                                        get_default_directory_tighter_layout (),
                                        tighter_layout);
    }
}
...
nautilus_icon_view_get_default_zoom_level (NautilusIconView *icon_view)
{
    NautilusZoomLevel zoom_level;

    if (icon_view->details->compact) {
       zoom_level = icon_view->details->default_compact_zoom_level;
    }
    else {
       zoom_level = icon_view->details->default_zoom_level;
    }
    return zoom_level;
}
                                   ...
                                   _Bool    always_emit)
{
  NautilusIconContainer *icon_container;
  NautilusIconView      *icon_view;
  NautilusZoomLevel      current_level;
  NautilusZoomLevel      default_level;

  g_return_if_fail (NAUTILUS_IS_ICON_VIEW (view));

  if (new_level > NAUTILUS_ZOOM_LEVEL_LARGEST)
    new_level = NAUTILUS_ZOOM_LEVEL_LARGEST;
  else if (new_level < NAUTILUS_ZOOM_LEVEL_SMALLEST)
    new_level = NAUTILUS_ZOOM_LEVEL_SMALLEST;

  icon_container = get_icon_container (view);

  current_level = nautilus_icon_container_get_zoom_level (icon_container);
  ...
  if ( current_level == new_level) {
    ...
    icon_view = (NautilusIconView*) view;

    if (!nautilus_settings_get_ignore_view_metadata ()) {
      if (view->details->compact) {
        nautilus_file_set_integer_metadata (nautilus_view_get_directory_as_file (NAUTILUS_VIEW (view)),
                                            NAUTILUS_METADATA_KEY_COMPACT_VIEW_ZOOM_LEVEL,
                                            icon_view->details->default_compact_zoom_level,
                                            new_level);
      }
      else {
        nautilus_file_set_integer_metadata (nautilus_view_get_directory_as_file (NAUTILUS_VIEW (view)),
...
/*  This function retrieves, validates, saves and returns the
 *  default zoom level for compact views */
static int
get_preferred_compact_zoom_level (NautilusIconView *icon_view)
{
    NautilusZoomLevel zoom_level;

    zoom_level = nautilus_settings_get_compact_zoom_level();

    zoom_level = CLAMP (zoom_level, NAUTILUS_ZOOM_LEVEL_SMALLEST, NAUTILUS_ZOOM_LEVEL_LARGEST);

    icon_view->details->default_compact_zoom_level = zoom_level;

    return zoom_level;
}

/* This function calls to retrieves the default zoom level for compact
 * views and then calls to set the current level to the retrieved value*/
static void
compact_zoom_level_pref_changed_callback (NautilusIconView *icon_view)
{
    NautilusZoomLevel zoom_level;
fprintf(stderr,"%s begin\n", __func__);
    zoom_level = get_preferred_compact_zoom_level(icon_view);

    nautilus_icon_view_set_zoom_level (icon_view, zoom_level, FALSE);
}

/*  This function retrieves, validates, saves and returns the
 *  default zoom level for icon views */
static int
get_preferred_icon_zoom_level (NautilusIconView *icon_view)
{
    NautilusZoomLevel zoom_level;

    zoom_level = nautilus_settings_get_icon_zoom_level();

    zoom_level = CLAMP (zoom_level, NAUTILUS_ZOOM_LEVEL_SMALLEST, NAUTILUS_ZOOM_LEVEL_LARGEST);

    icon_view->details->default_zoom_level = zoom_level;

    return zoom_level;
}

/* This function calls to retrieves the default zoom level for icon
 * views and then calls to set the current level to the retrieved value*/
static void
icon_zoom_level_pref_changed_callback (NautilusIconView *icon_view)
{
    NautilusZoomLevel zoom_level;
fprintf(stderr,"%s begin\n", __func__);
    zoom_level = get_preferred_icon_zoom_level(icon_view);

    nautilus_icon_view_set_zoom_level (icon_view, zoom_level, FALSE);
}

static NautilusZoomLevel
icon_view_get_zoom_level (NautilusView *view)
{
...
static void
icon_view_bump_zoom_level (NautilusView *view, int zoom_increment)
{
        NautilusZoomLevel new_level;

        //g_return_if_fail (NAUTILUS_IS_ICON_VIEW (view));

        new_level = icon_view_get_zoom_level (view) + zoom_increment;
...
static _Bool
icon_view_can_zoom_in (NautilusView *view)
{
    return icon_view_get_zoom_level (view) < NAUTILUS_ZOOM_LEVEL_LARGEST;
}

static _Bool
icon_view_can_zoom_out (NautilusView *view)
{
    return icon_view_get_zoom_level (view) > NAUTILUS_ZOOM_LEVEL_SMALLEST;
}

static void
icon_view_zoom_to_level (NautilusView *view, NautilusZoomLevel zoom_level)
{
        NautilusIconView *icon_view;
...
static void
icon_view_restore_default_zoom_level (NautilusView *view)
{
    NautilusIconView *icon_view;
    NautilusZoomLevel zoom_level;
    ...
    nautilus_view_zoom_to_level (view, nautilus_icon_view_get_default_zoom_level (icon_view));
}

static NautilusZoomLevel
icon_view_get_default_zoom_level (NautilusView *view)
{
    return nautilus_icon_view_get_default_zoom_level(NAUTILUS_ICON_VIEW (view));
}
...
set_labels_beside_icons (NautilusIconView *icon_view)
{
	...
	if (nautilus_icon_view_supports_labels_beside_icons (icon_view)) {
		labels_beside = nautilus_icon_view_is_compact (icon_view) ||
			eel_settings_get_boolean (nautilus_icon_view_preferences,
...
{
	_Bool    all_columns_same_width;

	if (nautilus_icon_view_is_compact (icon_view)) {
		all_columns_same_width = eel_settings_get_boolean (nautilus_compact_view_preferences,
...
static void
icon_view_begin_loading (NautilusView *view)
{
    NautilusIconView *icon_view;
    GtkWidget        *icon_container;
    NautilusFile     *file;
    ...
    icon_view_restore_default_zoom_level (view);

    level = nautilus_icon_view_get_default_zoom_level (icon_view);
...
static void
icon_view_end_loading (NautilusView *view, _Bool all_files_seen)
{
	NautilusIconView *icon_view;
	GtkWidget *icon_container;
...
static _Bool
icon_view_is_empty (NautilusView *view)
{
	g_assert (NAUTILUS_IS_ICON_VIEW (view));
...
static GList *
icon_view_get_selection (NautilusView *view)
{
	GList *list;
...
static void
count_item (NautilusIconData *icon_data, void *callback_data)
{
	unsigned int *count;
...
static unsigned int
icon_view_get_item_count (NautilusView *view)
{
	unsigned int count;
				 ...
				 NautilusFileSortType  sort_type)
{
	...
	icon_view_reveal_selection (NAUTILUS_VIEW (icon_view));
}


static void
action_reversed_order_callback (GtkAction *action, void *user_data)
{
	NautilusIconView *icon_view;
...
static void
action_keep_aligned_callback (GtkAction *action, void *user_data)
{
	NautilusIconView *icon_view;
	NautilusFile *file;
	_Bool    keep_aligned;
...
static _Bool
icon_view_can_rename_file (NautilusView *view, NautilusFile *file)
{
    return NAUTILUS_VIEW_CLASS(nautilus_icon_view_parent_class)->can_rename_file (view, file);
}

static void
icon_view_start_renaming_file (NautilusView *view,
				  NautilusFile *file,
				  _Bool    select_all)
{
...
static void
icon_view_merge_menus (NautilusView *view)
{
	NautilusIconView *icon_view;
	...
	g_object_unref (action_group); /* owned by ui manager */
        ...
        icon_view->details->icon_merge_id =
                gtk_ui_manager_add_ui_from_resource (ui_manager, "/apps/nautilus/nautilus-icon-view-ui.xml", NULL);
...
static void
icon_view_unmerge_menus (NautilusView *view)
{
	NautilusIconView *icon_view;
	GtkUIManager *ui_manager;
...
static void
icon_view_update_menus (NautilusView *view)
{
	NautilusIconView *icon_view;
        int selection_count;
	GtkAction *action;
...
static void
icon_view_reset_to_defaults (NautilusView *view)
{
    NautilusIconContainer *icon_container;
    NautilusIconView      *icon_view;
    ...
    nautilus_icon_container_set_keep_aligned (icon_container, get_default_directory_keep_aligned ());
    nautilus_icon_container_set_tighter_layout (icon_container, get_default_directory_tighter_layout ());
    ...
    if (nautilus_settings_get_ignore_view_metadata ()) {
        NautilusWindow *window = nautilus_view_get_nautilus_window (view);
        nautilus_window_set_ignore_meta_tighter_layout (window, TIGHTER_NULL);
        nautilus_window_set_ignore_meta_zoom_level (window, NAUTILUS_ZOOM_LEVEL_NULL);
    }
}

static void
icon_view_select_all (NautilusView *view)
{
	NautilusIconContainer *icon_container;
...
static void
icon_view_reveal_selection (NautilusView *view)
{
	GList *selection;
...
static GArray *
icon_view_get_selected_icon_locations (NautilusView *view)
{
	g_return_val_if_fail (NAUTILUS_IS_ICON_VIEW (view), NULL);
...
static void
icon_view_set_selection (NautilusView *view, GList *selection)
{
	g_return_if_fail (NAUTILUS_IS_ICON_VIEW (view));
...
static void
icon_view_invert_selection (NautilusView *view)
{
	g_return_if_fail (NAUTILUS_IS_ICON_VIEW (view));
...
static _Bool
icon_view_using_manual_layout (NautilusView *view)
{
	g_return_val_if_fail (NAUTILUS_IS_ICON_VIEW (view), FALSE);
...
static void
icon_view_widget_to_file_operation_position (NautilusView *view,
						GdkPoint *position)
{
	//g_assert (NAUTILUS_IS_ICON_VIEW (view));
					    ...
					    NautilusIconView *icon_view)
{
	...
	_Bool    open_in_tab, open_in_window, close_behind;
                                  ...
                                  NautilusFile *b)
{
    ...
    return nautilus_file_compare_for_sort (a, b, icon_view->details->sort->sort_type,
                                           nautilus_view_should_sort_directories_first ((NautilusView *)icon_view),
                                           icon_view->details->sort_reversed);
}

static int
icon_view_compare_files (NautilusView *view, NautilusFile *a, NautilusFile *b)
{
    NautilusIconView *icon_view = (NautilusIconView*)view;
    return nautilus_file_compare_for_sort (a, b,
                                           icon_view->details->sort->sort_type,
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
icon_view_react_to_change_callback (void *data)
{
        NautilusIconView *icon_view;
	...
	if (nautilus_view_get_active (NAUTILUS_VIEW (icon_view)) &&
	    icon_view->details->react_to_icon_change_idle_id == 0) {
                icon_view->details->react_to_icon_change_idle_id
                        = g_idle_add (icon_view_react_to_change_callback,
				      icon_view);
	}
...
static void
icon_view_click_policy_changed (NautilusView *view)
{
    nautilus_icon_view_update_click_mode (NAUTILUS_ICON_VIEW (view));
}

static void
image_display_policy_changed_callback (void *callback_data)
{
	NautilusIconView *icon_view;
...
static void
text_attribute_names_changed_callback (void *callback_data)
{
	NautilusIconView *icon_view;
...
static void
default_sort_in_reverse_order_changed_callback (void *callback_data)
{
	NautilusIconView *icon_view;
	NautilusFile *file;
	NautilusIconContainer *icon_container;
...
static void
default_use_tighter_layout_changed_callback (void *callback_data)
{
   NautilusIconView *icon_view;
   NautilusFile *file;
   NautilusIconContainer *icon_container;
...
static void
labels_beside_icons_changed_callback (void *callback_data)
{
	NautilusIconView *icon_view;
...
static void
all_columns_same_width_changed_callback (void *callback_data)
{
	NautilusIconView *icon_view;
...
static void
icon_view_sort_directories_first_changed (NautilusView *directory_view)
{
	NautilusIconView *icon_view;
...
nautilus_icon_view_update_click_mode (NautilusIconView *icon_view)
{
    ...
    if (icon_container) {
        ...
        click_mode = nautilus_settings_get_click_policy();
...
static _Bool
button_press_callback (GtkWidget *widget, GdkEventFocus *event, void *user_data)
{
    NautilusIconView *view = NAUTILUS_ICON_VIEW (user_data);
...
create_icon_container (NautilusIconView *icon_view)
{
	...
	g_object_add_weak_pointer (G_OBJECT (icon_container),
				   (void **) &icon_view->details->icon_container);
...
static const char *
icon_view_get_id (NautilusView *view)
{
...
nautilus_icon_view_finalize (GObject *object)
{
    ...
    g_signal_handlers_disconnect_by_func (nautilus_icon_view_preferences,
                                          icon_zoom_level_pref_changed_callback,
                                          icon_view);
    g_signal_handlers_disconnect_by_func (nautilus_icon_view_preferences,
    ...
    g_signal_handlers_disconnect_by_func (nautilus_compact_view_preferences,
                                          compact_zoom_level_pref_changed_callback,
                                          icon_view);
    g_signal_handlers_disconnect_by_func (nautilus_compact_view_preferences,
...
nautilus_icon_view_class_init (NautilusIconViewClass *klass)
{
	...
	view_class->add_file                           = icon_view_add_file;
	view_class->begin_loading                      = icon_view_begin_loading;
	view_class->can_rename_file                    = icon_view_can_rename_file;
	view_class->clear                              = icon_view_clear;
	view_class->end_loading                        = icon_view_end_loading;
	view_class->file_changed                       = icon_view_file_changed;
	view_class->get_selected_icon_locations        = icon_view_get_selected_icon_locations;
	view_class->get_selection                      = icon_view_get_selection;
	view_class->get_selection_for_transfer         = icon_view_get_selection;
	view_class->get_item_count                     = icon_view_get_item_count;
	view_class->is_empty                           = icon_view_is_empty;
	view_class->remove_file                        = icon_view_remove_file;
	view_class->reset_to_defaults                  = icon_view_reset_to_defaults;
	view_class->reveal_selection                   = icon_view_reveal_selection;
	view_class->select_all                         = icon_view_select_all;
	view_class->set_selection                      = icon_view_set_selection;
        view_class->invert_selection                   = icon_view_invert_selection;
	view_class->compare_files                      = icon_view_compare_files;
        view_class->click_policy_changed               = icon_view_click_policy_changed;
        view_class->merge_menus                        = icon_view_merge_menus;
        view_class->unmerge_menus                      = icon_view_unmerge_menus;
        view_class->sort_directories_first_changed     = icon_view_sort_directories_first_changed;
        view_class->start_renaming_file                = icon_view_start_renaming_file;
        view_class->update_menus                       = icon_view_update_menus;
	view_class->using_manual_layout                = icon_view_using_manual_layout;
	view_class->widget_to_file_operation_position  = icon_view_widget_to_file_operation_position;
	view_class->get_view_id                        = icon_view_get_id;
	view_class->get_first_visible_file             = icon_view_get_first_visible_file;
	view_class->scroll_to_file                     = icon_view_scroll_to_file;

/* Zoom Related functions */
        view_class->get_zoom_level                     = icon_view_get_zoom_level;
        view_class->bump_zoom_level                    = icon_view_bump_zoom_level;
        view_class->can_zoom_in                        = icon_view_can_zoom_in;
        view_class->can_zoom_out                       = icon_view_can_zoom_out;
        view_class->zoom_to_level                      = icon_view_zoom_to_level;
        view_class->restore_default_zoom_level         = icon_view_restore_default_zoom_level;
        view_class->get_default_zoom_level             = icon_view_get_default_zoom_level;
...
nautilus_icon_view_init (NautilusIconView *icon_view)
{
	...
	g_signal_connect_swapped (nautilus_icon_view_preferences,
				  ...
				  G_CALLBACK (icon_zoom_level_pref_changed_callback),
				  icon_view);
	g_signal_connect_swapped (nautilus_icon_view_preferences,
	...
	g_signal_connect_swapped (nautilus_compact_view_preferences,
				  ...
				  G_CALLBACK (compact_zoom_level_pref_changed_callback),
				  icon_view);
	g_signal_connect_swapped (nautilus_compact_view_preferences,
