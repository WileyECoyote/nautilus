...
nautilus_image_properties_page_init (NautilusImagePropertiesPage *page)
{
	...
	page->details->loading_label = append_label (page->details->vbox,_("loading..."));
	gtk_box_pack_start (GTK_BOX (page), page->details->vbox, FALSE, TRUE, 2);
fprintf(stderr, "%s how about loading the image?\n", __func__);
...
{
   unsigned int type = nautilus_image_properties_page_provider_get_type ();
   nautilus_module_add_type (type);
}
