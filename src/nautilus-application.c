...
static _Bool  save_of_accel_map_requested = FALSE;

static void   desktop_changed_callback      (void                      *user_data);
...
check_required_directories (NautilusApplication *application)
{
   ...
   _Bool   ret;
	...
	if (directories != NULL) {
		...
		dialog = eel_show_error_dialog (error_string, detail_string, "Nautilus Error", NULL);
...
static void
menu_provider_items_updated_handler (NautilusMenuProvider *provider, GtkWidget* parent_window, void *data)
{
				    ...
				    GdkScreen           *screen)
{
	...
	maximized = eel_settings_get_boolean
	...
	geometry_string = eel_settings_get_string
...
static void
desktop_changed_callback (void *user_data)
{
	NautilusApplication *application;

	application = NAUTILUS_APPLICATION (user_data);
	if (nautilus_settings_get_show_desktop()) {
...
nautilus_application_finalize (GObject *object)
{
    ...
    /* Terminate the NautilusSettings object */
    nautilus_settings_terminate();
                          ...
                          char **remaining)
{
    ...
    if (perform_self_check && (remaining != NULL || kill_shell)) {
        g_printerr (_("--check cannot be used with other options.\n"));
    }
    else if (kill_shell && remaining != NULL) {
        g_printerr (_("--quit cannot be used with URIs.\n"));
    }
    else if (self->priv->geometry != NULL &&
        remaining != NULL && remaining[0] != NULL && remaining[1] != NULL)
    {
        g_printerr (_("--geometry cannot be used with more than one URI.\n"));
    }
    else {
        retval = TRUE;
    }
...
nautilus_save_settings (NautilusApplication *self)
{
...
nautilus_application_local_command_line (GApplication *application,
                                         ...
                                         int          *exit_status)
{
  _Bool   perform_self_check = FALSE;
  _Bool   version = FALSE;
  _Bool   browser = FALSE;
  _Bool   kill_shell = FALSE;
  _Bool   no_default_window = FALSE;
  char   **remaining = NULL;
  ...
  const GOptionEntry options[] =
  {
    ...
    { "browser", '\0', G_OPTION_FLAG_HIDDEN, G_OPTION_ARG_NONE, &browser, NULL, NULL },
    { "version", '\0', 0, G_OPTION_ARG_NONE, &version,  N_("Show the version of the program."), NULL },
    { "geometry", 'g', 0, G_OPTION_ARG_STRING, &self->priv->geometry, N_("Create the initial window with the given geometry."), N_("GEOMETRY") },
    { "no-default-window", 'n', 0, G_OPTION_ARG_NONE, &no_default_window, N_("Only create windows for explicitly specified URIs."), NULL },
#if HAVE_GNOME_DESKTOP
    { "no-desktop", '\0', 0, G_OPTION_ARG_NONE, &self->priv->no_desktop, N_("Do not manage the desktop (ignore the preference set in the preferences dialog)."), NULL },
#endif
    { "quit", 'q', 0, G_OPTION_ARG_NONE, &kill_shell,  N_("Quit Nautilus."), NULL },
...
init_desktop (NautilusApplication *self)
{
	...
	if (!self->priv->no_desktop && !nautilus_settings_get_show_desktop())
        {
		self->priv->no_desktop = TRUE;
	}
...
/* Is really _Bool but glib errently defines gboolean to be type int */
static int
nautilus_application_save_accel_map (void *data)
{
...
static void
queue_accel_map_save_callback (GtkAccelMap *object, char *accel_path,
...
nautilus_application_startup (GApplication *app)
{
    ...
    /* create DBus manager -why? */
    self->priv->dbus_manager = nautilus_dbus_manager_new (app);
    self->priv->fdb_manager  = nautilus_freedesktop_dbus_new (app);
...
nautilus_application_get_singleton (void)
{
    return g_object_new (NAUTILUS_TYPE_APPLICATION,
                         "application-id", "apps.NautilusApplication",
