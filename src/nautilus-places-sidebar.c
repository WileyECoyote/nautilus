...
#include "libnautilus-private/nautilus-action.h"
...
typedef struct {
	...
	_Bool devices_header_added;
	_Bool bookmarks_header_added;

	/* DnD */
	GList     *drag_list;
    int        drag_data_info;
	_Bool      drag_data_received;
	_Bool      drop_occured;
    _Bool      in_drag;
	...
	unsigned int bookmarks_changed_id;
    _Bool my_computer_expanded;
    _Bool bookmarks_expanded;
    _Bool devices_expanded;
    _Bool network_expanded;

    _Bool updating_sidebar;
     ...
     * of new bookmarks */
    char *top_bookend_uri;
    char *bottom_bookend_uri;

    int bookmark_breakpoint;

    unsigned int expand_timeout_source;

    unsigned int popup_menu_action_index;
...
static _Bool eject_or_unmount_bookmark              (NautilusPlacesSidebar *sidebar,
							GtkTreePath *path);
static _Bool eject_or_unmount_selection             (NautilusPlacesSidebar *sidebar);
		...
		_Bool highlighted)
{
	...
	icon_size = nautilus_get_icon_size_for_stock_size (STD_ICON_SIZE_MENU);
	...
	if (icon_info != NULL) {
		...
		eject = gtk_icon_set_render_icon_pixbuf (icon_set, style, STD_ICON_SIZE_MENU);
	}
...
{
	return nautilus_settings_get_show_desktop() &&
	   ...
	   const char *tooltip)
{
	...
	int icon_size;
	_Bool show_eject, show_unmount;
	_Bool show_eject_button;
		   ...
		   GtkTreePath **path)
{
	...
	*path = NULL;
	...
	if (gtk_tree_view_get_path_at_pos (sidebar->tree_view,
		...
		eject_button_size = nautilus_get_icon_size_for_stock_size (STD_ICON_SIZE_MENU);
...
static void
desktop_setting_changed_callback (void   * user_data)
{
	NautilusPlacesSidebar *sidebar;
...
static void
volume_mounted_cb (GVolume *volume, _Bool success, GObject *user_data)
{
	GMount *mount;
	NautilusPlacesSidebar *sidebar;
	GFile *location;
			      ...
			      void   *      user_data)
{
	...
	if (!g_drive_poll_for_media_finish (G_DRIVE (source_object), res, &error)) {
		if (error->code != G_IO_ERROR_FAILED_HANDLED) {
			...
			primary = g_strdup_printf (_("Unable to start <b>%s</b>"), name);
			g_free (name);
			eel_show_error_dialog (primary, error->message, "Bookmarks", NULL);
			g_free (primary);
		}
		g_clear_error (&error);
	}
...
static void
show_unmount_progress_aborted_cb (GMountOperation *op, void *user_data)
{
    NautilusApplication *app = NAUTILUS_APPLICATION (g_application_get_default ());
...
static void
do_unmount (GMount *mount, NautilusPlacesSidebar *sidebar)
{
    GMountOperation *mount_op;

    if (mount != NULL) {
        mount_op = get_unmount_operation (sidebar);
        nautilus_file_operations_unmount_mount_full (NULL, mount, mount_op,
                                                     FALSE, TRUE, unmount_done,
...
static void
unmount_shortcut_cb (GtkMenuItem *item, NautilusPlacesSidebar *sidebar)
{
	do_unmount_selection (sidebar);
}

static void
drive_eject_cb (GObject *source_object, GAsyncResult *res, void *user_data)
{
	NautilusWindow *window;
	GError *error;
	char *primary;
	...
	if (!g_drive_eject_with_operation_finish (G_DRIVE (source_object), res, &error)) {
		if (error->code != G_IO_ERROR_FAILED_HANDLED) {
			name = g_drive_get_name (G_DRIVE (source_object));
			primary = g_strdup_printf (_("Unable to eject <b>%s</b>"), name);
			g_free (name);
			eel_show_error_dialog (primary, error->message, "Drive Eject", NULL);
			g_free (primary);
		}
		g_clear_error (&error);
	}
...
static void
volume_eject_cb (GObject *source_object, GAsyncResult *res, void *user_data)
{
	NautilusWindow *window;
	GError         *error;
	char           *primary;
	...
	if (!g_volume_eject_with_operation_finish (G_VOLUME (source_object), res, &error)) {
		if (error->code != G_IO_ERROR_FAILED_HANDLED) {
			...
			primary = g_strdup_printf (_("Unable to eject <b>%s</b>"), name);
			g_free (name);
			eel_show_error_dialog (primary, error->message, "Volume Eject", NULL);
			g_free (primary);
		}
		g_clear_error (&error);
	}
		...
		void   * user_data)
{
	...
	if (!g_mount_eject_with_operation_finish (G_MOUNT (source_object), res, &error)) {
		if (error->code != G_IO_ERROR_FAILED_HANDLED) {
			...
			primary = g_strdup_printf (_("Unable to eject <b>%s</b>"), name);
			g_free (name);
			eel_show_error_dialog (primary, error->message, "Mount Eject", NULL);
			g_free (primary);
		}
		g_clear_error (&error);
	}
			 ...
			 void   * user_data)
{
	...
	if (!g_drive_poll_for_media_finish (G_DRIVE (source_object), res, &error)) {
		if (error->code != G_IO_ERROR_FAILED_HANDLED) {
			name = g_drive_get_name (G_DRIVE (source_object));
			primary = g_strdup_printf (_("Unable to poll <b>%s</b> for media changes"), name);
			g_free (name);
			eel_show_error_dialog (primary, error->message, "Nautilus", NULL);
			g_free (primary);
		}
		g_clear_error (&error);
	}
	       ...
	       void   * user_data)
{
	...
	if (!g_drive_poll_for_media_finish (G_DRIVE (source_object), res, &error)) {
		if (error->code != G_IO_ERROR_FAILED_HANDLED) {
			name = g_drive_get_name (G_DRIVE (source_object));
			primary = g_strdup_printf (_("Unable to stop <b>%s</b>"), name);
			g_free (name);
			eel_show_error_dialog (primary, error->message, "Nautilus", NULL);
			g_free (primary);
		}
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
nautilus_places_sidebar_focus (GtkWidget *widget, GtkDirectionType direction)
{
	...
	result = get_selected_iter (sidebar, &iter);

	if (!result) {
		gtk_tree_model_get_iter_first (GTK_TREE_MODEL (sidebar->store), &iter);
		result = find_next_row (sidebar, &iter);
		if (result) {
			path = gtk_tree_model_get_path (GTK_TREE_MODEL (sidebar->store), &iter);
...
bookmarks_build_popup_menu (NautilusPlacesSidebar *sidebar)
{
	...
	use_browser = nautilus_settings_get_always_use_browser();
	...
	gtk_image_menu_item_set_image (GTK_IMAGE_MENU_ITEM (item),
				       gtk_image_new_from_stock (GTK_STOCK_OPEN, STD_ICON_SIZE_MENU));
	...
	gtk_image_menu_item_set_image (GTK_IMAGE_MENU_ITEM (item),
				 gtk_image_new_from_stock (GTK_STOCK_REMOVE, STD_ICON_SIZE_MENU));
	g_signal_connect (item, "activate",
...
tree_selection_func (GtkTreeSelection *selection,
                     GtkTreeModel *model,
                     GtkTreePath *path,
                     int path_currently_selected, /* is _Bool */
                     void   * user_data)
{
    GtkTreeIter iter;
    PlaceType row_type;
