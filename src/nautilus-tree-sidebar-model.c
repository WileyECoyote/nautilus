...
static unsigned int tree_model_signals[LAST_SIGNAL] = { 0 };

typedef _Bool (* FilePredicate) (NautilusFile *);
...
struct TreeNode {
	...
	unsigned int done_loading_id;
	unsigned int files_added_id;
	unsigned int files_changed_id;

	TreeNode *first_child;

	/* misc. flags */
	unsigned int done_loading : 1;
	unsigned int force_has_dummy : 1;
...
struct FMTreeModelDetails {
	...
	unsigned int monitoring_update_idle_id;

	_Bool show_hidden_files;
	_Bool show_only_directories;
...
*/
static void
object_unref_if_not_NULL (void *object)
{
	if (object == NULL) {
		return;
	}
...
static GdkPixbuf *
get_menu_icon_for_file (TreeNode *node, NautilusFile *file, NautilusFileIconFlags flags)
{
	NautilusIconInfo *info;
	GIcon *gicon, *emblem_icon, *emblemed_icon;
	GEmblem *emblem;
	GdkPixbuf *pixbuf, *ret_buf;
	_Bool highlight;
	int size;
	FMTreeModel *model;
	...
	ret_buf = nautilus_icon_info_get_pixbuf_nodefault_at_size (info, size);
	...
	if (highlight) {
		pixbuf = eel_create_spotlight_pixbuf (ret_buf);

		if (pixbuf != NULL) {
			g_object_unref (ret_buf);
			ret_buf = pixbuf;
		}
	}
...
static GdkPixbuf *
tree_node_get_pixbuf (TreeNode *node, NautilusFileIconFlags flags)
{
	if (node->parent == NULL) {
...
report_node_inserted (FMTreeModel *model, TreeNode *node)
{
    ...
    _Bool add_child = FALSE;
    _Bool has_subdirs;

	if (node->directory != NULL) {
        int count;
        if (!model->details->show_only_directories &&
            nautilus_file_get_directory_item_count (node->file, &count, NULL)) {
            add_child = count > 0 || node->parent == NULL;
        } else {
            if (node->parent == NULL) {
                add_child = TRUE;
            } else if (nautilus_dir_has_children_now (nautilus_file_get_location (node->file),
                                                  &has_subdirs)) {
                if (model->details->show_only_directories)
...
{
	_Bool had_dummy_child, has_dummy_child;
	_Bool had_directory, has_directory;
	_Bool changed;
...
static void
set_done_loading (FMTreeModel *model, TreeNode *node, _Bool done_loading)
{
	_Bool had_dummy;
...
start_monitoring_directory (FMTreeModel *model, TreeNode *node)
{
	...
	node->done_loading_id = g_signal_connect (directory, "done_loading",
		 G_CALLBACK (done_loading_callback), node->root);
	node->files_added_id = g_signal_connect (directory, "files_added",
		 G_CALLBACK (files_changed_callback), node->root);
	node->files_changed_id = g_signal_connect (directory, "files_changed",
		 G_CALLBACK (files_changed_callback), node->root);
...
fm_tree_model_get_column_type (GtkTreeModel *model, int index)
{
	...
	default:
	        fprintf(stderr, "%s unknown column:<%d>\n", __func__, index);
	}
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
update_monitoring_idle_callback (void *callback_data)
{
	FMTreeModel *model;
	TreeNode *node;
                                  ...
                                  GtkTreeIter *iter_b)
{
  ...
  if (a != NULL && a->parent == NULL) {
    ...
    if (b != NULL && b->parent == NULL) {
      ...
      BUG_MSG("b != NULL && b->parent == NULL failed");
    }
  }
  else {
    BUG_MSG("a != NULL && a->parent == NULL failed");
  }
...
static void
do_update_node (NautilusFile *file, FMTreeModel *model)
{
...
void
fm_tree_model_set_highlight_for_files (FMTreeModel *model, GList *files)
{
	GList *old_files;
...
fm_tree_model_finalize (GObject *object)
{
	...
	EEL_SOURCE_REMOVE_IF_THEN_ZERO (model->details->monitoring_update_idle_id);
