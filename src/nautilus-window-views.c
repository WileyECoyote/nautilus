 ...
 *
 * WEH Apparently not since the file compiles without an error
 *     if this header is not included:
 */
...
static void load_new_location                         (NautilusWindowSlot         *slot,
						       GFile                      *location,
						       GList                      *selection,
						       _Bool                       tell_current_content_view,
						       _Bool                       tell_new_content_view);
...
static void
handle_go_back (NautilusWindowSlot *slot, GFile *location)
{
	unsigned int i;
	GList *link;
...
static void
handle_go_forward (NautilusWindowSlot *slot, GFile *location)
{
	unsigned int i;
	GList *link;
	NautilusBookmark *bookmark;
...
static void
viewed_file_changed_callback (NautilusFile *file, NautilusWindowSlot *slot)
{
  GFile *new_location;
  _Bool  is_in_trash, was_in_trash;
  _Bool  is_gone;
  ...
  is_gone = nautilus_file_is_gone (file);
  ...
  if ( is_gone || (is_in_trash && !was_in_trash)) {
...
static void
new_window_show_callback (GtkWidget *widget, void  *user_data)
{
...
#endif
        {
		use_same |= nautilus_settings_get_always_use_browser();
	}
		       ...
		       void  *user_data)
{
	...
	nautilus_file_call_when_ready (slot->determine_view_file,
				      NAUTILUS_FILE_ATTRIBUTE_INFO | NAUTILUS_FILE_ATTRIBUTE_MOUNT,
...
#include <errno.h>
static void
got_file_info_for_view_selection_callback (NautilusFile *file, void *userdata)
{
    NautilusWindow      *window;
    NautilusWindowSlot  *slot;
    ...
    if (error &&
        error->domain == G_IO_ERROR && error->code == G_IO_ERROR_NOT_MOUNTED &&
        !slot->tried_mount)
    {
    ...
    if (error == NULL || (error->domain == G_IO_ERROR && error->code == G_IO_ERROR_NOT_SUPPORTED))
    {
        ...
        if (nautilus_settings_get_ignore_view_metadata ()) {
           view_id =  nautilus_window_get_ignore_meta_view_id (window);
        }
        else {
           view_id = nautilus_file_get_metadata (file, NAUTILUS_METADATA_KEY_DEFAULT_VIEW, NULL);
        }

        if (view_id != NULL &&
            !nautilus_view_factory_view_supports_uri (view_id, location,
        ...
        if (view_id == NULL) {
            ...
            if (g_strcmp0 (name, "x-nautilus-search") == 0)
                view_id = NAUTILUS_LIST_VIEW_IID;
            else
                view_id = nautilus_settings_get_default_folder_viewer ();
    ...
    if (view_id != NULL) {
        ...
        if (!report_callback (slot, error)) {
            display_view_selection_failure (window, file, location, error);
        }
...
static int
setup_loading_floating_bar_timeout_cb (void *user_data)
{
...
setup_loading_floating_bar (NautilusWindowSlot *slot)
{
	...
	EEL_SOURCE_REMOVE_IF_THEN_ZERO (slot->set_status_timeout_id);

	EEL_SOURCE_REMOVE_IF_THEN_ZERO (slot->loading_timeout_id);
...
slot_add_extension_extra_widgets (NautilusWindowSlot *slot)
{
	...
	for (iter = providers; iter != NULL; iter = iter->next) {
		...
		provider = NAUTILUS_LOCATION_WIDGET_PROVIDER (iter->data);
...
static void
found_content_type_cb (const char **x_content_types, void *user_data)
{
    NautilusWindowSlot *slot;
				...
				GFile *location, GError *error)
{
  ...
  if (error == NULL) {
    ...
    switch (error->code) {
      ...
      case G_IO_ERROR_NOT_SUPPORTED:
        ...
        error_message = g_strdup_printf (_("<b>Could not display \"%s\".</b>"),
        uri_for_display);
      ...
      case G_IO_ERROR_HOST_NOT_FOUND:
         ...
         */
        error_message = g_strdup_printf (_("<b>Could not display \"%s\", because the host could not be found.</b>"),
        uri_for_display);
  ...
  if (error_message == NULL) {
    error_message = g_strdup_printf (_("<b>Could not display \"%s\".</b>"), uri_for_display);
    detail_message = g_strdup_printf (_("Error: %s\nPlease select another viewer and try again."), error->message);
  }

  eel_show_error_dialog (error_message, detail_message, "Nautilus", NULL);
				       ...
				       const char *id)
{
  ...
  if (slot != NULL) {
    ...
    if (nautilus_settings_get_ignore_view_metadata ()) {
      nautilus_window_set_ignore_meta_view_id (nautilus_window_slot_get_window (slot), id);
    }
    else {
      nautilus_file_set_metadata (file, NAUTILUS_METADATA_KEY_DEFAULT_VIEW, NULL, id);
    }
				 ...
				 _Bool    new_tab)
{
	...
	list = back ? slot->back_list : slot->forward_list;
        ...
        len = (unsigned int) g_list_length (list);
