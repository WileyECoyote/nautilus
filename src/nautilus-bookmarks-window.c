...
static _Bool		     text_changed = FALSE;
static _Bool		     name_text_changed = FALSE;
...
static unsigned int    get_selected_row                            (void);
static _Bool get_selection_exists                        (void);
...
static void	on_row_activated			    (GtkTreeView	  *view,
							     ...
							     gpointer		   user_data);
static _Bool	on_button_pressed                           (GtkTreeView	  *view,
                                                             GdkEventButton       *event,
							     gpointer		   user_data);
static _Bool	on_key_pressed                              (GtkTreeView	  *view,
                                                             GdkEventKey          *event,
							     gpointer		   user_data);
static void     on_selection_changed                        (GtkTreeSelection     *treeselection,
							     gpointer              user_data);

static _Bool on_text_field_focus_out_event               (GtkWidget            *widget,
							     GdkEventFocus        *event,
							     gpointer              user_data);
static void     on_uri_field_changed                        (GtkEditable          *editable,
							     gpointer              user_data);
static _Bool on_window_delete_event                      (GtkWidget            *widget,
							     GdkEvent             *event,
							     gpointer              user_data);
...
static void
bookmarks_set_empty (_Bool empty)
{
	GtkTreeIter iter;
...
create_bookmarks_window (NautilusBookmarkList *list, GObject *undo_manager_source)
{
	...
	if (!gtk_builder_add_from_resource (builder,
					    "/apps/nautilus/nautilus-bookmarks-window-ui.xml",
					    NULL)) {
		return NULL;
	}
...
static void
open_selected_bookmark (void    *user_data, GdkScreen *screen)
{
		...
		void    *user_data)
{
	...
	if (row < (int) nautilus_bookmark_list_length (bookmarks)) {
		      ...
		      void    *user_data)
{
  ...
  EEL_BLOCK_SIGNAL_HANDLER (name_field, name_field_changed_signal_id);
  nautilus_entry_set_text (NAUTILUS_ENTRY (name_field), name ? name : "");
  EEL_UNBLOCK_SIGNAL_HANDLER (name_field, name_field_changed_signal_id);

  EEL_BLOCK_SIGNAL_HANDLER (uri_field, uri_field_changed_signal_id);
  nautilus_entry_set_text (NAUTILUS_ENTRY (uri_field), entry_text ? entry_text : "");
  EEL_UNBLOCK_SIGNAL_HANDLER (uri_field, uri_field_changed_signal_id);
...
update_bookmark_from_text (void)
{
	if (text_changed) {
		...
		gtk_tree_selection_get_selected (bookmark_selection,
						 NULL, &iter);
		EEL_BLOCK_SIGNAL_HANDLER (bookmark_list_store,
					row_changed_signal_id);
		...
		gtk_list_store_set (bookmark_list_store, &iter,
				    ...
				    -1);
		EEL_UNBLOCK_SIGNAL_HANDLER (bookmark_list_store,
					  row_changed_signal_id);
...
static _Bool
on_text_field_focus_out_event (GtkWidget *widget, GdkEventFocus *event, void *user_data)
{
...
static void
on_window_hide_event (GtkWidget *widget, void *user_data)
{
...
static void
on_window_destroy_event (GtkWidget *widget, void *user_data)
{
