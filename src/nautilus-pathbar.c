...
struct _ButtonData
{
	...
	_Bool is_base_dir;
        ...
        GtkWidget *label;
	GtkWidget *alignment;
        /*WEH: Does it make sense do declare unsigned when only using first bit ?*/
...
static _Bool    nautilus_path_bar_slider_button_press      (GtkWidget       *widget,
                                                            GdkEventButton  *event,
                                                            NautilusPathBar *path_bar);
static _Bool    nautilus_path_bar_slider_button_release    (GtkWidget       *widget,
                                                            GdkEventButton  *event,
                                                            NautilusPathBar *path_bar);
...
static void     nautilus_path_bar_update_button_state      (ButtonData      *button_data,
                                                            _Bool            current_dir);
static _Bool    nautilus_path_bar_update_path              (NautilusPathBar *path_bar,
                                                            GFile           *file_path,
                                                            _Bool            emit_signal);
...
static void
desktop_location_changed_callback (void *user_data)
{
	NautilusPathBar *path_bar;
                                     ...
                                     void           *user_data)
{
	...
	EEL_SOURCE_REMOVE_IF_THEN_ZERO (path_bar->drag_slider_timeout);
}
			    	 ...
			    	 GtkAllocation *allocation)
{
        ...
        _Bool need_sliders;
        int up_slider_offset;
        int down_slider_offset;
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
nautilus_path_bar_scroll (GtkWidget *widget, GdkEventScroll *event)
{
	NautilusPathBar *path_bar;
...
static void
nautilus_path_bar_remove_1 (GtkContainer *container, GtkWidget *widget)
{
        _Bool was_visible = gtk_widget_get_visible (widget);
...
static void
nautilus_path_bar_remove (GtkContainer *container, GtkWidget *widget)
{
        NautilusPathBar *path_bar;
        GList           *children;
...
static void
nautilus_path_bar_grab_notify (GtkWidget *widget, int was_grabbed)
{
        if (!was_grabbed) {
                nautilus_path_bar_stop_scrolling (NAUTILUS_PATH_BAR (widget));
	}
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
nautilus_path_bar_scroll_timeout (void *data)
{
    _Bool retval = FALSE;
    NautilusPathBar *path_bar;

    if (NAUTILUS_IS_PATH_BAR(data)) {
        path_bar = data;
        GDK_THREADS_ENTER ();
...
{
  if (path_bar->timer) {
    EEL_SOURCE_REMOVE_THEN_ZERO (path_bar->timer);
    path_bar->need_timer = FALSE;
  }
}
...
set_label_size_request (ButtonData *button_data)
{
        ...
        PangoLayout *layout;
        int width, height, bold_width, bold_height;
        gchar *markup;
			       ...
			       _Bool emit_signal)
{
	NautilusFile *file, *parent_file;
        _Bool first_directory, last_directory;
        _Bool result;
