...
typedef struct {
  ...
  _Bool                      timed_wait_active;
  NautilusFileListHandle    *files_handle;
  _Bool                      tried_mounting;
  char                      *activation_directory;
  _Bool                      user_confirmation;
} ActivateParameters;
...
static void cancel_activate_callback                (void               *callback_data);
...
static void
launch_location_update_from_file (LaunchLocation *location, NautilusFile *file)
{
	nautilus_file_unref (location->file);
	g_free (location->uri);
	location->file = nautilus_file_ref (file);
...
static void
launch_location_update_from_uri (LaunchLocation *location, const char *uri)
{
	nautilus_file_unref (location->file);
	g_free (location->uri);
...
application_launch_parameters_new (GAppInfo *application, GList *uris)
{
...
get_executable_text_file_action (GtkWindow *parent_window, NautilusFile *file)
{
	...
	preferences_value = eel_settings_get_enum	(nautilus_preferences,
	...
	dialog = eel_create_question_dialog (prompt,
					     ...
					     _("Run in _Terminal"), RESPONSE_RUN_IN_TERMINAL,
     					     _("_Display"), RESPONSE_DISPLAY, _("Confirmation"),
					      parent_window);
...
{
#ifdef HAVE_GNOME_DESKTOP
  return (nautilus_file_is_directory (file) || NAUTILUS_IS_DESKTOP_ICON_FILE (file));
#else
...
static void
list_to_parameters_foreach (GAppInfo *application, GList *uris, GList **ret)
{
...
activate_parameters_special_free (ActivateParametersSpecial *parameters_special)
{
    ...
    nautilus_file_unref (parameters_special->file);
    g_free (parameters_special);
}

static void
make_exec_callback (NautilusFile *file,
                       GFile *res_loc,
                      GError *error,
                     gpointer callback_data)
{
    ...
    if (error == NULL) {
        gchar *path = nautilus_file_get_path (params->file);
        nautilus_launch_application_from_command (gtk_widget_get_screen (GTK_WIDGET (params->parent_window)),
                                              path, FALSE, NULL);
        g_free (path);
    }
                              ...
                              gpointer user_data)
{
    ...
    NautilusMimeApplicationChooser *chooser = children->data;

    g_list_free (children);

    info = nautilus_mime_application_chooser_get_info (chooser);
    file = nautilus_file_get_by_uri (nautilus_mime_application_chooser_get_uri (chooser));
...
run_open_with_dialog (ActivateParametersSpecial *params)
{
    ...
    mime_type = nautilus_file_get_mime_type (params->file);
    uri = nautilus_file_get_uri (params->file);
    ...
    GtkWidget *chooser = nautilus_mime_application_chooser_new (uri, uris, mime_type);
...
application_unhandled_uri (ActivateParameters *parameters, char *uri)
{
    ...
    NautilusFile *file = nautilus_file_get_existing_by_uri (uri);
    ...
    parameters_special->file = nautilus_file_ref (file);
...
activate_files (ActivateParameters *parameters)
{
	...
	for (l = parameters->locations; l != NULL; l = l->next) {
		...
		case ACTIVATION_ACTION_ASK :
			BUG_IMSG("Invalid action", action);
			break;
		}
	}
	...
	if (parameters->slot != NULL &&
		...
		if ((flags & NAUTILUS_WINDOW_OPEN_FLAG_NEW_TAB) != 0 &&
		    eel_settings_get_enum (nautilus_preferences, NAUTILUS_PREFERENCES_NEW_TAB_POSITION) ==
	...
	if (open_in_app_uris != NULL &&
		...
		dialog = eel_show_yes_no_dialog (prompt, detail,
						 GTK_STOCK_OK, GTK_STOCK_CANCEL,
						 _("Confirmation"), parameters->parent_window);
		g_free (detail);
				       ...
				       gpointer user_data)
{
  ...
  if (!g_file_mount_enclosing_volume_finish (G_FILE (source_object), res, &error)) {
    if (error->domain != G_IO_ERROR ||
      ...
      eel_show_error_dialog (_("Unable to mount location"), error->message,
                             "Nautilus", parameters->parent_window);
      }
