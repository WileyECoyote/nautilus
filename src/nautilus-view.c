...
#define DirActionGrp view->details->dir_action_group
enum {
    ADD_FILE,
    BEGIN_FILE_CHANGES,
    BEGIN_LOADING,
...
struct NautilusViewDetails
{
    ...
    unsigned int        scripts_merge_id;

    GList              *templates_directory_list;
    GtkActionGroup     *templates_action_group;
    unsigned int        templates_merge_id;

    GtkActionGroup     *extensions_menu_action_group;
    unsigned int        extensions_menu_merge_id;

    unsigned int        display_selection_idle_id;
    unsigned int        update_menus_timeout_id;
    unsigned int        update_status_idle_id;
    unsigned int        reveal_selection_idle_id;

    unsigned int        display_pending_source_id;
    unsigned int        changes_timeout_id;

    unsigned int        update_interval;
    guint64             last_queued;

    unsigned int        files_added_handler_id;
    unsigned int        files_changed_handler_id;
    unsigned int        load_error_handler_id;
    unsigned int        done_loading_handler_id;
    unsigned int        file_changed_handler_id;

    unsigned int        delayed_rename_file_id;
     ...
     */
    _Bool loading;
    _Bool menu_states_untrustworthy;
    _Bool scripts_invalid;
    _Bool templates_invalid;
    _Bool reported_load_error;
     ...
     */
    _Bool         updates_frozen;
    unsigned int  updates_queued;
    _Bool         needs_reload;
    ...
    _Bool show_foreign_files;
    _Bool show_hidden_files;
    _Bool ignore_hidden_file_preferences;

    _Bool batching_selection_level;
    _Bool selection_changed_while_batched;

    _Bool selection_was_removed;

    _Bool metadata_for_directory_as_file_pending;
    _Bool metadata_for_files_in_directory_pending;

    _Bool selection_change_is_due_to_shell;
    _Bool send_selection_change_to_shell;

    GtkActionGroup *open_with_action_group;
    unsigned int    open_with_merge_id;
...
static int      display_selection_info_idle_callback           (void               *data);
...
static void     trash_or_delete_files                          (GtkWindow            *parent_window,
                                                                const GList          *files,
                                                                _Bool              delete_if_all_already_in_trash,
                                                                NautilusView      *view);
...
static _Bool file_list_all_are_folders                      (GList *file_list);
...
static GList *
nautilus_view_get_selection_for_transfer (NautilusView *view)
{
	g_return_val_if_fail (NAUTILUS_IS_VIEW (view), NULL);

	return NAUTILUS_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->get_selection_for_transfer (view);
}
...
nautilus_view_reset_to_defaults (NautilusView *view)
{
    ...
    if (nautilus_settings_get_show_hidden()) {
        nautilus_window_set_hidden_files_mode (view->details->window, NAUTILUS_WINDOW_SHOW_HIDDEN_FILES_ENABLE);
    } else {
        nautilus_window_set_hidden_files_mode (view->details->window, NAUTILUS_WINDOW_SHOW_HIDDEN_FILES_DISABLE);
    }
}
 ...
 **/
void
nautilus_view_bump_zoom_level (NautilusView *view, int zoom_increment)
{
    g_return_if_fail (NAUTILUS_IS_VIEW (view));
 ...
 **/
void
nautilus_view_zoom_to_level (NautilusView *view, NautilusZoomLevel zoom_level)
{
    g_return_if_fail (NAUTILUS_IS_VIEW (view));
...
nautilus_view_get_zoom_level (NautilusView *view)
{
    ...
    if (NAUTILUS_IS_VIEW (view) && nautilus_view_supports_zooming (view)) {
        zoom_level = NAUTILUS_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->get_zoom_level (view);
    }
    else {
...
nautilus_view_can_zoom_in (NautilusView *view)
{
    ...
    if (!nautilus_view_supports_zooming (view)) {
        ...
        answer = NAUTILUS_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->can_zoom_in (view);
    }
    return answer;
}
...
nautilus_view_can_zoom_out (NautilusView *view)
{
    ...
    if (!nautilus_view_supports_zooming (view)) {
        ...
        answer = NAUTILUS_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->can_zoom_out (view);
    }
    return answer;
}
 ...
 *
 * Update the sensitivity and wording of dynamic popup menu items.
 * @view: NautilusView in question.
 */
void
...
static _Bool
nautilus_view_confirm_multiple (GtkWindow *parent_window, int count, _Bool tabs)
{
	GtkDialog *dialog;
	char *prompt;
	char *detail;
	...
	prompt = _("<b>Are you sure you want to open all files?</b>");
	if (tabs) {
...
real_trash (NautilusView *view)
{
	...
	action = gtk_action_group_get_action (DirActionGrp, NAUTILUS_ACTION_TRASH);
...
real_delete (NautilusView *view)
{
	...
	action = gtk_action_group_get_action (DirActionGrp, NAUTILUS_ACTION_DELETE);
	if (gtk_action_get_sensitive (action) &&
	    gtk_action_get_visible (action)) {
...
static void
pattern_select_response_cb (GtkWidget *dialog, int response, void * user_data)
{
	NautilusView *view;
	NautilusDirectory *directory;
	GtkWidget *entry;
	...
	case GTK_RESPONSE_HELP :
		...
		if (error) {
			eel_show_error_dialog (_("<b>There was an error displaying help.</b>"), error->message,
					       NULL, GTK_WINDOW (dialog));
			g_clear_error (&error);
		}
		break;
	default :
		BUG_IMSG("Unknown response", response);
	}
}
...
static void
action_save_search_callback (GtkAction *action, void *callback_data)
{
...
static void
action_save_search_as_callback (GtkAction *action, void *callback_data)
{
	NautilusView	*directory_view;
	NautilusSearchDirectory *search;
...
delayed_rename_file_hack_callback (RenameData *data)
{
  ...
  if (view->details->window != NULL && view->details->active) {
...
rename_file (NautilusView *view, NautilusFile *new_file)
{
	...
	*/
	if (NAUTILUS_IS_LIST_VIEW (view)) {
		...
		EEL_SOURCE_REMOVE_IF (view->details->delayed_rename_file_id);
...
track_newly_added_locations (NautilusView *view, NautilusFile *new_file,
			     NautilusDirectory *directory, void * user_data)
{
	NewFolderData *data;
...
static void
new_folder_done (GFile *new_folder, _Bool success, void *user_data)
{
	NautilusView  *directory_view;
	NautilusFile  *file;
	GdkScreen     *screen;
	...
	if (g_hash_table_lookup_extended (data->added_locations, new_folder, NULL, NULL)) {
		 ...
		 */
		g_signal_connect_data (directory_view, "add_file",
				       G_CALLBACK (reveal_newly_added_folder),
...
nautilus_view_new_folder (NautilusView *directory_view)
{
	...
	nautilus_file_operations_new_folder (GTK_WIDGET (directory_view),
					     pos, parent_uri,
					    (NautilusCreateCallback)new_folder_done, data);
...
static void
click_policy_changed_callback (void *callback_data)
{
	NautilusView *view;
...
static void
sort_directories_first_changed_callback (void *callback_data)
{
	NautilusView *view;
	_Bool preference_value;

	view = NAUTILUS_VIEW (callback_data);

	preference_value = nautilus_settings_get_sort_directories_first();
...
/** \brief Set View Active
 *  \par Function Description
 *   Set the view as active if not already active and
 *   calls external functions to update the menus.
 */
static void
slot_active (NautilusWindowSlot *slot, NautilusView *view)
{
    if (!view->details->active) {
...
/** \brief Set View InActive
 *  \par Function Description
 *   Set the view as inactive only if the view is active and
 *   calls external functions to update the menus.
 */
static void
slot_inactive (NautilusWindowSlot *slot, NautilusView *view)
{
    if (view->details->active) {
...
update_undo_actions (NautilusView *view)
{
	...
	_Bool available, is_undo;
	_Bool undo_active, redo_active;
	char *undo_label, *undo_description, *redo_label, *redo_description;
	...
	/* Update undo entry */
	action = gtk_action_group_get_action (DirActionGrp, "Undo");
	...
	/* Update redo entry */
	action = gtk_action_group_get_action (DirActionGrp, "Redo");
...
nautilus_view_init (NautilusView *view)
{
	...
	view->details->non_ready_files =
		g_hash_table_new_full (file_and_directory_hash,
				      (GEqualFunc) file_and_directory_equal,
	...
	view->details->sort_directories_first = TRUE;
...
nautilus_view_destroy (GtkWidget *object)
{
	...
	EEL_SOURCE_REMOVE_IF_THEN_ZERO (view->details->display_selection_idle_id);

	EEL_SOURCE_REMOVE_IF_THEN_ZERO (view->details->reveal_selection_idle_id);

	EEL_SOURCE_REMOVE_IF_THEN_ZERO (view->details->delayed_rename_file_id);
 ...
 *
 * Display information about the current selection, and notify the view frame
...
nautilus_view_display_selection_info (NautilusView *view)
{
	...
	_Bool non_folder_size_known;
	unsigned int non_folder_count, folder_count, folder_item_count;
	_Bool folder_item_count_known;
	unsigned int file_item_count;
	GList *p;
...
static int
reveal_selection_idle_callback (void *data)
{
	NautilusView *view;
...
static void
done_loading (NautilusView *view, _Bool all_files_seen)
{
  GList *selection;
  ...
  if (view->details->window != NULL) {
    ...
    if (selection != NULL && all_files_seen) {
      ...
      if (NAUTILUS_IS_LIST_VIEW (view)) {
         ...
         */
        EEL_SOURCE_REMOVE_IF (view->details->reveal_selection_idle_id);

        view->details->reveal_selection_idle_id = g_idle_add (reveal_selection_idle_callback, view);
      }
      else {
        nautilus_view_reveal_selection (view);
      }
 ...
 */
static _Bool
copy_move_done_partition_func (void * data, void *callback_data)
{
 	GFile *location;
 	_Bool result;
...
static _Bool
remove_not_really_moved_files (void * key,
			       void * value,
			       void *callback_data)
{
	GList **added_files;
 ...
 */
static void
copy_move_done_callback (GHashTable *debuting_files, _Bool success, void *data)
{
	NautilusView  *directory_view;
	CopyMoveDoneData *copy_move_done_data;
	...
	if (directory_view != NULL) {
		...
		debuting_files_data->added_files = eel_g_list_partition
			(copy_move_done_data->added_files,
			 (EelPredicateFunction)copy_move_done_partition_func,
			 debuting_files,
			 &copy_move_done_data->added_files);
		...
		g_hash_table_foreach_remove (debuting_files,
					    (GHRFunc)remove_not_really_moved_files,
...
static int
compare_files_cover (gconstpointer a, gconstpointer b, void *callback_data)
{
	const FileAndDirectory *fad1, *fad2;
	NautilusView *view;
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
display_selection_info_idle_callback (void *data)
{
	NautilusView *view;
...
{
    EEL_SOURCE_REMOVE_IF_THEN_ZERO (view->details->update_menus_timeout_id);
}
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
update_menus_timeout_callback (void *data)
{
	NautilusView *view;
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
display_pending_callback (void *data)
{
	NautilusView *view;
...
schedule_idle_display_of_pending_files (NautilusView *view)
{
	...
	/* We want higher priority than the idle that handles the relayout
	   ...
	   and other high priority events while we have pending files to show. */
...
static void
schedule_timeout_display_of_pending_files (NautilusView *view, unsigned int interval)
{
...
unschedule_display_of_pending_files (NautilusView *view)
{
   ...
   EEL_SOURCE_REMOVE_IF_THEN_ZERO (view->details->display_pending_source_id);
}
...
reset_update_interval (NautilusView *view)
{
    ...
    EEL_SOURCE_REMOVE_IF_THEN_ZERO (view->details->changes_timeout_id);
...
static _Bool
changes_timeout_callback (void * data)
{
	signed long now;
	signed long time_delta;
	_Bool ret;
	NautilusView *view;
...
schedule_changes (NautilusView *view)
{
	...
	view->details->changes_timeout_id =
		g_timeout_add (UPDATE_INTERVAL_TIMEOUT_INTERVAL, (GSourceFunc)changes_timeout_callback, view);
}
...
static void
files_changed_callback (NautilusDirectory *directory, GList *files, void *callback_data)
{
	NautilusView *view;
	GtkWindow *window;
	char *uri;
 ...
 *
 * Return value: #_Bool inicating whether @view is currently loaded.
 *
 **/
_Bool
...
static void
prepend_uri_one (void * data, void *callback_data)
{
	NautilusFile *file;
	GList **result;
				      ...
				      GArray *relative_item_points)
{
	...
	nautilus_file_operations_copy_move (uris, relative_item_points, dir_uri, GDK_ACTION_LINK,
					   GTK_WIDGET (view),
                                            (NautilusCopyCallback)copy_move_done_callback,
                                            copy_move_done_data);
	g_free (dir_uri);
...
static void
open_one_in_new_window (void * data, void *callback_data)
{
	g_assert (NAUTILUS_IS_FILE (data));
	g_assert (NAUTILUS_IS_VIEW (callback_data));
...
reset_open_with_menu (NautilusView *view, GList *selection)
{
	...
	NautilusFile *file;
	_Bool submenu_visible, filter_default;
	int num_applications;
	int index;
	_Bool other_applications_visible;
	_Bool open_with_chooser_visible;
	GtkUIManager *ui_manager;
	GtkAction *action;
	...
	if (submenu_visible) {
		action = gtk_action_group_get_action (DirActionGrp,
...
get_menu_icon_for_file (NautilusFile *file)
{
	...
	size = nautilus_get_icon_size_for_stock_size (STD_ICON_SIZE_MENU);
...
static char *
get_file_paths_or_uris_as_newline_delimited_string (GList *selection, _Bool get_paths)
{
	char *path;
	char *uri;
	char *result;
...
static void
run_script_callback (GtkAction *action, void *callback_data)
{
...
update_scripts_menu (NautilusView *view)
{
	...
	action = gtk_action_group_get_action (DirActionGrp, NAUTILUS_ACTION_SCRIPTS);
	gtk_action_set_visible (action, any_scripts);
}

static void
create_template_callback (GtkAction *action, void *callback_data)
{
...
update_templates_menu (NautilusView *view)
{
	...
	action = gtk_action_group_get_action (DirActionGrp, NAUTILUS_ACTION_NO_TEMPLATES);
				     ...
				     void *callback_data)
{
	...
	eel_show_info_dialog_with_details
		(_("<b>All executable files in this folder will appear in the</b> "
		   "Scripts menu."),
		 ...
		 "Action Scripts", nautilus_view_get_containing_window (view));
}
...
static void
copy_or_cut_files (NautilusView *view, GList *clipboard_contents, _Bool cut)
{
	int count;
...
static void
action_copy_files_callback (GtkAction *action, void *callback_data)
{
	NautilusView *view;
	GList *selection;
...
static void
move_copy_selection_to_next_pane (NautilusView *view, int copy_action)
{
	NautilusWindowSlot *slot;
	char *dest_location;
...
static void
action_copy_to_next_pane_callback (GtkAction *action, void *callback_data)
{
	NautilusView *view;
...
static void
action_move_to_next_pane_callback (GtkAction *action, void *callback_data)
{
	NautilusWindowSlot *slot;
	char *dest_location;
	NautilusView *view;
...
static void
action_copy_to_home_callback (GtkAction *action, void *callback_data)
{
	NautilusView *view;
	char *dest_location;
...
static void
action_move_to_home_callback (GtkAction *action, void *callback_data)
{
	NautilusView *view;
	char *dest_location;
...
static void
action_copy_to_desktop_callback (GtkAction *action, void *callback_data)
{
	NautilusView *view;
	char *dest_location;
...
static void
action_move_to_desktop_callback (GtkAction *action, void *callback_data)
{
	NautilusView *view;
	char *dest_location;
...
static void
action_cut_files_callback (GtkAction *action, void *callback_data)
{
	NautilusView *view;
	GList *selection;
...
void
nautilus_view_action_cut (GtkAction *action, NautilusView *view)
{
  action_cut_files_callback (action, view);
}
...
void
action_paste_files_callback (GtkAction *action, void *callback_data)
{
	NautilusView *view;
...
void
nautilus_view_action_paste (GtkAction *action, NautilusView *view)
{
  action_paste_files_callback (action, view);
}

static void
paste_into (NautilusView *view, NautilusFile *target)
{
	PasteIntoData *data;
...
static void
action_paste_files_into_callback (GtkAction *action, void *callback_data)
{
	NautilusView *view;
	GList *selection;
...
static void
invoke_external_bulk_rename_utility (NautilusView *view, GList *selection)
{
	GString *cmd;
	char *parameter;
...
static void
real_action_rename (NautilusView *view, _Bool select_all)
{
	NautilusFile *file;
	GList *selection;
...
static void
action_rename_callback (GtkAction *action, void *callback_data)
{
		     ...
		     void        *callback_data)
{
	if (error != NULL &&
		...
		eel_show_error_dialog (_("<b>Unable to mount location!</b>"),
		       ...
		       void          *callback_data)
{
	...
	if (error != NULL &&
	    (error->domain != G_IO_ERROR ||
	     (error->code != G_IO_ERROR_CANCELLED &&
	      error->code != G_IO_ERROR_FAILED_HANDLED))) {
		eel_show_error_dialog (_("<b>Unable to unmount location!</b>"),
				       error->message, NULL, NULL);
	}
}
		     ...
		     void        *callback_data)
{
	...
	if (error != NULL &&
	    (error->domain != G_IO_ERROR ||
	     (error->code != G_IO_ERROR_CANCELLED &&
	      error->code != G_IO_ERROR_FAILED_HANDLED))) {
		eel_show_error_dialog (_("<b>Unable to eject location!</b>"),
				       error->message, NULL, NULL);
	}
}
		    ...
		    void        *callback_data)
{
	if (error != NULL &&
	    (error->domain != G_IO_ERROR ||
	     (error->code != G_IO_ERROR_CANCELLED &&
	      error->code != G_IO_ERROR_FAILED_HANDLED))) {
		eel_show_error_dialog (_("<b>Unable to stop drive!</b>"),
				       error->message, NULL, NULL);
	}
}
...
static void
action_unmount_volume_callback (GtkAction *action, void *data)
{
	NautilusFile *file;
	GList *selection, *l;
	NautilusView *view;
...
static void
action_eject_volume_callback (GtkAction *action, void *data)
{
	NautilusFile *file;
	GList *selection, *l;
	NautilusView *view;
	...
	if (error != NULL &&
	    (error->domain != G_IO_ERROR ||
	     (error->code != G_IO_ERROR_CANCELLED &&
	      error->code != G_IO_ERROR_FAILED_HANDLED &&
	      error->code != G_IO_ERROR_ALREADY_MOUNTED))) {
		eel_show_error_dialog (_("<b>Unable to start location</b>"),
				       error->message, NULL, NULL);
	}
}
...
static void
action_detect_media_callback (GtkAction *action, void *data)
{
	NautilusFile *file;
	GList *selection, *l;
	NautilusView *view;
...
static void
action_self_mount_volume_callback (GtkAction *action, void *data)
{
	NautilusFile *file;
	NautilusView *view;
	GMountOperation *mount_op;
...
static void
action_self_unmount_volume_callback (GtkAction *action, void *data)
{
	NautilusFile *file;
	NautilusView *view;
	GMountOperation *mount_op;
...
static void
action_self_eject_volume_callback (GtkAction *action, void *data)
{
	NautilusFile *file;
	NautilusView *view;
	GMountOperation *mount_op;
...
static void
action_self_start_volume_callback (GtkAction *action, void *data)
{
	NautilusFile *file;
	NautilusView *view;
	GMountOperation *mount_op;
...
static void
action_self_stop_volume_callback (GtkAction *action, void *data)
{
	NautilusFile *file;
	NautilusView *view;
	GMountOperation *mount_op;
...
static void
action_self_detect_media_callback (GtkAction *action, void *data)
{
	NautilusFile *file;
	NautilusView *view;
...
static void
action_location_mount_volume_callback (GtkAction *action, void *data)
{
	NautilusFile *file;
	NautilusView *view;
	GMountOperation *mount_op;
...
static void
action_location_unmount_volume_callback (GtkAction *action, void *data)
{
	NautilusFile *file;
	NautilusView *view;
	GMountOperation *mount_op;
...
static void
action_location_eject_volume_callback (GtkAction *action, void *data)
{
	NautilusFile *file;
	NautilusView *view;
	GMountOperation *mount_op;
...
static void
action_location_start_volume_callback (GtkAction *action, void *data)
{
	NautilusFile *file;
	NautilusView *view;
	GMountOperation *mount_op;
...
static void
action_location_stop_volume_callback (GtkAction *action,  void *data)
{
	NautilusFile *file;
	NautilusView *view;
	GMountOperation *mount_op;
...
static void
action_location_detect_media_callback (GtkAction *action, void *data)
{
	NautilusFile *file;
	NautilusView *view;
...
static void
connect_to_server_response_callback (GtkDialog *dialog, int response_id, void *data)
{
...
static void
action_connect_to_server_link_callback (GtkAction *action, void *data)
{
	NautilusFile *file;
	GList *selection;
	NautilusView *view;
...
static void
action_location_open_alternate_callback (GtkAction *action, void *callback_data)
{
	NautilusView *view;
	NautilusFile *file;
...
static void
action_location_open_in_new_tab_callback (GtkAction *action, void *callback_data)
{
	NautilusView *view;
	NautilusFile *file;
...
static void
action_location_cut_callback (GtkAction *action, void *callback_data)
{
	NautilusView *view;
	NautilusFile *file;
	GList *files;
...
static void
action_location_copy_callback (GtkAction *action, void *callback_data)
{
	NautilusView *view;
	NautilusFile *file;
	GList *files;
...
static void
action_location_trash_callback (GtkAction *action, void *callback_data)
{
	NautilusView *view;
	NautilusFile *file;
	GList *files;
...
static void
action_location_delete_callback (GtkAction *action, void *callback_data)
{
	NautilusView *view;
	NautilusFile *file;
	GFile *location;
...
real_merge_menus (NautilusView *view)
{
  ...
  DirActionGrp = gtk_action_group_new ("DirViewActions");

  gtk_action_group_set_translation_domain (DirActionGrp, GETTEXT_PACKAGE);

  gtk_action_group_add_actions (DirActionGrp,
  ...
  gtk_action_group_add_action (DirActionGrp, action);
  g_object_unref (action);
  g_free (tooltip);
  ...
  g_signal_connect_object (DirActionGrp, "connect-proxy",
                           G_CALLBACK (connect_proxy), G_OBJECT (view),
                           G_CONNECT_SWAPPED);
  g_signal_connect_object (DirActionGrp, "pre-activate",
                           G_CALLBACK (pre_activate), G_OBJECT (view),
                           G_CONNECT_SWAPPED);

  /* Insert action group at end so clipboard action group ends up before it */
  gtk_ui_manager_insert_action_group (ui_manager, DirActionGrp, -1);
  g_object_unref (DirActionGrp); /* owned by ui manager */

  view->details->dir_merge_id = gtk_ui_manager_add_ui_from_resource (ui_manager, "/apps/nautilus/nautilus-directory-view-ui.xml", NULL);
...
void
nautilus_view_merge_toolbar (NautilusView *view,  GtkActionGroup *action_group)
{
  GtkAction *action;

  action = gtk_action_group_get_action (action_group, NAUTILUS_ACTION_PASTE);
  gtk_action_group_add_action (DirActionGrp, action);
                            ...
                            void             *user_data)
{
	...
	action = gtk_action_group_get_action (DirActionGrp, NAUTILUS_ACTION_PASTE);

	gtk_action_set_sensitive (action, can_paste && !nautilus_view_is_read_only (view));

	action = gtk_action_group_get_action (DirActionGrp, NAUTILUS_ACTION_PASTE_FILES_INTO);
	...
	g_object_set_data (G_OBJECT (action), "can-paste-according-to-clipboard",
...
file_list_all_are_folders (GList *file_list)
{
  ...
  for (l = file_list; l != NULL; l = l->next) {
    ...
    if (nautilus_file_is_nautilus_link (file)) {
       ...
       */
      is_dir = (linked_file != NULL && nautilus_file_is_directory (linked_file)) ||
      (activation_uri != NULL &&  activation_uri[strlen (activation_uri) - 1] == '/');
...
#if HAVE_GNOME_DESKTOP
    } else if (!(nautilus_file_is_directory (file) || NAUTILUS_IS_DESKTOP_ICON_FILE (file))) {
#else
    } else if (!nautilus_file_is_directory (file)) {
#endif
      return FALSE;
			   ...
			   int selection_count)
{
	...
	_Bool show_poll;
	GDriveStartStopType start_stop_type;
	_Bool show_self_mount;
	_Bool show_self_unmount;
	_Bool show_self_eject;
	_Bool show_self_start;
	_Bool show_self_stop;
	_Bool show_self_poll;
	...
	for (l = selection; l != NULL && (show_mount || show_unmount
					  ...
					  || show_poll);
	     l = l->next) {
		_Bool show_mount_one;
		_Bool show_unmount_one;
		_Bool show_eject_one;
		_Bool show_connect_one;
		_Bool show_start_one;
		_Bool show_stop_one;
		_Bool show_poll_one;
 ...
 */
static void
real_update_paste_menu (NautilusView *view, GList *selection, int selection_count)
{
    _Bool can_paste_files_into;
    _Bool selection_is_read_only;
    _Bool is_read_only;
    GtkAction *action;
    ...
    pane = nautilus_window_get_active_pane (view->details->window);
    action_group = nautilus_window_pane_get_toolbar_action_group(pane);
...
real_update_location_menu (NautilusView *view)
{
	GtkAction *action;
	NautilusFile *file;
	_Bool is_special_link;
	_Bool is_desktop_or_home_dir;
	_Bool can_delete_file, show_delete;
	_Bool show_separate_delete_command;
	_Bool show_open_in_new_tab;
	_Bool show_open_alternate;
	GList l;
	char *label;
	char *tip;

	show_open_in_new_tab = nautilus_settings_get_always_use_browser();
	show_open_alternate = show_open_in_new_tab;
...
#endif
	can_delete_file =
		nautilus_file_can_delete (file) && !is_special_link && !is_desktop_or_home_dir;
	...
	if (file != NULL &&
		...
		show_separate_delete_command = nautilus_settings_get_enable_delete();
	}
...
static void
update_toolbar_cut_copy_actions(NautilusWindow *window, int can_delete, int can_copy)
{
    NautilusWindowPane *pane;
    GtkAction          *action;
    GtkActionGroup     *action_group;
    ...
    action = gtk_action_group_get_action (action_group, NAUTILUS_ACTION_CUT);
    gtk_action_set_sensitive (action, can_delete);

    action = gtk_action_group_get_action (action_group, NAUTILUS_ACTION_COPY);
...
real_update_menus (NautilusView *view)
{
	...
	_Bool selection_contains_special_link;
	_Bool selection_contains_desktop_or_home_dir;
	_Bool can_create_files;
	_Bool can_delete_files;
	_Bool can_copy_files;
	_Bool can_link_files;
	_Bool can_duplicate_files;
	_Bool show_separate_delete_command;
	_Bool show_open_alternate;
	_Bool show_open_in_new_tab;
	_Bool can_open;
	_Bool show_app;
	_Bool show_save_search;
	_Bool save_search_sensitive;
	_Bool show_save_search_as;
	_Bool show_desktop_target;
	_Bool next_pane_is_writable;
	_Bool show_properties;
	...
	show_open_alternate = file_list_all_are_folders (selection) &&
		selection_count > 0 &&
		nautilus_settings_get_always_use_browser() &&
...
#else
        show_properties = (selection_count > 0);
#endif
	action = gtk_action_group_get_action (DirActionGrp,
	...
	action = gtk_action_group_get_action (DirActionGrp, NAUTILUS_ACTION_CUT);
	gtk_action_set_sensitive (action, can_delete_files);

	action = gtk_action_group_get_action (DirActionGrp, NAUTILUS_ACTION_COPY);
	gtk_action_set_sensitive (action, can_copy_files);

	real_update_paste_menu (view, selection, selection_count);

        update_toolbar_cut_copy_actions(view->details->window, can_delete_files, can_copy_files);
	...
	show_desktop_target =
		nautilus_settings_get_show_desktop() &&
		!nautilus_settings_get_desktop_is_home_dir();
					    ...
					    const char      *location)
{
    ...
    if (NAUTILUS_IS_VIEW (view)) {
        ...
        fprintf(stderr, "%s, Bad view object=<%p>\n", __func__, view);
    }
}
...
schedule_update_menus (NautilusView *view)
{
    if (NAUTILUS_IS_VIEW (view)) {
        ...
        if (view->details->active && view->details->window) {
...
{
  EEL_SOURCE_REMOVE_IF_THEN_ZERO (view->details->update_status_idle_id);
}

/* Is really _Bool but glib errently defines gboolean as int */
static int
update_status_idle_callback (void *data)
{
	NautilusView *view;
...
schedule_update_status (NautilusView *view)
{
    if (NAUTILUS_IS_VIEW (view)) {
        ...
        fprintf(stderr, "%s, Bad view object=<%p>", __func__, view);
    }
}
...
static void
file_changed_callback (NautilusFile *file, void *callback_data)
{
		...
		NautilusDirectory *directory)
{
    ...
    if (!NAUTILUS_IS_VIEW (view)) {
        ...
        view->details->file_changed_handler_id =
        g_signal_connect(view->details->directory_as_file, "changed",
...
finish_loading (NautilusView *view)
{
    ...
    view->details->done_loading_handler_id =
        g_signal_connect (view->details->model, "done_loading",
                          G_CALLBACK (done_loading_callback), view);

    view->details->load_error_handler_id =
        g_signal_connect (view->details->model, "load_error",
    ...
    view->details->files_added_handler_id =
        g_signal_connect (view->details->model, "files_added",
                          G_CALLBACK (files_added_callback), view);

    view->details->files_changed_handler_id =
        g_signal_connect (view->details->model, "files_changed",
                          G_CALLBACK (files_changed_callback), view);
}
			       ...
			       int x, int y)
{
	...
	nautilus_file_operations_copy_move
		(item_uris, relative_item_points,
		 ...
		 (NautilusCopyCallback)copy_move_done_callback, pre_copy_move (view));
}

static void
nautilus_view_trash_state_changed_callback (NautilusTrashMonitor *trash_monitor,
                                            _Bool state, void *callback_data)
{
...
static int
nautilus_view_scroll_event (GtkWidget *widget, GdkEventScroll *event)
{
	NautilusView *directory_view;
...
static void
nautilus_view_parent_set (GtkWidget *widget, GtkWidget *old_parent)
{
	NautilusView       *view;
	GtkWidget          *parent;
...
nautilus_view_class_init (NautilusViewClass *klass)
{
	...
	GtkScrolledWindowClass *scrolled_class;
	GtkBindingSet          *binding_set;

	widget_class   = GTK_WIDGET_CLASS (klass);
	scrolled_class = GTK_SCROLLED_WINDOW_CLASS (klass);
	 ...
	 */
	scrolled_class->scrollbar_spacing = 0;
	...
	g_object_class_install_properties (oclass, NUM_PROPERTIES, properties);
        ...
        signals[ADD_FILE]            = g_signal_new ("add_file",
        ...
        signals[BEGIN_FILE_CHANGES]  = g_signal_new ("begin_file_changes",
                                                     G_TYPE_FROM_CLASS (klass),
                                                     G_SIGNAL_RUN_LAST,
                                                     ...
                                                     G_TYPE_NONE, 0);
        signals[BEGIN_LOADING]       = g_signal_new ("begin_loading",
                                                     G_TYPE_FROM_CLASS (klass),
                                                     G_SIGNAL_RUN_LAST,
        ...
        signals[CLEAR]               = g_signal_new ("clear",
                                                     ...
                                                     g_cclosure_marshal_VOID__VOID,
                                                     G_TYPE_NONE, 0);
        signals[END_FILE_CHANGES]    = g_signal_new ("end_file_changes",
                                                     G_TYPE_FROM_CLASS (klass),
                                                     G_SIGNAL_RUN_LAST,
                                                     ...
                                                     g_cclosure_marshal_VOID__VOID,
                                                     G_TYPE_NONE, 0);
        signals[END_LOADING]         = g_signal_new ("end_loading",
                                                     G_TYPE_FROM_CLASS (klass),
                                                     G_SIGNAL_RUN_LAST,
        ...
        signals[FILE_CHANGED]        = g_signal_new ("file_changed",
                                                     G_TYPE_FROM_CLASS (klass),
                                                     G_SIGNAL_RUN_LAST,
                                                     ...
                                                     g_cclosure_marshal_generic,
                                                     G_TYPE_NONE, 2, NAUTILUS_TYPE_FILE, NAUTILUS_TYPE_DIRECTORY);
        signals[LOAD_ERROR]          = g_signal_new ("load_error",
                                                     G_TYPE_FROM_CLASS (klass),
                                                     G_SIGNAL_RUN_LAST,
        ...
        signals[REMOVE_FILE]         = g_signal_new ("remove_file",
                                                     ...
                                                     g_cclosure_marshal_generic,
                                                     G_TYPE_NONE, 2, NAUTILUS_TYPE_FILE, NAUTILUS_TYPE_DIRECTORY);
        signals[ZOOM_LEVEL_CHANGED]   = g_signal_new ("zoom-level-changed",
                                                    G_TYPE_FROM_CLASS (klass),
                                                    G_SIGNAL_RUN_LAST,
                                                    0, NULL, NULL,
                                                    g_cclosure_marshal_VOID__VOID,
        ...
        signals[SELECTION_CHANGED]   = g_signal_new ("selection-changed",
                                                     ...
                                                     g_cclosure_marshal_VOID__VOID,
                                                     G_TYPE_NONE, 0);
        signals[TRASH]               = g_signal_new ("trash",
                                                     G_TYPE_FROM_CLASS (klass),
