...
connect_dialog_set_connecting (NautilusConnectServerDialog *dialog)
{
	...
	gtk_icon_size_lookup (STD_ICON_SIZE_SMALL_TOOLBAR, &width, &height);
...
connect_dialog_gvfs_error (NautilusConnectServerDialog *dialog)
{
	...
	image = gtk_image_new_from_stock (GTK_STOCK_DIALOG_ERROR, STD_ICON_SIZE_SMALL_TOOLBAR);
...
static void
iconized_entry_restore (void *data,
			void *user_data)
{
	GtkEntry *entry;
	NautilusConnectServerDialog *dialog;
			    ...
			    void *data)
{
	...
	case GTK_RESPONSE_HELP :
		...
		break;
	default :
		BUG_IMSG("Unknown response", response_id);
	}
}
