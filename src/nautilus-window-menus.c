...
static void
action_close_window_slot_callback (GtkAction *action, void *user_data)
{
...
static void
action_connect_to_server_callback (GtkAction *action, void *user_data)
{
...
static void
action_stop_callback (GtkAction *action, void *user_data)
{
	NautilusWindow *window;
	NautilusWindowSlot *slot;
...
static void
action_undo_callback (GtkAction *action, void *user_data)
{
...
static void
action_home_callback (GtkAction *action, void *user_data)
{
	NautilusWindow *window;
	NautilusWindowSlot *slot;
...
static void
action_go_to_computer_callback (GtkAction *action, void *user_data)
{
	NautilusWindow *window;
	NautilusWindowSlot *slot;
	GFile *computer;
...
static void
action_go_to_network_callback (GtkAction *action, void *user_data)
{
	NautilusWindow *window;
	NautilusWindowSlot *slot;
	GFile *network;
...
static void
action_go_to_templates_callback (GtkAction *action, void *user_data)
{
	NautilusWindow *window;
	NautilusWindowSlot *slot;
	char *path;
...
static void
action_go_to_trash_callback (GtkAction *action, void *user_data)
{
	NautilusWindow *window;
	NautilusWindowSlot *slot;
	GFile *trash;
...
static void
action_reload_callback (GtkAction *action, void *user_data)
{
	NautilusWindowSlot *slot;
...
static void
action_zoom_in_callback (GtkAction *action, void *user_data)
{
	nautilus_view_bump_zoom_level (get_current_view (user_data), 1);
}

static void
action_zoom_out_callback (GtkAction *action, void *user_data)
{
	nautilus_view_bump_zoom_level (get_current_view (user_data), -1);
}

static void
action_zoom_normal_callback (GtkAction *action, void *user_data)
{
	nautilus_view_restore_default_zoom_level (get_current_view (user_data));
}

static void
action_show_hidden_files_callback (GtkAction *action, void *user_data)
{
	NautilusWindow *window;
...
static void
action_preferences_callback (GtkAction *action, void *user_data)
{
	GtkWindow *window;

	window = GTK_WINDOW (user_data);

	nautilus_preferences_dialog_show (window);
}

static void
action_about_nautilus_callback (GtkAction *action, void *user_data)
{
	const char *license[] = {
...
static void
action_up_callback (GtkAction *action, void *user_data)
{
...
static void
action_nautilus_manual_callback (GtkAction *action, void *user_data)
{
	NautilusWindow *window;
	GError *error;
	GtkWidget *dialog;
...
static void
menu_item_select_cb (GtkMenuItem *proxy, NautilusWindow *window)
{
	GtkAction *action;
	char *message;
...
static void
menu_item_deselect_cb (GtkMenuItem *proxy, NautilusWindow *window)
{
...
static void
action_back_callback (GtkAction *action, void *user_data)
{
...
static void
action_forward_callback (GtkAction *action, void *user_data)
{
	nautilus_window_back_or_forward (NAUTILUS_WINDOW (user_data),
...
static void
action_split_view_same_location_callback (GtkAction *action, void *user_data)
{
	NautilusWindow *window;
...
static void
action_show_hide_sidebar_callback (GtkAction *action, void *user_data)
{
	NautilusWindow *window;
...
static void
action_split_view_callback (GtkAction *action, void *user_data)
{
	NautilusWindow *window;
	_Bool is_active;
...
nautilus_window_update_split_view_actions_sensitivity (NautilusWindow *window)
{
	GtkActionGroup *action_group;
	GtkAction *action;
	_Bool have_multiple_panes;
	_Bool next_pane_is_in_same_location;
...
static void
sidebar_radio_entry_changed_cb (GtkAction *action, GtkRadioAction *current, void *user_data)
{
    int current_value;
    NautilusWindow *window = NAUTILUS_WINDOW (user_data);
    ...
    if (current_value == SIDEBAR_PLACES) {
        nautilus_window_set_sidebar_id (window, NAUTILUS_WINDOW_SIDEBAR_PLACES);
    } else if (current_value == SIDEBAR_TREE) {
        nautilus_window_set_sidebar_id (window, NAUTILUS_WINDOW_SIDEBAR_TREE);
    }
}

/* TODO: bind all of this with g_settings_bind and GBinding */
static unsigned int
sidebar_id_to_value (const char *sidebar_id)
{
	unsigned int retval = SIDEBAR_PLACES;
...
static void
update_side_bar_radio_buttons (NautilusWindow *window)
{
    GtkActionGroup *action_group;
    GtkAction      *action;
    ...
    action = gtk_action_group_get_action (action_group,
                          "Sidebar Places");
    current_value = sidebar_id_to_value (nautilus_window_get_sidebar_id (window));
...
static void
action_add_bookmark_callback (GtkAction *action, void *user_data)
{
        nautilus_window_add_bookmark_for_current_location (NAUTILUS_WINDOW (user_data));
}

static void
action_edit_bookmarks_callback (GtkAction *action, void *user_data)
{
...
static void
action_new_window_callback (GtkAction *action, void *user_data)
{
    ...
    uri = nautilus_window_slot_get_current_uri (nautilus_window_get_active_slot (current_window));
    ...
    nautilus_window_slot_open_location (nautilus_window_get_active_slot (new_window), loc, 0, NULL);
    g_object_unref (loc);
    g_free (uri);
}

static void
action_new_tab_callback (GtkAction *action, void *user_data)
{
	NautilusWindow *window;
...
void action_toggle_location_entry_callback (GtkToggleAction *action, void *user_data);

static void
toggle_location_entry_setting (NautilusWindow     *window,
                               NautilusWindowPane *pane,
                               _Bool               from_accel_or_menu)
{
    _Bool current_view, temp_toolbar_visible, default_toolbar_visible, grab_focus_only, already_has_focus;

    GtkToggleAction *button_action;
    GtkActionGroup  *action_group;


    current_view = nautilus_toolbar_get_show_location_entry (pane->tool_bar);

    temp_toolbar_visible = pane->temporary_navigation_bar;
    default_toolbar_visible = eel_settings_get_boolean (nautilus_window_state,
                                                      NAUTILUS_WINDOW_STATE_START_WITH_TOOLBAR);
    already_has_focus = nautilus_location_bar_has_focus (NAUTILUS_LOCATION_BAR (pane->location_bar));
    ...
    if ((temp_toolbar_visible || default_toolbar_visible) && !grab_focus_only) {
        nautilus_toolbar_set_show_location_entry (pane->tool_bar, !current_view);
        eel_settings_set_boolean (nautilus_preferences, NAUTILUS_PREFERENCES_SHOW_LOCATION_ENTRY, !current_view);

        action_group = pane->toolbar_action_group;
        button_action = GTK_TOGGLE_ACTION (gtk_action_group_get_action (action_group, NAUTILUS_ACTION_TOGGLE_LOCATION));
...
void
action_toggle_location_entry_callback (GtkToggleAction *action, void *user_data)
{
    NautilusWindow *window = user_data;
    NautilusWindowPane *pane;
...
static void
action_menu_edit_location_callback (GtkAction *action, void *user_data)
{
	NautilusWindow *window = user_data;
	NautilusWindowPane *pane;
...
static void
action_icon_view_callback (GtkAction *action, void *user_data)
{
    NautilusWindow *window;
    NautilusWindowSlot *slot;
    window = NAUTILUS_WINDOW (user_data);
    ...
    toolbar_set_view_button (ICON_VIEW, nautilus_window_get_active_pane(window));
}


static void
action_list_view_callback (GtkAction *action, void *user_data)
{
    NautilusWindow *window;
    NautilusWindowSlot *slot;
    window = NAUTILUS_WINDOW (user_data);
    ...
    toolbar_set_view_button (LIST_VIEW, nautilus_window_get_active_pane(window));
}


static void
action_compact_view_callback (GtkAction *action, void *user_data)
{
    NautilusWindow *window;
    NautilusWindowSlot *slot;
    window = NAUTILUS_WINDOW (user_data);
    ...
    toolbar_set_view_button (COMPACT_VIEW, nautilus_window_get_active_pane(window));
}

unsigned int
toolbar_action_for_view_id (const char *view_id)
{
    if (g_strcmp0(view_id, NAUTILUS_ICON_VIEW_ID) == 0) {
        return ICON_VIEW;
    } else if (g_strcmp0(view_id, NAUTILUS_LIST_VIEW_ID) == 0) {
        return LIST_VIEW;
...
void
toolbar_set_view_button (unsigned int action_id, NautilusWindowPane *pane)
{
...
static void
action_tabs_previous_callback (GtkAction *action, void *user_data)
{
	NautilusWindowPane *pane;
	NautilusWindow *window = user_data;
...
static void
action_tabs_next_callback (GtkAction *action, void *user_data)
{
	NautilusWindowPane *pane;
	NautilusWindow *window = user_data;
...
static void
action_tabs_move_left_callback (GtkAction *action, void *user_data)
{
	NautilusWindowPane *pane;
	NautilusWindow *window = user_data;
...
static void
action_tabs_move_right_callback (GtkAction *action, void *user_data)
{
	NautilusWindowPane *pane;
	NautilusWindow *window = user_data;
...
static void
action_tab_change_action_activate_callback (GtkAction *action, void *user_data)
{
	NautilusWindowPane *pane;
	NautilusWindow *window = user_data;
...
static void
action_new_folder_callback (GtkAction *action, void *user_data)
{
    NautilusWindow *window = user_data;
    NautilusView *view = get_current_view (window);

    nautilus_view_new_folder (view);
}

static void
action_toolbar_cut_callback (GtkAction *action, void *user_data)
{
    NautilusWindow *window = user_data;
    NautilusView   *view = get_current_view (window);

    nautilus_view_action_cut (action, view);
}
static void
action_toolbar_copy_callback (GtkAction *action, void *user_data)
{
    NautilusWindow *window = user_data;
    NautilusView   *view = get_current_view (window);

    nautilus_view_action_copy (action, view);

}
static void
action_toolbar_paste_callback (GtkAction *action, void *user_data)
{
    NautilusWindow *window = user_data;
    NautilusView   *view = get_current_view (window);

    nautilus_view_action_paste (action, view);
}
static const GtkActionEntry main_entries[] = {
  ...
  /* name, stock id */         { NAUTILUS_ACTION_COMPUTER, NAUTILUS_ICON_COMPUTER,
  ...
  /* name, stock id, label */  { NAUTILUS_ACTION_EDIT_LOCATION, NULL, N_("Toggle _Location Entry"),
                                 "<control>L", N_("Switch between location entry and Path buttons"),
...
static const GtkToggleActionEntry main_toggle_entries[] = {
  ...
  /* tooltip */              N_("Change the visibility of this window's statusbar"),
                             NULL,
  /* is_active */            TRUE },
  /* name, stock id */     { NAUTILUS_ACTION_SHOW_HIDE_MENUBAR, NULL,
...
nautilus_window_create_toolbar_action_group (NautilusWindow *window)
{
	_Bool show_label_search_icon_toolbar;
        _Bool show_location_entry_initially;
	...
	action = g_object_new (NAUTILUS_TYPE_NAVIGATION_ACTION,
			       ...
			       "label", _("_Back"),
			       "icon_name", "nautilus-go-previous-symbolic",
	...
	action = g_object_new (NAUTILUS_TYPE_NAVIGATION_ACTION,
			       ...
			       "label", _("_Forward"),
			       "icon_name", "nautilus-go-next-symbolic",
	...
	/** Nautilus 2.30/2.32 type actions */
   	action = g_object_new (NAUTILUS_TYPE_NAVIGATION_ACTION,
   			       ...
   			       "label", _("_Up"),
   			       "icon_name", "nautilus-go-up-symbolic",
   	...
   	action = g_object_new (NAUTILUS_TYPE_NAVIGATION_ACTION,
   			       ...
   			       "label", _("_Reload"),
   			       "icon_name", "nautilus-view-refresh-symbolic",
   	...
   	action = g_object_new (NAUTILUS_TYPE_NAVIGATION_ACTION,
   			       ...
   			       "label", _("_Home"),
   			       "icon_name", "nautilus-go-home-symbolic",
    ...
    action = GTK_ACTION (gtk_action_new (NAUTILUS_ACTION_CUT,
                                                _("Cut Selection"),
                                                _("Cut Selection to the system clipboard"),
                                                NULL));

    gtk_action_group_add_action (action_group, GTK_ACTION (action));
    g_signal_connect (action, "activate", G_CALLBACK (action_toolbar_cut_callback), window);
    gtk_action_set_icon_name (GTK_ACTION (action), "gtk-cut");
    g_object_unref (action);

    action = GTK_ACTION (gtk_action_new (NAUTILUS_ACTION_COPY,
                                                _("Copy Selection"),
                                                _("Copy Selection to the system clipboard"),
                                                NULL));

    gtk_action_group_add_action (action_group, GTK_ACTION (action));
    g_signal_connect (action, "activate", G_CALLBACK (action_toolbar_copy_callback), window);
    gtk_action_set_icon_name (GTK_ACTION (action), "gtk-copy");
    g_object_unref (action);

    action = GTK_ACTION (gtk_action_new (NAUTILUS_ACTION_PASTE,
                                         _("Paste from clipboard"),
                                         _("Paste contents of the system clipboard"),
                                         NULL));

    gtk_action_group_add_action (action_group, GTK_ACTION (action));
    g_signal_connect (action, "activate", G_CALLBACK (action_toolbar_paste_callback), window);
    gtk_action_set_icon_name (GTK_ACTION (action), "gtk-paste");
    g_object_unref (action);


    action = GTK_ACTION (gtk_toggle_action_new (NAUTILUS_ACTION_TOGGLE_LOCATION,
                                                _("Location"),
                                                _("Toggle Location Entry"),
                                                NULL /* stock-id */));

    gtk_action_group_add_action (action_group, GTK_ACTION (action));
    show_location_entry_initially = eel_settings_get_boolean (nautilus_preferences, NAUTILUS_PREFERENCES_SHOW_LOCATION_ENTRY);
    gtk_toggle_action_set_active (GTK_TOGGLE_ACTION (action), show_location_entry_initially);
    g_signal_connect (action, "activate", G_CALLBACK (action_toggle_location_entry_callback), window);
    gtk_action_set_icon_name (GTK_ACTION (action), "location-symbolic");
    g_object_unref (action);

    action = GTK_ACTION (gtk_action_new (NAUTILUS_ACTION_NEW_FOLDER,
                                         _("New folder"),
                                         _("Create a new folder"),
                                         NULL));

    gtk_action_group_add_action (action_group, GTK_ACTION (action));
    g_signal_connect (action, "activate", G_CALLBACK (action_new_folder_callback), window);
    gtk_action_set_icon_name (GTK_ACTION (action), "nautilus-folder-new-symbolic");
    g_object_unref (action);

    action = GTK_ACTION (gtk_toggle_action_new (NAUTILUS_ACTION_ICON_VIEW,
                                                _("Icons"),
                                                _("Icon View"),
                                                NULL));

    g_signal_connect (action, "activate", G_CALLBACK (action_icon_view_callback), window);
    gtk_action_group_add_action (action_group, action);
    gtk_action_set_icon_name (GTK_ACTION (action), "view-grid-symbolic");
    g_object_unref (action);

    action = GTK_ACTION (gtk_toggle_action_new (NAUTILUS_ACTION_LIST_VIEW,
                                                _("List"),
                                                _("List View"),
                                                NULL));

    g_signal_connect (action, "activate", G_CALLBACK (action_list_view_callback), window);
    gtk_action_group_add_action (action_group, action);
    gtk_action_set_icon_name (GTK_ACTION (action), "view-list-symbolic");
    g_object_unref (action);

    action = GTK_ACTION (gtk_toggle_action_new (NAUTILUS_ACTION_COMPACT_VIEW,
                                                _("Compact"),
                                                _("Compact View"), NULL));

    g_signal_connect (action, "activate", G_CALLBACK (action_compact_view_callback), window);
    gtk_action_group_add_action (action_group, action);
    ...
    gtk_action_group_add_action (action_group, action);
    gtk_action_set_icon_name (GTK_ACTION (action), "nautilus-edit-find-symbolic");
...
window_menus_set_bindings (NautilusWindow *window)
{
    ...
    action = gtk_action_group_get_action (action_group,
                          NAUTILUS_ACTION_SHOW_HIDE_MENUBAR);
    ...
    g_object_bind_property (window, "show-sidebar", action, "active",
...
{
	GtkActionGroup *action_group;
	const char *nav_state_actions[] = {
		NAUTILUS_ACTION_BACK, NAUTILUS_ACTION_FORWARD, NAUTILUS_ACTION_UP,
                 NAUTILUS_ACTION_RELOAD, NAUTILUS_ACTION_COMPUTER, NAUTILUS_ACTION_HOME,
                 /*NAUTILUS_ACTION_EDIT_LOCATION, NAUTILUS_ACTION_TOGGLE_LOCATION,*/
...
nautilus_window_initialize_menus (NautilusWindow *window)
{
	...
	g_object_set (action, "short_label", _("_Home"), NULL);
  	...
  	action = gtk_action_group_get_action (action_group, NAUTILUS_ACTION_EDIT_LOCATION);
	...
	gtk_toggle_action_set_active (GTK_TOGGLE_ACTION (action),nautilus_settings_get_show_hidden());
	...
	/* add the UI */
        gtk_ui_manager_add_ui_from_resource (ui_manager, "/apps/nautilus/nautilus-shell-ui.xml", NULL);
...
get_extension_menus (NautilusWindow *window)
{
  ...
  for (iter = providers; iter != NULL; iter = iter->next) {
    ...
    NautilusMenuProvider *provider = NAUTILUS_MENU_PROVIDER (iter->data);

    GList *file_items = NULL;
                          ...
                          const char     *subdirectory)
{
  ...
  for (list = menu_items; list; list = list->next) {
    NautilusMenuItem *item;
    NautilusMenu *menu;
    GtkAction *action;
    char *path;

    item = NAUTILUS_MENU_ITEM (list->data);
