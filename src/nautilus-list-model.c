...
static unsigned int list_model_signals[LAST_SIGNAL] = { 0 };
...
static void
nautilus_list_model_ptr_to_iter (NautilusListModel *model, GSequenceIter *seq_ptr, GtkTreeIter *iter)
{
	g_assert (!g_sequence_iter_is_end (seq_ptr));
	if (iter != NULL) {
		iter->stamp = model->details->stamp;
		iter->user_data = seq_ptr;
	}
}
...
nautilus_list_model_get_iter (GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreePath *path)
{
	...
	for (d = 0; d < gtk_tree_path_get_depth (path); d++) {
		...
		seq_ptr = g_sequence_get_iter_at_pos (files, i);
		file_entry = g_sequence_get (seq_ptr);
		files = file_entry->files;
	}

	nautilus_list_model_ptr_to_iter (model, seq_ptr, iter);
...
nautilus_list_model_get_path (GtkTreeModel *tree_model, GtkTreeIter *iter)
{
    ...
    seq_ptr = iter->user_data;
    while (seq_ptr != NULL) {
        gtk_tree_path_prepend_index (path, g_sequence_iter_get_position (seq_ptr));
        file_entry = g_sequence_get (seq_ptr);
        if (file_entry->parent != NULL) {
            seq_ptr = file_entry->parent->seq_ptr;
        } else {
            seq_ptr = NULL;
        }
    }
  ...
  switch (column) {
    ...
    case NAUTILUS_LIST_MODEL_LARGEST_ICON_COLUMN:
      ...
      if (file != NULL) {
        ...
        flags = NAUTILUS_FILE_ICON_FLAGS_USE_THUMBNAILS | NAUTILUS_FILE_ICON_FLAGS_FORCE_THUMBNAIL_SIZE | NAUTILUS_FILE_ICON_FLAGS_USE_MOUNT_ICON_AS_EMBLEM;

        if (model->details->drag_view != NULL) {
          GtkTreePath *path_a, *path_b;

          gtk_tree_view_get_drag_dest_row (model->details->drag_view, &path_a, NULL);
        ...
        emblem_icons = nautilus_file_get_emblem_icons (file, emblems_to_ignore);
        ...
        if (model->details->highlight_files != NULL &&
          g_list_find_custom (model->details->highlight_files, file, (GCompareFunc) nautilus_file_compare_location))
        {
      ...
      default:
        if (column >= NAUTILUS_LIST_MODEL_NUM_COLUMNS ||
            column < NAUTILUS_LIST_MODEL_NUM_COLUMNS + model->details->columns->len)
        {
          ...
          g_object_get (nautilus_column, "attribute_q", &attribute, NULL);

          if (file != NULL) {
            str = nautilus_file_get_string_attribute_with_default_q (file, attribute);
...
nautilus_list_model_iter_parent (GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreeIter *child)
{
	...
	iter->stamp = model->details->stamp;
	iter->user_data = file_entry->parent->seq_ptr;
...
{
	FileEntry *file_entry;
	GSequenceIter *seq_ptr, *parent_ptr;
	...
	if (parent_ptr) {
		...
		seq_ptr = g_hash_table_lookup (file_entry->reverse_map, file);
	} else {
		seq_ptr = g_hash_table_lookup (model->details->top_reverse_map, file);
	}

	if (seq_ptr) {
		g_assert (((FileEntry *)g_sequence_get (seq_ptr))->file == file);
	}
...
nautilus_list_model_sort_file_entries (NautilusListModel *model, GSequence *files, GtkTreePath *path)
{
    ...
    if (gtk_tree_path_get_depth (path) != 0) {
        ...
        if (gtk_tree_model_get_iter (GTK_TREE_MODEL (model), &iter, path)) {
            ...
            fprintf(stderr, "%s invalid tree iter\n", __func__);
        }
    }
...
static void
nautilus_list_model_set_sort_column_id (GtkTreeSortable *sortable, int sort_column_id, GtkSortType order)
{
	NautilusListModel *model;
...
add_dummy_row (NautilusListModel *model, FileEntry *parent_entry)
{
	...
	dummy_file_entry->seq_ptr = g_sequence_insert_sorted (parent_entry->files, dummy_file_entry,
							  nautilus_list_model_file_entry_compare_func, model);
	iter.stamp = model->details->stamp;
	iter.user_data = dummy_file_entry->seq_ptr;
			      ...
			      NautilusDirectory *directory)
{
	...
	file_entry->seq_ptr = g_sequence_insert_sorted (files, file_entry,
					    nautilus_list_model_file_entry_compare_func, model);

	g_hash_table_insert (parent_hash, file, file_entry->seq_ptr);

	iter.stamp = model->details->stamp;
	iter.user_data = file_entry->seq_ptr;
                                  ...
                                  NautilusDirectory *directory)
{
	...
	seq_ptr = lookup_file (model, file, directory);
	if (!seq_ptr) {
		return;
	}

	pos_before = g_sequence_iter_get_position (seq_ptr);

	g_sequence_sort_changed (seq_ptr, nautilus_list_model_file_entry_compare_func, model);

	pos_after = g_sequence_iter_get_position (seq_ptr);

	if (pos_before != pos_after) {
		/* The file moved, we need to send rows_reordered */

		parent_file_entry = ((FileEntry *)g_sequence_get (seq_ptr))->parent;
		...
		if (parent_file_entry == NULL) {
			...
			has_iter = TRUE;
			nautilus_list_model_ptr_to_iter (model, parent_file_entry->seq_ptr, &iter);
...
{
	GSequenceIter *seq_ptr, *child_ptr;
	...
	if (parent_file_entry && g_sequence_get_length (parent_file_entry->files) == 0) {
		...
		parent_iter.user_data = parent_file_entry->seq_ptr;
...
nautilus_list_model_load_subdirectory (NautilusListModel *model, GtkTreePath *path, NautilusDirectory **directory)
{
	...
	g_hash_table_insert (model->details->directory_reverse_map,
			     subdirectory, file_entry->seq_ptr);
...
void
nautilus_list_model_set_should_sort_directories_first (NautilusListModel *model, _Bool sort_directories_first)
{
