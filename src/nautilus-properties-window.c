...
struct NautilusPropertiesWindowDetails {
	...
	unsigned int update_directory_contents_timeout_id;
	unsigned int update_files_timeout_id;
	...
	_Bool has_recursive_apply;

	GList *value_fields;

	GList *mime_list;

	_Bool deep_count_finished;

        unsigned int total_count;
        goffset total_size;
        unsigned int hidden_count;

	unsigned int long_operation_underway;
...
static void remove_pending                        (StartupData        *data,
						   _Bool            cancel_call_when_ready,
						   _Bool            cancel_timed_wait,
						   _Bool            cancel_destroy_handler);
static void append_extension_pages                (NautilusPropertiesWindow *window);

static _Bool name_field_focus_out              (NautilusEntry *name_field,
						   GdkEventFocus *event,
						   void   * callback_data);
...
static void
add_prompt (GtkWidget *vbox, const char *prompt_text, _Bool pack_at_start)
{
	GtkWidget *prompt;
					       ...
					       unsigned int info, unsigned int time)
{
	...
	if (!exactly_one) {
		eel_show_error_dialog
			(_("<b>You cannot assign more than one custom icon at a time!</b>"),
		...
		if (uri_is_local_image (uris[0])) {
			...
			if (!g_file_is_native (f)) {
				eel_show_error_dialog
					(_("<b>The file that you dropped is not local.</b>"),
					 _("You can only use local images as custom icons."),
					 "Nautilus", window);

			} else {
				eel_show_error_dialog
					(_("<b>The file that you dropped is not an image.</b>"),
					 _("You can only use local images as custom icons."),
					 "Nautilus", window);
			}
			g_object_unref (f);
		...
		const char *name)
{
	...
	/* Only replace text if the file's name has changed. */
		...
		if (use_label) {
			 ...
			 * selection change).
			 */
			char *displayed_name = gtk_editable_get_chars (GTK_EDITABLE (window->details->name_field), 0, -1);
...
static void
rename_callback (NautilusFile *file, GFile *res_loc, GError *error, void   * callback_data)
{
	NautilusPropertiesWindow *window;
...
static void
name_field_activate (NautilusEntry *name_field, void   * callback_data)
{
			  ...
			  GList *files)
{
	...
	_Bool dirty_original = FALSE;
	_Bool dirty_target = FALSE;
...
static int
update_files_callback (void *data)
{
 	NautilusPropertiesWindow *window;
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
schedule_group_change_timeout (void *data)
{
	NautilusFile *file;
	char *group;

        NautilusPropertiesWindow *window = NAUTILUS_PROPERTIES_WINDOW (data);
...
unschedule_or_cancel_group_change (NautilusPropertiesWindow *window)
{
	...
	EEL_SOURCE_REMOVE_IF_THEN_ZERO (window->details->group_change_timeout);
}
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
combo_box_row_separator_func (GtkTreeModel *model, GtkTreeIter  *iter, void   *data)
{
    char *text;
    _Bool ret;
...
unschedule_or_cancel_owner_change (NautilusPropertiesWindow *window)
{
	...
	EEL_SOURCE_REMOVE_IF_THEN_ZERO (window->details->owner_change_timeout);
}
...
directory_contents_value_field_update (NautilusPropertiesWindow *window)
{
        ...
        goffset total_size;
        _Bool used_two_lines;
        NautilusFile *file;
        GList *l;
        unsigned int file_unreadable;
        goffset file_size;
        ...
        if (total_count == 0) {
                ...
                char *size_str;
                int prefix;
                prefix = nautilus_settings_get_size_prefixes();
...
/* Is really _Bool but glib errently defines gboolean as int */
static int
update_directory_contents_callback (void   *data)
{
	NautilusPropertiesWindow *window;
...
create_basic_page (NautilusPropertiesWindow *window)
{
	...
	hbox = create_page_with_hbox (window->details->notebook, _("Basic"),
				      "help:gnome-help/nautilus_preferences-basic");
...
update_permissions (NautilusPropertiesWindow *window,
		    unsigned int vfs_new_perm,
		    unsigned int vfs_mask,
		    _Bool is_folder,
		    _Bool apply_to_both_folder_and_dir,
		    _Bool use_original)
{
	GList *l;
...
initial_permission_state_consistent (NautilusPropertiesWindow *window,
				     unsigned int mask,
				     _Bool is_folder,
				     _Bool both_folder_and_dir)
{
	GList *l;
	_Bool first;
	unsigned int first_permissions;
			  ...
			  GtkToggleButton *button)
{
	...
	_Bool all_set;
	_Bool all_unset;
	_Bool all_cannot_set;
	_Bool is_folder, is_special;
	_Bool no_match;
	_Bool sensitive;
	unsigned int button_permission;
...
add_permissions_checkbox_with_label (NautilusPropertiesWindow *window,
				     ...
				     GtkWidget *sibling,
				     const char *label,
				     unsigned int permission_to_check,
				     GtkLabel *label_for,
				     _Bool is_folder)
{
	GtkWidget *check_button;
...
static unsigned int vfs_perms[3][3] = {
...
permission_from_vfs (PermissionType type, unsigned int vfs_perm)
{
...
permission_combo_changed (GtkWidget *combo, NautilusPropertiesWindow *window)
{
	GtkTreeIter iter;
	GtkTreeModel *model;
	_Bool is_folder, use_original;
	PermissionType type;
	int new_perm, mask;
	unsigned int vfs_new_perm, vfs_mask;
			 ...
			 GtkComboBox *combo)
{
	...
	_Bool is_folder, no_files, no_dirs, all_file_same, all_dir_same, all_same;
	_Bool all_dir_cannot_set, all_file_cannot_set, sensitive;
	GtkTreeIter iter;
	int mask;
	GtkTreeModel *model;
	GtkListStore *store;
...
add_permissions_combo_box (NautilusPropertiesWindow *window, GtkGrid *grid,
			   PermissionType type, _Bool is_folder,
			   _Bool short_label)
{
	GtkWidget *combo;
	GtkLabel *label;
...
static void
set_recursive_permissions_done (_Bool success, void *callback_data)
{
	NautilusPropertiesWindow *window;
...
apply_recursive_clicked (GtkWidget *recursive_button,
			 NautilusPropertiesWindow *window)
{
	unsigned int file_permission, file_permission_mask;
	unsigned int dir_permission, dir_permission_mask;
	unsigned int vfs_mask, vfs_new_perm, p;
	GtkWidget *button, *combo;
	_Bool active, is_folder, is_special, use_original;
	GList *l;
	GtkTreeModel *model;
	GtkTreeIter iter;
	PermissionType type;
...
create_permissions_page (NautilusPropertiesWindow *window)
{
	...
	vbox = create_page_with_vbox (window->details->notebook,
				      _("Permissions"),
				      "help:gnome-help/nautilus_preferences-permissions");
	...
	if (all_can_get_permissions (file_list) && all_can_get_permissions (window->details->target_files)) {
		...
		if (eel_settings_get_boolean (nautilus_preferences, NAUTILUS_PREFERENCES_SHOW_ADVANCED_PERMISSIONS)) {
...
static void
file_changed_callback (NautilusFile *file, void   * user_data)
{
...
static void
cancel_create_properties_window_callback (void   * callback_data)
{
	remove_pending ((StartupData *)callback_data, TRUE, FALSE, TRUE);
}

static void
parent_widget_destroyed_callback (GtkWidget *widget, void   * callback_data)
{
...
static void
cancel_call_when_ready_callback (void   * key,
				 void   * value,
				 void   * user_data)
{
	       ...
	       int        response)
{
	...
	case GTK_RESPONSE_HELP:
		...
		gtk_show_uri (gtk_window_get_screen (GTK_WINDOW (dialog)),
			      ...
			      &error);
		if (error != NULL) {
			eel_show_error_dialog (_("<b>There was an error displaying help.</b>"),
                                               error->message, "Nautilus", GTK_WINDOW (dialog));
			g_clear_error (&error);
		}
		break;
...
real_destroy (GtkWidget *object)
{
	...
	EEL_SOURCE_REMOVE_IF_THEN_ZERO(window->details->update_directory_contents_timeout_id);

	EEL_SOURCE_REMOVE_IF_THEN_ZERO(window->details->update_files_timeout_id);
			...
			scale = (double)gdk_pixbuf_get_height (pixbuf) /
				gdk_pixbuf_get_width (pixbuf);
                   ...
                   scaled_pixbuf = gdk_pixbuf_scale_simple (pixbuf,
                                                            ...
                                                            GDK_INTERP_BILINEAR);
/*
			scaled_pixbuf = nautilus_thumbnail_scale_down_pixbuf
				(pixbuf,
				 PREVIEW_IMAGE_WIDTH,
				 scale * PREVIEW_IMAGE_WIDTH);
*/
			      ...
			      NautilusPropertiesWindow *window)
{
	...
	if (dialog == NULL) {
		...
		g_object_add_weak_pointer (G_OBJECT (dialog),
					   (void   * *) &window->details->icon_chooser);
	}
