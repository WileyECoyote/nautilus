...
static void
fill_menu (NautilusWindow *window, GtkWidget *menu, _Bool back)
{
...
static void
show_menu (NautilusNavigationAction *self, unsigned int button, unsigned int event_time)
{
	NautilusWindow *window;
	GtkWidget *menu;
	...
	case NAUTILUS_NAVIGATION_DIRECTION_BACK:
 	...
 	case NAUTILUS_NAVIGATION_DIRECTION_EDIT:
 		return;
	default:
		BUG_IMSG("Bad case", self->priv->direction);
		break;
	}
...
{
  EEL_SOURCE_REMOVE_IF_THEN_ZERO (self->priv->popup_timeout_id);
}
