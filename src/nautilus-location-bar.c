...
G_DEFINE_TYPE (NautilusLocationBar, nautilus_location_bar, GTK_TYPE_BOX);
                        ...
                        void             *callback_data)
{
  ...
  if (selection_data != NULL) {
    ...
    if (info == NAUTILUS_DND_URI_LIST || info == NAUTILUS_DND_TEXT_PLAIN ) {
      entry_text = nautilus_location_bar_get_location (self);
      gtk_selection_data_set (selection_data,
                              gtk_selection_data_get_target (selection_data),
                              8, (unsigned char *) entry_text,
                              strlen (entry_text));
      g_free (entry_text);
    }
  }
...
static _Bool
label_button_pressed_callback (GtkWidget *widget, GdkEventButton *event)
{
...
static void
editable_activate_callback (GtkEntry *entry, void * user_data)
{
...
static void
editable_changed_callback (GtkEntry *entry, void *user_data)
{
...
finalize (GObject *object)
{
	...
	EEL_SOURCE_REMOVE_IF_THEN_ZERO (bar->details->idle_id);
