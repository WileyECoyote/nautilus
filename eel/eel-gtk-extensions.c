...
static int
eel_tooltip_paint_window (GtkWindow *window, cairo_t *cr)
{
  ...
  maybe_update_shape (GTK_WIDGET(window));

  return FALSE;
}
GtkWidget*
eel_setup_gtk_tooltip_window (GtkWindow *window, int margins, int padding)
{
  GtkWidget       *box;
  GdkScreen       *screen;
  GdkVisual       *visual;
  ...
  gtk_widget_set_name (GTK_WIDGET(window), "gtk-tooltip-window");

  context = gtk_widget_get_style_context (widget);
  gtk_style_context_add_class (context, GTK_STYLE_CLASS_TOOLTIP);

  box = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, padding);
  gtk_widget_set_margin_left (box, margins);
  gtk_widget_set_margin_right (box, margins);
  gtk_widget_set_margin_top (box, margins);
  gtk_widget_set_margin_bottom (box, margins);
  gtk_container_add (GTK_CONTAINER (window), box);

  g_signal_connect_swapped (window, "draw",
                            G_CALLBACK (eel_tooltip_paint_window), window);
  gtk_widget_show (box);
  return box;
}
/**
