...
eel_strdup_strftime (const char *format, struct tm *time_pieces)
{
	...
	_Bool strip_leading_zeros, turn_leading_zeros_to_spaces;
	char modifier;
	int i;
...
eel_get_system_time (void)
{
	...
	return (signed long)tmp.tv_usec + (signed long)tmp.tv_sec * G_GINT64_CONSTANT (1000000);
}
			    ...
			    gpointer     user_data)
{
	...
	value = eel_settings_get_strv (settings, key);
...
eel_self_check_glib_extensions (void)
{
  ...
  /* ------------------------- Signals ------------------------- */
_Bool
eel_signal_handler_block (void *func, unsigned int handler_id)
{
  GTypeInstance *instance = func;
  _Bool result = FALSE;

  if (instance && g_type_check_instance(instance)) {

     if ( handler_id > 0) {

       if (g_signal_handler_is_connected(func, handler_id)) {
         g_signal_handler_block(func, handler_id);
       }
       result = TRUE;
     }
  }
...
_Bool
eel_signal_handler_unblock (void *func, unsigned int handler_id)
{
  GTypeInstance *instance = func;
  _Bool result = FALSE;
  ...
  if (instance && g_type_check_instance(instance)) {
     ...
     if ( handler_id > 0) {
       ...
       if (g_signal_handler_is_connected(func, handler_id)) {
         g_signal_handler_unblock(func, handler_id);
       }
       result = TRUE;
     }
  }
...
_Bool
eel_signal_handler_disconnect (void *func, unsigned int handler_id)
{
  GTypeInstance *instance = func;
  _Bool result = FALSE;
  ...
  if (instance && g_type_check_instance(instance)) {
     ...
     if ( handler_id > 0) {
       ...
       if (g_signal_handler_is_connected(func, handler_id)) {
         g_signal_handler_disconnect(func, handler_id);
       }
       result = TRUE;
     }
  }
...
_Bool
eel_source_remove (unsigned int tag)
{
  GSource *source;
...
eel_get_filename(GFile *file)
{
    ...
    if (file != NULL) {
        ...
        buffer = g_file_get_parse_name(file);
        ...
        for(tail = head; tail < len; tail++) {
            if (buffer[tail] == '\0' ||
                buffer[tail] == bad_tail ||
                buffer[tail] == lame_tail) {  break;
            }
        }

        filename = g_strndup (buffer + head, tail - head); /* copy just the file spec */
        len = strlen(filename);
