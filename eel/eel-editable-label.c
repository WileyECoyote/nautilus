...
static int     eel_editable_label_draw                    (GtkWidget             *widget,
							    cairo_t               *cr);
...
static int     eel_editable_label_button_press            (GtkWidget             *widget,
							    GdkEventButton        *event);
static int     eel_editable_label_button_release          (GtkWidget             *widget,
							    GdkEventButton        *event);
static int     eel_editable_label_motion                  (GtkWidget             *widget,
							    GdkEventMotion        *event);
static int     eel_editable_label_key_press               (GtkWidget             *widget,
							    GdkEventKey           *event);
static int     eel_editable_label_key_release             (GtkWidget             *widget,
							    GdkEventKey           *event);
static int     eel_editable_label_focus_in                (GtkWidget             *widget,
							    GdkEventFocus         *event);
static int     eel_editable_label_focus_out               (GtkWidget             *widget,
							    GdkEventFocus         *event);
...
static int     eel_editable_label_move_forward_word       (EelEditableLabel      *label,
							    int                   start);
static int     eel_editable_label_move_backward_word      (EelEditableLabel      *label,
							    int                   start);
...
static void     editable_real_set_position    (GtkEditable *editable,
					       int         position);
static int     editable_get_position         (GtkEditable *editable);
	  ...
	  if (set_width > 0)
	      ...
	      if (longest_paragraph > 0)
		{
		  ...
		  if (perfect_width < width)
			...
			{
			  int mid_width = (perfect_width + width) / 2;
...
static void
eel_editable_label_draw_cursor (EelEditableLabel  *label, cairo_t *cr, int xoffset, int yoffset)
{
...
eel_editable_label_select_word (EelEditableLabel *label)
{
  ...
  int start_index = eel_editable_label_move_backward_word (label, label->selection_end);
  int end_index = eel_editable_label_move_forward_word (label, label->selection_end);
...
eel_editable_label_move_logically (EelEditableLabel *label,
				   ...
				   int      count)
{
  int offset = g_utf8_pointer_to_offset (label->text,
					  label->text + start);
...
{
  GtkEditable *editable = GTK_EDITABLE (label);
  int start_pos = label->selection_anchor;
  int end_pos = label->selection_anchor;
...
static gboolean
eel_editable_label_accessible_set_caret_offset (AtkText *text, int offset)
{
  GtkWidget *widget;
						...
						int    selection_num)
{
  GtkWidget *widget;
  EelEditableLabel *label;
  int select_start, select_end, caret_pos;
						     ...
						     AtkCoordType coords)
{
  ...
  int index, cursor_index, x_layout, y_layout;
