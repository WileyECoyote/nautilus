...
/* A _Bool variant for bit fields. WEH: is there some other kind of bit besides boolean? */
typedef unsigned int eel_boolean_bit;

/* Predicate. */
typedef _Bool (* EelPredicateFunction) (void *arg1, void *arg2);
typedef void  (* EelHFunc)             (void *key, void *value, void *user_data);
/* Date & time functions. */
...
/* GList functions. */
_Bool    eel_g_list_exactly_one_item                    (GList                 *list);
_Bool    eel_g_list_more_than_one_item                  (GList                 *list);
_Bool    eel_g_list_equal                               (GList                 *list_a,
                                                         GList                 *list_b);
_Bool    eel_g_lists_sort_and_check_for_intersection    (GList                **list_a,
                                                         GList                **list_b);
...
_Bool       eel_g_str_list_equal                        (GList                 *str_list_a,
                                                         GList                 *str_list_b);
...
int         eel_g_strv_find                             (char                 **strv,
                                                         const char            *find_me);
_Bool       eel_g_strv_equal                            (char                 **a,
                                                         char                 **b);

/* return the time in microseconds since the machine was started */
signed long eel_get_system_time                         (void);

/* math */
int         eel_round                                   (double                 d);

/* Better weak pointer functions */
void        eel_add_weak_pointer                        (void                  *pointer_location);
void        eel_remove_weak_pointer                     (void                  *pointer_location);

void        eel_g_settings_add_auto_strv_as_quarks      (GSettings             *settings,
                                                         const char            *key,
                                                         GQuark               **storage);
_Bool       eel_signal_handler_block                    (void                  *func,
                                                         unsigned int           handler_id);
_Bool       eel_signal_handler_unblock                  (void                  *func,
                                                         unsigned int           handler_id);
_Bool       eel_signal_handler_disconnect               (void                  *func,
                                                         unsigned int           handler_id);
_Bool       eel_remove_source                           (unsigned int           tag);

/* File  functions */
char       *eel_get_filename                            (GFile *file);
