/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   eel-glib-macros.h: Macros to reduce boilerplate when using GTK.
...
#ifndef EEL_GLIB_MACROS_H
#define EEL_GLIB_MACROS_H
...
#define EEL_BLOCK_SIGNAL_HANDLER(func, id_tag) \
  if ( !eel_signal_handler_block (func, id_tag)) { \
    fprintf(stderr, "%s id <%u> not found %s\n", __func__, id_tag, #func); \
  }

#define EEL_UNBLOCK_SIGNAL_HANDLER(func, id_tag) \
  if ( !eel_signal_handler_unblock (func, id_tag)) { \
    fprintf(stderr, "%s id <%u> not found %s\n", __func__, id_tag, #func); \
  }
...
#define EEL_BLOCK_SIGNAL_HANDLER(func, id_tag) \
    eel_signal_handler_block (func, id_tag)
...
#define EEL_DISCONNECT_HANDLER_THEN_ZERO( func, id_tag) \
  eel_signal_handler_disconnect(func, id_tag); \
  id_tag = 0;

#define EEL_DISCONNECT_HANDLER_IF_THEN_ZERO(func, id_tag) \
  if ( id_tag != 0 ) { \
    EEL_DISCONNECT_HANDLER_THEN_ZERO (func, id_tag); \
  }

  /* ------------------------- Threads ------------------------- */
#define EEL_SOURCE_REMOVE(id_tag) \
  if ( !eel_source_remove(id_tag)) { \
    fprintf(stderr, "%s %s line %d source id <%u> not found\n", \
     __FILE__, __func__, __LINE__, id_tag); \
  }

#define EEL_SOURCE_REMOVE_IF(id_tag) \
  if ( id_tag != 0 ) { \
    EEL_SOURCE_REMOVE (id_tag); \
  }

#define EEL_SOURCE_REMOVE_THEN_ZERO(id_tag) \
  EEL_SOURCE_REMOVE (id_tag) \
  id_tag = 0;

#define EEL_SOURCE_REMOVE_IF_THEN_ZERO(id_tag) \
  if ( id_tag != 0 ) { \
    EEL_SOURCE_REMOVE_THEN_ZERO (id_tag); \
  }

#endif /* EEL_GLIB_MACROS_H */
