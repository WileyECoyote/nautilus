...
/*! \file eel-stock-dialogs.c: Various utility dialogs for Eel.
 *
 ...
 * Date:    05/05/2014
 *
 * \remarks Redo to reduce G'ism, add support for pango markup.
 */
...
const char* IDS_MESSEAGE_TITLES[] = {
  "Information", "Warning", "Confirmation", "Error", "Nautilus", /* Message Title Strings*/
  NULL
};
...
typedef struct {
	...
	unsigned int timeout_handler_id;

	/* Window, once it's created. */
	GtkDialog *dialog;

	/* system time (microseconds) when dialog was created */
	signed long dialog_creation_time;

} TimedWait;

static GHashTable *timed_wait_hash_table;

static void timed_wait_dialog_destroy_callback (GtkWidget *object, void *callback_data);

static unsigned int
timed_wait_hash ( const void *value)
{
...
static int
timed_wait_hash_equal ( const void *value1, const void *value2)
{
...
static void
timed_wait_delayed_close_destroy_dialog_callback (GtkWidget *object, void *callback_data)
{
  unsigned int id_tag;
  id_tag = GPOINTER_TO_UINT (callback_data);
  EEL_SOURCE_REMOVE_IF_THEN_ZERO (id_tag);
}

static int
timed_wait_delayed_close_timeout_callback (void *callback_data)
{
...
{
	unsigned int delayed_close_handler_id;
	guint64 time_up;
	...
	EEL_SOURCE_REMOVE_IF_THEN_ZERO (wait->timeout_handler_id);
...
static void
timed_wait_dialog_destroy_callback (GtkWidget *object, void *callback_data)
{
	TimedWait *wait;
...
static int
timed_wait_callback (void *callback_data)
{
	TimedWait *wait;
	GtkDialog *dialog;
	const char *button;
...
void
eel_timed_wait_stop (EelCancelCallback cancel_callback, void *callback_data)
{
	TimedWait key;
	TimedWait *wait;
...
int
eel_run_simple_dialog (GtkWidget *parent, _Bool ignore_close_box,
...
static GtkDialog *
create_message_dialog (const char *title,
		       ...
		       GtkWindow *parent)
{
  ...
  g_object_set (dialog, "text", primary_text,
                "secondary-text", secondary_text,
                NULL);
  ...
  if(title) {
    ...
    gtk_window_set_title(GTK_WINDOW(dialog), _(IDS_MESSEAGE_TITLES[type]));
  }
...
static GtkDialog *
show_message_dialog (const char *title,
                    const char *primary_text,
                    const char *secondary_text,
                    GtkMessageType type,
                    GtkButtonsType buttons_type,
                    ...
                    GtkWindow *parent)
{
  ...
  _Bool msg_1_has_markup = FALSE;
  _Bool msg_2_has_markup = FALSE;

  if (strstr(primary_text, "</"))
    msg_1_has_markup = TRUE;

  if (strstr(secondary_text, "</"))
  ...
  if (parent != NULL && GTK_IS_WIDGET(parent)) {
    if (gtk_widget_is_toplevel (GTK_WIDGET(parent))) {
      gtk_window_set_transient_for (GTK_WINDOW (dialog), parent);
    }
    else {
      top_widget = gtk_widget_get_toplevel (GTK_WIDGET(parent));
      if (GTK_IS_WINDOW (top_widget)) {
        gtk_window_set_transient_for (GTK_WINDOW (dialog), GTK_WINDOW (top_widget));
      }
    }
  }
...
static GtkDialog*
show_ok_dialog (const char *title,
                const char *primary_text,
                const char *secondary_text,
		GtkMessageType type, GtkWindow *parent)
{
	GtkDialog *dialog;

	dialog = show_message_dialog (title, primary_text, secondary_text, type,
...
eel_create_info_dialog (const char *primary_text,
                        ...
                        GtkWindow  *parent)
{
  return show_message_dialog (title,
                                   ...
                                   GtkWindow  *parent)
{
  ...
  if (detailed_info == NULL || strcmp (primary_text, detailed_info) == 0) {
    dialog = eel_show_info_dialog (title, primary_text, secondary_text, parent);
  }
  else
    dialog = show_message_dialog (title,
                                  primary_text,
                                  secondary_text,
                                  GTK_MESSAGE_INFO,
                                  GTK_BUTTONS_OK,
                                    ...
                                    GtkWindow  *parent)
{
  ...
  if (detailed_error_message == NULL || strcmp (primary_text, detailed_error_message) == 0)
  {
    dialog = eel_show_error_dialog (title, primary_text, secondary_text, parent);
  }
  else {
    dialog = show_message_dialog (title,
                                  primary_text,
 ...
 * @title: Optional title for the dialog, can be NULL
 * @parent: The parent window for this dialog, can be NULL.
 */
GtkDialog *
                            ...
                            GtkWindow  *parent)
{
  ...
  dialog = create_message_dialog (title,
                                  primary_text,
                                  secondary_text,
